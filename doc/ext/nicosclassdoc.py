...
class PyDerivedParameter(PyAttribute):
    def handle_signature(self, sig, signode):
        ...
        refnode = addnodes.pending_xref(
            "", refdomain="py", reftype="attr", reftarget=name
        )
...
class NicosClassDocumenter(ClassDocumenter):
    ...
    def add_directive_header(self, sig):
        ...
        if not self.doc_as_attr:
            ...
            if self.object.__bases__:
                ...
                if bases:
                    self.add_line("   Bases: %s" % ", ".join(bases), "<autodoc>")
    ...
    def _format_methods(self, methodlist, orig_indent):
        ...
        for name, (args, doc, funccls, is_usermethod) in sorted(methodlist):
            ...
            if not doc:
                logger.warning(
                    "%s.%s: usermethod has no documentation" % (myclsname, name)
                )
    ...
    def _format_attached_devices(self, attachedlist):
        ...
        for adev, attach in sorted(attachedlist):
            if (
                isinstance(attach.multiple, list) and attach.multiple[0] > 1
    ...
    def document_members(self, all_members=False):
        if (
            not issubclass(self.object, Device)
            and not issubclass(self.object, DeviceMixinBase)
            and not issubclass(self.object, NicosWidget)
        ):
        ...
        self.env.nicos_all_devices[self.object.__name__, self.object.__module__] = {
            "docname": self.env.docname,
            "name": self.object.__name__,
            "module": self.env.ref_context.get("py:module", self.object.__module__),
            "blurb": docstr.split("\n\n", maxsplit=1)[0],
        }
    ...
    def _format_properties(self, obj, orig_indent):
        ...
        for prop in sorted(dir(obj.__class__)):
            ...
            self.add_line(
                ".. parameter:: %s : %s" % (prop, ", ".join(addinfo)), "<autodoc>"
            )
            self.add_line("", "<autodoc>")
            descr = info.doc or ""
    ...
    def _format_parameters(self, paraminfolist, orig_indent):
        for param, info in paraminfolist:
            ...
            if info.derived:
                self.add_line(
                    ".. derivedparameter:: %s.%s : %s"
                    % (info.classname, param, ", ".join(addinfo)),
                    "<autodoc>",
                )
            else:
                self.add_line(
                    ".. parameter:: %s : %s" % (param, ", ".join(addinfo)), "<autodoc>"
                )
            self.add_line("", "<autodoc>")
            self.indent += self.content_indent
...
def resolve_devindex(app, doctree, fromdocname):
    ...
    for indexnode in doctree.traverse(device_index):
        ...
        for entry in sorted(env.nicos_all_devices.values(), key=lambda v: v["name"]):
            ...
            row.append(
                nodes.entry(
                    "", nodes.paragraph("", "", nodes.literal("", entry["module"][6:]))
                )
            )
...
def purge_devindex(app, env, docname):
    ...
    env.nicos_all_devices = dict(
        (k, e) for (k, e) in env.nicos_all_devices.items() if e["docname"] != docname
    )
