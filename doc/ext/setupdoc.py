...
class SetupDirective(Directive):
    ...
    def _readSetup(self):
        ...
        if setup_roots in all_setups_cache:
            ...
            all_setups_cache[setup_roots] = all_setups = dict(iterSetups(setup_roots))
    ...
    def _buildIncludesBlock(self, setupInfo):
        setupInfo["includes"] = ", ".join(
            self._buildSetupLink(setup) for setup in setupInfo["includes"]
        )
        return RST_INCLUDES % setupInfo if setupInfo["includes"] else ""

    def _buildExcludesBlock(self, setupInfo):
        setupInfo["excludes"] = ", ".join(
            self._buildSetupLink(setup) for setup in setupInfo["excludes"]
        )
        return RST_EXCLUDES % setupInfo if setupInfo["excludes"] else ""

    def _buildModulesBlock(self, setupInfo):
        setupInfo["modules"] = ", ".join(
            self._buildModuleLink(module) for module in setupInfo["modules"]
        )
    ...
    def _buildDevicesBlock(self, setupInfo):
        ...
        for devName, (devClass, devParams) in sorted(
            devices_dict.items(), key=lambda d: d[0].lower()
        ):
            ...
            rst.append("")
            rst.append(
                "Device class: :class:`%s.%s`" % (klass.__module__, klass.__name__)
            )
            rst.append("")
            ...
            for param_name in sorted(klass.parameters):
                ...
                if param_name in devParams:
                    ...
                    paramRows += [
                        (
                            self._buildParamLink(param_name, param_info),
    ...
    def _buildSetupLink(self, setup_name):
        shortPath = "%s.py" % path.join(path.dirname(self._shortSetupPath), setup_name)
        fullPath = self._getAbsoluteSetupPath(shortPath)

        if path.exists(fullPath):
            setuplink = self._getUniqueSetupId(shortPath)
            return RST_SETUP_LINK % {"setupname": setup_name, "setuplink": setuplink}
        return setup_name
    ...
    def _getAbsoluteSetupPath(self, shortpath):
        ...
        """
        return path.join(
            self.env.srcdir, self.env.config.setupdoc_setup_base_dir, shortpath
        )
...
def setupdoc_builder_inited(app):
    ...
    base_dir = (
        Path(app.builder.srcdir).joinpath(app.config.setupdoc_setup_base_dir).resolve()
    )
    ...
    for facility_dir in Path(app.builder.srcdir).glob("nicos_*"):
        ...
        for setup_dir in base_dir.joinpath(facility).rglob("setups"):
            ...
            for setup in added:
                ...
                try:
                    rstpath.parent.mkdir(mode=0o755, parents=True, exist_ok=True)
                except OSError:
                ...
                with rstpath.open("w", encoding="utf-8") as f:
                    f.write(
                        RST_SETUP_FILE
                        % {"facility": facility, "instr": instr, "setupname": setup}
                    )
...
def setup(app):
    ...
    return {"parallel_read_safe": True, "version": "0.2.0"}
