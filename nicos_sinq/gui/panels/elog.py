...
import elog
from elog import LogbookInvalidMessageID, LogbookMessageRejected, LogbookServerProblem
...
class ElogPanel(Panel):
    ...
    def updateMessagesTable(self):
        ...
        for index, message_id in enumerate(reversed(message_ids)):
            ...
            self.messagesTable.setItem(index, 0, QTableWidgetItem(f"{message_id}"))
            self.messagesTable.setItem(
