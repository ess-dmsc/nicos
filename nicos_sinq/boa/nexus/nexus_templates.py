...
class CaminiFileList(NexusElementBase):
    ...
    def results(self, name, h5parent, sinkhandler, results):
        dset = h5parent[name]
        data = dset[0] + bytes(results["camera"][0][0], encoding="utf-8") + b"\n"
        dset[0] = data
...
class BOATemplateProvider(NexusTemplateProvider):
    ...
    _excluded_devices = ["slit1", "slit2", "dslit", "ccdwww_connector", "ccd_cooler"]
    _detector = None
        ...
        for d in devices:
            ...
            try:
                ...
                content[dev.name] = DeviceDataset(
                    dev.name, dtype="float32", units=NXAttribute(dev.unit, "string")
                )
    ...
    def makeDetector(self):
        ...
        content = {}
        if name == "single":
            content["data"] = DetectorDataset(
                "countval", "int32", units=NXAttribute("counts", "string")
            )
            content["time_stamp"] = AbsoluteTime()
        elif name == "andor":
            content["data"] = ImageDataset(0, 0, signal=NXAttribute(1, "int32"))
        elif name == "ccdwww":
            content["data"] = ImageDataset(0, 0, signal=NXAttribute(1, "int32"))
