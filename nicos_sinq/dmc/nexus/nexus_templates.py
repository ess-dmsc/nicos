...
dmc_base = {
    ...
    "entry:NXentry": {
        ...
        "monitor:NXmonitor": {
            ...
            "monitor1": DetectorDataset(
                "m1", "float32", units=NXAttribute("counts", "string")
            ),
            "monitor2": DetectorDataset(
                "m2", "float32", units=NXAttribute("counts", "string")
            ),
            "time": DetectorDataset(
                "det_timer", "float32", units=NXAttribute("seconds", "string")
            ),
            "proton_charge": DetectorDataset(
                "proton_charge", dtype="float32", units=NXAttribute("uC", "string")
            ),
        },
        "DMC:NXinstrument": {
            "SINQ:NXSource": {
                ...
                "type": ConstDataset(
                    "Continuous flux spallation source", dtype="string"
                ),
                "probe": ConstDataset("neutron", dtype="string"),
            },
            "monochromator:NXmonochromator": {
                "curvature": DeviceDataset("mcv", dtype="float32"),
                "wavelength": DeviceDataset("wavelength_refined", dtype="float32"),
                "wavelength_raw": DeviceDataset("wavelength", dtype="float32"),
                "rotation_angle": DeviceDataset(
                    "a1", dtype="float32", units=NXAttribute("degree", "string")
                ),
                "takeoff_angle": DeviceDataset(
                    "a2", dtype="float32", units=NXAttribute("degree", "string")
                ),
                "type": "Pyrolithic Graphite",
                "translation_lower": DeviceDataset(
                    "mtl", dtype="float32", units=NXAttribute("mm", "string")
                ),
                "translation_upper": DeviceDataset(
                    "mtu", dtype="float32", units=NXAttribute("mm", "string")
                ),
                "goniometer_lower": DeviceDataset(
                    "mgl", dtype="float32", units=NXAttribute("degree", "string")
                ),
                "goniometer_upper": DeviceDataset(
                    "mgu", dtype="float32", units=NXAttribute("degree", "string")
                ),
                "curvature_vertical": DeviceDataset(
                    "mcv", dtype="float32", units=NXAttribute("degree", "string")
                ),
            },
        },
        "data:NXdata": {
    ...
    if "dmccontrol" in session.experiment.detlist:
        return {
            ...
            "integral": DetectorDataset(
                "monitorval", "float32", units=NXAttribute("counts", "string")
            ),
            "preset": DetectorDataset("monitorpreset", "float32"),
            "time": DetectorDataset(
...
def makeDetector():
    ...
    if "dmccontrol" in session.experiment.detlist:
        content["summed_counts"] = DetectorDataset(
            "ccdwww", dtype="int32", units=NXAttribute("counts", "string")
        )
        content["time_stamp"] = (AbsoluteTime(),)

    if "det" in session.experiment.detlist:
        content["summed_counts"] = DetectorDataset(
            "det_image", dtype="int32", units=NXAttribute("det_image", "string")
        )
        content["detector_position"] = DeviceDataset(
