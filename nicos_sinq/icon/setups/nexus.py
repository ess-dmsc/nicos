...
sysconfig = dict(
    datasinks=["nxsink", "quiecksink", "fits"],
)

devices = dict(
    sample_pos1=device(
        "nicos_sinq.devices.componenttable.ComponentTable",
    ...
    image_type=device(
        ...
        moveable="image_key",
        mapping={"projection": 0, "flat_field": 1, "dark_field": 2, "invalid": 3},
        precision=0.1,
    ),
    pixel_size=device(
    ...
    lenses=device(
        ...
        states=["Nikkor 50mm/1.4", "Zeiss Macroplanar 100m/2.0", "Zeiss Otus 55mm/1.4"],
    ),
    scintillator=device(
        "nicos.devices.generic.ManualSwitch",
    ...
    scintillator_thickness=device(
        "nicos.devices.generic.ManualSwitch",
        description="Switch for the thickness of the scintillator",
        states=[10, 20, 30, 50, 100, 200, 300],
    ),
    nxsink=device(
        "nicos.nexus.nexussink.NexusSink",
        description="Sink for NeXus file writer",
        filenametemplate=["icon%(year)sn%(scancounter)06d.hdf"],
        templateclass="nicos_sinq.icon.nexus.nexus_templates.ICONTemplateProvider",
    ),
    quiecksink=device(
        "nicos_sinq.devices.datasinks.QuieckSink",
        description="Sink for sending UDP datafile notifications",
    ),
    fits=device(
        "nicos_sinq.devices.niagfitssink.NIAGFitsSink",
        description="Sink for FITS files",
        detectors=[
            "andor",
        ],
        filenametemplate=["icon%(year)sn%(scancounter)06di%(pointcounter)08d.fits"],
    ),
)
