...
for name, pv in vswritables.items():
    devices["vs_" + name] = device(
        "nicos.devices.epics.pyepics.EpicsDigitalMoveable",
...
for name, pv in vssetpwritable.items():
    devices["vs_" + name] = device(
        "nicos.devices.epics.pyepics.EpicsAnalogMoveable",
...
devices["vs_state"] = device(
    "nicos_sinq.icon.devices.velocity_selector.VSState",
...
devices["vs_speed"] = device(
    "nicos_sinq.icon.devices.velocity_selector.VSSpeed",
...
devices["vs_lambda"] = device(
    "nicos_sinq.icon.devices.velocity_selector.VSLambda",
