...
for col, colpv in segments.items():
    for name, itempv in segwrite.items():
        devices[col + "_io" + name] = device(
            "nicos_sinq.sans.devices.collimator.SegmentMoveable",
            description="%s %s switch device" % (col, name),
            visibility=hide,
            writepv=pvprefix + colpv + itempv,
            readpv=pvprefix + colpv + itempv + "_RBV",
        )
        devices[col + "_" + name] = device(
            "nicos_sinq.sans.devices.collimator.SegmentPulse",
    ...
    devices[col] = device(
        "nicos_sinq.sans.devices.collimator.Segment",
...
devices["pol"] = device(
    "nicos_sinq.sans.devices.collimator.Polariser",
    description="Switch for polariser",
    cols1="cols1",
)
devices["coll"] = device(
    "nicos_sinq.sans.devices.collimator.Collimator",
