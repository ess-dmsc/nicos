...
class RadialCollimator(EpicsDevice, MappedMoveable):
    ...
    parameters = {
        ...
        "target": Param(
            "Target value for the radial collimator",
            type=oneof("on", "off"),
            mandatory=False,
            volatile=False,
            default="off",
        ...
        "type": Param(
            "Type of the radial collimator", type=str, userparam=True, settable=True
        ),
        "readback": Param(
            "Radial Collimator Status Readback",
            type=oneof("on", "off"),
            volatile=True,
            default="off",
            userparam=True,
        ...
        "automatic": Param(
            "Flag for switching radial collimator on when " "counting",
            type=bool,
            settable=True,
            userparam=True,
            default=False,
        ...
        "radialcheck": Param(
            "Flag for automatically checking the radial " "collimator on count",
            type=bool,
            settable=True,
            default=False,
            userparam=True,
    ...
    def startcheck(self):
        if not self.radialcheck:
            session.log.warning(
                "Checking radial collimator switched off on " "user request"
            )
            return False
    ...
    def stopcheck(self):
        ...
        if not self.automatic:
            return
        if self.read() == "off":
            session.log.warning("Radial collimator stoppped during " "measurement")
