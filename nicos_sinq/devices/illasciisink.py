...
class ILLAsciiHandler(DataSinkHandler):
    ...
    def _getMLZCryst(self):
        ...
        if mode == "CKI":
            ...
            result["FX"] = 1
        result["KFIX"] = self._findValue(session.instrument.name, "scanconstant")
        sample = session.getDevice("Sample")
        result["V1"] = getattr(sample, "orient1")
        result["V2"] = getattr(sample, "orient2")
        result["CELL"] = getattr(sample, "lattice") + getattr(sample, "angles")
        return result

    def _getUBCryst(self):
        result = {}
        refs = self._findValue(session.instrument.name, "orienting_reflections")
        ...
        result["ss"] = self._findValue(session.instrument.name, "scattering_sense")
        result["CELL"] = [
            self._findValue("Sample", p)
            for p in ["a", "b", "c", "alpha", "beta", "gamma"]
        ]
        result["UB"] = self._findValue("Sample", "ubmatrix")
        result["PN"] = self._findValue(session.instrument.name, "plane_normal")
        return result
        ...
        for det in self.sink.scaninfo:
            if det in subset.values:
                ...
                for idx, v in enumerate(subset.detvalueinfo):
                    ...
                    session.log.warning(
                        "Detector %s not found when" "saving scan data", det
                    )
        ...
        with open(self.dataset.filepaths[0], "w", encoding="utf-8") as out:
            ...
            for i in range(np):
                ...
                for dev in self.dataset.devices:
                    ...
                    if dev == session.instrument:
                        line += (
                            "%9.4f %9.4f %9.4f %9.4f " % self.scanvalues[dev.name][i]
                        )
                    else:
                        try:
...
class ILLAsciiSink(FileSink):
    ...
    parameters = {
        "scaninfo": Param(
            "NICOS devices for M1, M2, CNTS, TIME and M3 in " "that order",
            type=listof(nicosdev),
            mandatory=True,
        ),
        "varia": Param("List of variables to write", type=listof(str), mandatory=True),
    }
