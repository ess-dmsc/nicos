...
class WaitPV(SequenceItem):
    ...
    def isCompleted(self):
        ...
        if self._timeout != 0 and monotonic() > self._endtime:
            ...
            return True
        return bool(self._camini.readPV(self._pv) == self._value or self._stopped)
...
class CaminiDetector(EpicsDevice, SequencerMixin, Detector):
    ...
    parameters = {
        "trigpv": Param(
            "PV to send a trigger signal", type=pvname, mandatory=True, userparam=False
        ),
        "validpv": Param(
            "PV to inform whether the metadata is valid",
            type=pvname,
            mandatory=True,
            userparam=False,
        ),
        "metapv": Param(
            "PV containing the metadata", type=pvname, mandatory=True, userparam=False
        ),
        "shutpv": Param(
            "PV to monitor the camera shutter",
            type=pvname,
            mandatory=True,
            userparam=False,
        ),
        "armpv": Param(
            "PV to monitor whether the camera is ready",
            type=pvname,
            mandatory=True,
            userparam=False,
        ),
        "filepv": Param(
            "PV for the image filename", type=pvname, mandatory=True, userparam=False
        ),
        "arm_timeout": Param(
            "Timeout when waiting for the camera to be ready",
            type=float,
            mandatory=True,
            userparam=True,
            settable=True,
        ),
        "shutter_timeout": Param(
            "Timeout for the shutter to open",
            type=float,
            mandatory=True,
            userparam=True,
            settable=True,
        ),
        "exposure_timeout": Param(
            "Image exposure timeout > exposure_time",
            type=float,
            mandatory=True,
            userparam=True,
            settable=True,
    ...
    def doStatus(self, maxage=0):
        ...
        if self._seq_is_running():
            ...
            if stat[0] == status.DISABLED:
                session.log.error("Automatic shutter requested, but shutter disabled")
                self.stop()
                return stat
    ...
    def _generateSequence(self):
        ...
        s = WaitThreshold(
            self, self._attached_beam_current, self._attached_rate_threshold.read()
        )
        seq.append(s)
    ...
    def doRead(self, maxage=0):
        return [
            self._pvs["filepv"].get(
                timeout=self.epicstimeout, as_string=True, count=256
            ),
            self._timestamp,
        ]
        ...
        for s in setups_log:  # pylint: disable=consider-using-dict-items
            for dev in setups_log[s]:
                ...
                try:
                    ...
                    session.log.warning(
                        "Failed to read dev.name, %s," " with %s", dev.name, e
                    )
    ...
    def valueInfo(self):
        return (
            Value("Image File", unit=self.unit, fmtstr="%s"),
