    ...
    def addSubset(self, subset):
        ...
        for i in range(len(self.dataset.devices)):
            ...
            if isinstance(dev, SXTalBase):
                ...
                self.scanvalues[dev.name].append(
                    tuple(subset.devvaluelist[i : i + datalen])
                )
            else:
                ...
                if len(vals) > 1:
                    if isinstance(vals[0], tuple):
                        hklstep = [
                            v1 - v2
                            for v1, v2 in zip(
                                self.dataset.startpositions[1][0],
...
class SINQAsciiSink(FileSink):
    ...
    parameters = {
        "templatefile": Param(
            "Path to SICS style template file", type=str, mandatory=True
        ),
        "scaninfo": Param(
            "Header text and nicos device for each scan point",
