...
class Mupad(Moveable):
    ...
    parameters = {
        "w1": Param(
            "Weight 1", type=float, default=10.3, settable=True, userparam=True
        ),
        "w2": Param(
            "Weight 2", type=float, default=10.3, settable=True, userparam=True
        ),
        "w3": Param(
            "Weight 3", type=float, default=10.3, settable=True, userparam=True
        ),
        "w4": Param(
            "Weight 4", type=float, default=10.3, settable=True, userparam=True
        ),
        "p1": Param(
            "Phase shift 1", type=float, default=0, settable=True, userparam=True
        ),
        "p2": Param(
            "Phase shift 2", type=float, default=0, settable=True, userparam=True
        ),
        "p3": Param(
            "Phase shift 3", type=float, default=0, settable=True, userparam=True
        ),
        "p4": Param(
            "Phase shift 4", type=float, default=0, settable=True, userparam=True
        ),
        "last_currents": Param(
            "last known currents", type=listof(float), settable=True, internal=True
        ),
    }
    ...
    def calc_chi_phi_incoming(self, target):
        ...
        if angle_kiq_cross[2] < 0:
            angle_kiq = angle_kiq * -1.0

        theta = self.angle(pi, np.array([0, 0, 1.0]))

        piv = np.array([target[0], target[1], 0])
        if np.sqrt(piv.dot(piv)) > 0.01:
            phi = self.angle(piv, np.array([1.0, 0, 0]))
        else:
            phi = 0

        phicross = np.cross(np.array([1.0, 0, 0]), piv)
        if phicross[2] < 0:
            phi = phi * -1

        if np.abs(theta) < 0.01 or np.abs(theta - np.pi) < 0.01:
            phi = 0
        else:
            phi = angle_kiq + phi
    ...
    def calc_chi_phi_outgoing(self, target):
        ...
        theta = self.angle(pf, np.array([0.0, 0.0, 1.0]))
        pfp = np.array([pf[0], pf[1], 0.0])
        if np.sqrt(pfp.dot(pfp)) != 0:
            phi = self.angle(pfp, np.array([1.0, 0.0, 0.0]))
        else:
            phi = 0
        pfpcross = np.cross(np.array([1.0, 0.0, 0.0]), pfp)
        if pfpcross[2] < 0:
            phi *= -1
    ...
    def doRead(self, maxage=0):
        ...
        for mag, cur in zip(self._getWaiters(), self.last_currents):
            if not mag.isAtTarget(target=cur):
                self.log.warning(
                    "Magnet current mismatch: should %f, is %f", cur, mag.read(maxage)
                )
                return (-99,) * 6
        return self.target
...
class MuSwitch(TransformedMoveable):
    ...
    valuetype = tupleof(
        oneof("X", "-X", "Y", "-Y", "Z", "-Z"), oneof("X", "-X", "Y", "-Y", "Z", "-Z")
    )

    attached_devices = {"mupad": Attach("Actual mupad device", Mupad)}
...
class MuNumSwitch(MuSwitch):
    valuetype = tupleof(oneof(1, 2, 3, -1, -2, -3), oneof(1, 2, 3, -1, -2, -3))
