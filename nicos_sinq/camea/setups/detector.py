...
devices = dict(
    nxsink=device(
        ...
        templateclass="nicos_sinq.camea.nexus.nexus_templates" ".CameaTemplateProvider",
    ),
    timepreset=device(
    ...
    camea_detector=device(
        "nicos_sinq.devices.sinqhm.channel.ReshapeHistogramImageChannel",
        description="Image channel for area detector",
        visibility=(),
        bank="hm_bank0",
        dimensions={"x": 1024, "y": 104},
        connector="hm_connector",
    ),
    counts=device(
        "nicos.devices.generic.detector.RectROIChannel",
    ...
    cameadet=device(
        ...
        timers=["elapsedtime"],
        monitors=["monitor1", "monitor2", "monitor3", "protoncount", "monitor4"],
