...
class DistancesPlaceholder(PlaceholderBase):
    ...
    def fetch_info(self, metainfo):
        value = DeviceValuePlaceholder("Distances", self.component).fetch_info(metainfo)
        sample = DeviceValuePlaceholder("Distances", "sample").fetch_info(metainfo)
        if (
            sample
            and isinstance(sample[0], float)
            and value
