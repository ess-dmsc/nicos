...
class AmorControlPanel(GenericPanel):
    def __init__(self, parent, client, options):
        ...
        self.monitorPresetBox.setValidator(
            QRegularExpressionValidator(QRegularExpression(r"[\d.]+(?:e\d+)?"), self)
        )
    ...
    def _devsMoveButton(self, dic, cmd="move", issue_separate=False):
        dev_to_widget = {
            n: w for n, w in dic.items() if not isinstance(w._inner, MissingWidget)
        }
        targets = []
        try:
        ...
        expr = ", ".join(
            [n + ".isAtTarget(target=%r)" % v for n, v in zip(dev_to_widget, targets)]
        )
        ...
        move = {n: v for n, v, t in zip(dev_to_widget, targets, on_targ) if not t}

        if not move:
            return

        if issue_separate:
            code = "\n".join(("%s(%s, %r)" % (cmd, n, v) for n, v in move.items()))
        else:
            code = "%s(%s)" % (
                cmd,
                ", ".join(("%s, %r" % (n, v) for n, v in move.items())),
            )
        self.exec_command(code)
