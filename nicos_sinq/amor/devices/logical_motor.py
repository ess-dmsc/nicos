...
class AmorLogicalMotorHandler(InterfaceLogicalMotorHandler):
    ...
    attached_devices = {
        ...
        "distances": Attach("Device that stores the distances", DistancesHandler),
    }
    ...
    def doRead(self, maxage=0):
        ...
        return {M2T: round(actm2t, 3), S2T: round(acts2t, 3), ATH: round(actath, 3)}
    ...
    def _get_move_list(self, targets):
        ...
        if self._is_active("analyser"):
            ...
            sah = abs(distances.analyser - distances.sample)
            aoz = soz + sah * math.tan(math.radians(targets[S2T] - targets[M2T]))
            ...
            if self._is_active("detector"):
                ...
                val = (
                    sah
                    - sqrtsqsum
                    + (sdh - sqrtsqsum) * (math.cos(math.radians(com)) - 1.0)
                )
        ...
        if self._is_active("slit4"):
            ...
            if self._is_active("analyser"):
                val = (
                    soz
                    + dist * math.tan(math.radians(targets[S2T] - targets[M2T]))
                    - 0.5 * mot
                )
            else:
...
class AmorLogicalMotor(Motor):
    ...
    parameters = {
        "motortype": Param(
            "Type of motor ath/m2t/s2t", type=oneof(*motortypes), mandatory=True
        ),
    }
    ...
    attached_devices = {
        "controller": Attach(
            "Controller for the logical motors", AmorLogicalMotorHandler
        )
    }
...
class DetectorAngleMotor(TransformedMoveable):
    ...
    def _mapTargetValue(self, target):
        return -target, math.tan(math.radians(target)) * self.coz_scale_factor
