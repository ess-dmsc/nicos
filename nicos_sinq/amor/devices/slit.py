...
from nicos_sinq.devices.logical_motor import InterfaceLogicalMotorHandler, LogicalMotor
...
class SlitOpening(HasPrecision, DefaultSlitAxis):
    ...
    def doStatus(self, maxage=0):
        ...
        devs = [
            dname for dname, d in self._adevs.items() if d.status()[0] == st_devs[0]
        ]
...
def read_divergence(distance, slit):
    ...
    return {"div": degrees(s + d), "did": degrees((s - d) / 2), "dih": degrees(h)}
...
class AmorSlitHandler(InterfaceLogicalMotorHandler):
    attached_devices = {
        "xs": Attach("Sample x position", Readable, missingok=True, optional=True),
        ...
        "mu_offset": Attach(
            "Sample x position", Readable, missingok=True, optional=True
        ),
        "kappa": Attach(
            "Inclination of the beam after the Selene guide",
            Readable,
            missingok=True,
            optional=True,
        ),
        "soz_ideal": Attach(
            "Ideal sample omega", Readable, missingok=True, optional=True
        ),
    ...
    def doRead(self, maxage=0):
        ...
        if self._is_active("diaphragm1"):
            result.update(
                read_divergence(self._read_dev("xs"), self._read_dev("slit1"))
            )
    ...
    def _get_move_list(self, targets):
        ...
        if self._is_active("diaphragm1"):
            ...
            horizontal = distance * tan(0.5 * radians(dih))
            positions.extend(
                [(self._get_dev("slit1"), (horizontal, horizontal, bottom, top))]
            )
        if self._is_active("diaphragm2"):
            ...
            if self._is_active("deflector"):
                ...
                z = ltz - (distance - xl) * tan(
                    radians(self._read_dev("mu") + mu_offset)
                )
            else:
                z = distance * tan(radians(kappa))
            positions.extend(
                [
                    (self._get_dev("slit2z"), z),
                    (self._get_dev("slit2"), (top, bottom, horizontal, horizontal)),
                ]
            )
...
class AmorSlitLogicalMotor(LogicalMotor):
    ...
    parameters = {
        "motortype": Param(
            "Type of motor %s" % ",".join(motortypes),
    ...
    attached_devices = {
        "controller": Attach("Controller for the logical motors", AmorSlitHandler)
    }
...
class SlitAxis(DefaultSlitAxis):
    ...
    attached_devices = {
        "controller": Attach(
            "Controller, used to connect slit and distance " "to the axis", Device
        ),
    }
...
class DivergenceAperture(HasAutoDevices, Device):
    ...
    class HorizontalDivergence(SlitAxis):
        ...
        def _convertStart(self, target, current):
            distance = self._attached_controller._attached_distance.read()
            tgt = list(current)
            tgt[:2] = [distance * tan(0.5 * radians(target))] * 2
            return tgt
