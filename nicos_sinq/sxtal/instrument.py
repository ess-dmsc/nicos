...
from nicos.devices.sxtal.instrument import SXTalBase as NicosSXTalBase, SXTalIndex

from nicos_sinq.sxtal.singlexlib import (
    calcNBUBFromCellAndReflections,
    calcTheta,
    calcUBFromCellAndReflections,
    eulerian_to_kappa,
    kappa_to_eulerian,
    rotatePsi,
    z1FromAngles,
...
from nicos_sinq.sxtal.tasublib import (
    KToEnergy,
    calcPlaneNormal,
    calcTasQAngles,
    calcTasQH,
    calcTasUBFromTwoReflections,
    energyToK,
    tasAngles,
    tasQEPosition,
    tasReflection,
...
class SXTalBase(NicosSXTalBase):
    ...
    parameters = {
        "scan_width_multiplier": Param(
            "Multiplier for the scan width " "calculated from scan_uvw",
            type=float,
            userparam=True,
            settable=True,
            default=3.0,
        ),
        "center_counter": Param("Counter to use for centering", type=str),
        "center_maxpts": Param(
            "Width for centering", type=int, userparam=True, settable=True, default=60
        ),
        "center_order": Param(
            "Optional order of motors for centering",
        ...
        "center_steps": Param(
            "Step width for each angle to use for centering",
            type=listof(float),
            userparam=True,
            settable=True,
        ...
        "ccl_file": Param(
            "Flag for writing CCL data files",
            type=bool,
            default=False,
            settable=True,
            userparam=True,
        ),
        "orienting_reflections": Param(
            "Reflections from which the UB" "was calculated",
    ...
    def _checkPosList(self, poslist):
        ...
        for devname, value in poslist:
            ...
            if not devok:
                ok = False
                whyinfo.append(
                    "target position %s outside limits "
                    "for %s: %s -- " % (dev.format(value, unit=True), dev, devwhy)
                )
        if not ok:
            why = "---".join(whyinfo)
        return ok, why
...
class EulerSXTal(SXTalBase):
    ...
    parameters = {
        "searchpars": Param(
            "Limits and steps for peak searching",
            dictof(str, tupleof(float, float, float)),
            settable=True,
        ),
        "t2angle": Param(
            "Two theta angle when to switch to t2t mode",
            type=float,
            settable=True,
            userparam=True,
            default=60.0,
        ...
        "use_psi": Param(
            "If to use the psi angle in calculations",
            type=bool,
            settable=True,
            userparam=True,
            default=False,
        ...
        "psi_target": Param(
            "Target for the PSI angle", type=float, internal=True, settable=True
        ),
    }
    ...
    def _calcPos(self, hkl, wavelength=None):
        """Calculate the Z1 vector for a given HKL position."""
        if len(hkl) < 3:
            raise UsageError("Need at least three reciprocal space" " coordinates")
        if len(hkl) > 3:
    ...
    def _extractPos(self, pos):
        om, chi, phi = z1ToBisecting(self.wavelength, pos)
        tth = 2.0 * om
        if self.use_psi:
            om, chi, phi = rotatePsi(om, chi, phi, np.deg2rad(self.psi_target))
        poslist = [
        ...
        if (
            not ok
            and self._attached_ttheta.isAllowed(np.rad2deg(tth))
    ...
    def _readPos(self, maxage=0):
        apos = (
            self._attached_ttheta.read(maxage),
...
class LiftingSXTal(SXTalBase):
    ...
    def _convertPos(self, pos, wavelength=None):
        return z1FromNormalBeam(
            self.wavelength, np.deg2rad(pos[0]), np.deg2rad(pos[1]), np.deg2rad(pos[2])
        )

    def _readPos(self, maxage=0):
        apos = (
            self._attached_gamma.read(maxage),
            self._attached_omega.read(maxage),
...
class KappaSXTal(SXTalBase):
    ...
    parameters = {
        ...
        "kappa_angle": Param(
            "Kappa angle of the goniometer", type=float, mandatory=True
        ),
        "right_hand": Param(
            "Boolean if the goniometer is right handed or not",
            type=bool,
            mandatory=True,
        ),
    }
    ...
    def _extractPos(self, pos):
        eom, echi, ephi = z1ToBisecting(self.wavelength, pos)
        # Chi must be below 2*alpha for the kappa calculation to work.
        # Thus we rotate in PSI until this is the case.
        stt = np.rad2deg(eom) * 2.0
        psi = 10
        psichi = echi
        psiom = eom
        psiphi = ephi
        while psichi > 2.0 * np.deg2rad(self.kappa_angle) and psi < 360:
        ...
        #              np.rad2deg(psiphi))
        status, komega, kappa, kphi = eulerian_to_kappa(
            np.rad2deg(psiom),
            np.rad2deg(psichi),
            np.rad2deg(psiphi),
            self.kappa_angle,
        ...
        return [("stt", stt), ("omega", komega), ("kappa", kappa), ("kphi", kphi)]

    def _convertPos(self, pos, wavelength=None):
        status, om, chi, phi = kappa_to_eulerian(
            pos[1], pos[2], pos[3], self.kappa_angle, self.right_hand
        )
    ...
    def _readPos(self, maxage=0):
        apos = (
            self._attached_stt.read(maxage),
            self._attached_omega.read(maxage),
    ...
    def _refl_to_dict(self, r):
        status, om, chi, phi = kappa_to_eulerian(
            r[1][1], r[1][2], r[1][3], self.kappa_angle, self._right_hand
        )
        if not status:
            self.log.error("Failed to convert from kappa to eulerian angles")
        rd = {}
...
class TASSXTal(SXTalBase):
    ...
    parameters = {
        "inelastic": Param(
            "True for inelastic operation", type=bool, mandatory=True, settable=False
        ),
        "emode": Param(
            "Energy driving mode",
            oneof(*ENERGYMODES),
        ...
        "scattering_sense": Param(
            "Scattering sense at the sample",
            type=oneof(-1, 1),
            settable=True,
            category="instrument",
            default=1,
        ...
        "out_of_plane": Param(
            "Flag if we may move out of plane or not",
            type=bool,
            settable=True,
            category="instrument",
            default=True,
    ...
    parameter_overrides = {
        "unit": Override(default="rlu rlu rlu meV", mandatory=False, settable=True),
    }
    ...
    def _extractPos(self, pos):
        ...
        ub = session.experiment.sample.getUB()
        try:
            angpos = calcTasQAngles(
                ub, np.array(self.plane_normal), self.scattering_sense, 0, qepos
            )
    ...
    def _rfl_to_reflection(self, r):
        ...
        ki = energyToK(r[2][0])
        kf = energyToK(r[2][1])
        qe = tasQEPosition(ki, kf, r[0][0], r[0][1], r[0][2], 0.0)
...
class SXTalPSI(AutoDevice, Moveable):
    ...
    def doIsAllowed(self, target):
        if not self._attached_sxtal.use_psi:
            return (
                False,
                "Can only use PSI when use_psi is enabled on %s"
