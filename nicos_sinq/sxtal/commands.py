...
@usercommand
@helparglist(
    "reciprocal lattice coordinates, angles, " "auxiliary information, reflection list"
)
...
@parallel_safe
@helparglist(
    "Optional reflection list to list, " "if None list the default reflection list"
)
...
@usercommand
@helparglist(
    "index of reflection to modify, new reciprocal lattice "
...
@usercommand
@helparglist(
    "A tuple of (h, k, l) for the auxiliary reflection and the "
...
@usercommand
@parallel_safe
@helparglist("tuple of setting angles, when None, " "read current instrument position")
...
def ShowAng():
    ...
    for p in pos:
        posstr += "%8.3f," % p
    posstr = posstr[0:-1] + ")"
    session.log.info("%s = %s", str(rfl.column_headers[1]), posstr)


@usercommand
@helparglist("device, step width, " "counter to optimise against, maxsteps, preset")
def Max(dev, step, counter=None, maxpts=None, **preset):
    ...
    def findPeak(left, right):
        ...
        if counts[oidx + 1] == counts[oidx]:
            ...
            if (counts[oidx] ** 2 + 2.0 * s * da) < 0:
                session.log.error("Error calculating peak median")
                return False
            disc = np.sqrt(counts[oidx] ** 2 + 2.0 * s * da)
    ...
    if counts[half_point] < max_count / 2.0:
        right_idx = max_idx
        while counts[right_idx] > max_count / 2.0:
            right_idx += 1
    ...
    if idx == maxpts and count > max_count / 2.0:
        session.log.error("Peak out of range")
        return False
    # Test for peak in right half
    if counts[half_point] < max_count / 2.0 and left_idx < 0:
        left_idx = max_idx
    while counts[left_idx] > max_count / 2.0:
        left_idx -= 1
    ...
    try:
        ...
        for r in reflist.generate(0):
            ...
            if ok:
                ...
                if len(r[1]) == 4:
                    session.log.error(
                        "Failed to center reflection at " "%f %f %f %f",
                        r[1][0],
                        r[1][1],
                        r[1][2],
                        r[1][3],
                    ...
                    session.log.error(
                        "Failed to center reflection at " "%f %f %f",
                        r[1][0],
                        r[1][1],
                        r[1][2],
                    )
...
def write_rafin_header(out, sample, inst):
    ...
    out.write(
        "0 %8.4f 0 %8.4f 0 %8.4f 0 %8.4f 0 %8.4f 0 %8.4f\n"
...
def GenerateSuper(targetlist, vector, srclist=None):
    ...
    session.log.info("%d super structure reflection generated in %s", count, targetlist)
    ...
    def acquire(self, point, preset):
        if isinstance(session.instrument, EulerSXTal):
            ...
            if poslist[0][1] > inst.t2angle:
                ...
                inst.scanmode = "omega"
        try:
            _scanfuncs[self.scanmode](point.target[0], subscan=True, **self._preset)
            ...
            index = [
                i
                for (i, v) in enumerate(subscan.detvalueinfo)
            ...
            if vals:
                ...
                session.experiment.data.putResults(FINAL, {"intensity": [intensity]})
        except LimitError:
            session.log.info(
                "Skipping reflection %s after LimitError", str(point.target[0])
            )
...
def ScanOmega(hkl, subscan=False, **preset):
    ...
    cscan(instr._attached_omega, op, sw, sps // 2, instr, subscan=subscan, **preset)
...
def IndexTH(idx, reflist=None, hkllim=10):
    ...
    r = rfl.get_reflection(idx)
    if not r:
        session.log.error("Reflection %d not found in reflection list", idx)
        return
    z1 = inst._convertPos(r[1])
    _, theta = calcTheta(inst.wavelength, z1)
    rfl_tth = 2.0 * np.rad2deg(np.arcsin(theta))
    result = []
    for h in range(-hkllim, hkllim):
        for k in range(-hkllim, hkllim):
            for ql in range(-hkllim, hkllim):
                ...
                if d == 0:
                    continue
                hkl_tth = 2.0 * np.rad2deg(np.arcsin(theta))
                if rfl_tth - 0.2 < hkl_tth < rfl_tth + 0.2:
                    result.append((h, k, ql, hkl_tth))
    session.log.info(
        "Suggested indices for reflection %d, two_theta= %8.4f", idx, rfl_tth
    )
    for res in result:
        session.log.info(
            "(%d, %d, %d) tth = %8.4f versus %8.4f",
            res[0],
            res[1],
            res[2],
            res[3],
...
@usercommand
@helparglist(
    "key can be stt, chi, phi, other parameters define limits "
...
def PeakSearch(reflist=None, threshold=100, **preset):
    ...
    for stt in np.arange(sttpars[0], sttpars[1], sttpars[2]):
        for chi in np.arange(chipars[0], chipars[1], chipars[2]):
            ...
            if peaks:
                session.log.info("%d candidate peaks found in phi scan", len(peaks))
                for p in peaks:
                    dphi.maw(p)
                    ang = [m.read(0) for m in mots]
                    r = ((0, 0, 0), tuple(ang), ())
                    if not test_threshold(dphi.name, inst.center_counter, p, threshold):
                        session.log.info("Peak at %f below threshold %f", p, threshold)
                        continue
    ...
    def shortDesc(self):
        ...
        if len(self._startpositions) > 1:
            for i in range(len(self._startpositions[0][0])):
                if self._startpositions[0][0][i] != self._startpositions[1][0][i]:
                    comps.append("HKLE"[i])
        if self.dataset and self.dataset.counter > 0:
            return "Scan %s #%s" % (",".join(comps) or "Q", self.dataset.counter)
...
def qscan(Q, dQ, numpoints, *args, **kwargs):
    ...
    def hasFourVariables():
        if isinstance(session.instrument, TASSXTal):
            return True
        if isinstance(session.instrument, EulerSXTal) and session.instrument.use_psi:
            return True
        return False
    ...
    if hasFourVariables():
        ...
        values = [
            [(Q[0] + i * dQ[0], Q[1] + i * dQ[1], Q[2] + i * dQ[2])]
            for i in range(numpoints)
        ]

    scan = SinqQScan(values, move, multistep, detlist, envlist, preset, scaninfo)
    scan.run()
