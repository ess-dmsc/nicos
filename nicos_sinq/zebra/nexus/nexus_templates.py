...
class ZEBRATemplateProvider(NexusTemplateProvider):
    _zebra_default = {
        ...
        "entry1:NXentry": {
            ...
            "start_time": NXTime(),
            "zebra_mode": DeviceDataset("zebramode", dtype="string"),
            "end_time": NXTime(),
            "user:NXuser": {
            ...
            "proton_beam:NXmonitor": {
                "data": DetectorDataset(
                    "protoncurr", "int32", units=NXAttribute("counts", "string")
                )
            },
            "beam_monitor:NXmonitor": {
                "data": DetectorDataset(
                    "monitorval", "int32", units=NXAttribute("counts", "string")
                )
            },
            "area_detector2:NXdata": {
                "data": NXLink("/entry1/ZEBRA/" "area_detector2/data"),
                "None": NXScanLink(),
            },
        },
    }
    _zebra_instrument = {
        ...
        "collimator_before_mono:NXcollimator": {
            "cex1": DeviceDataset("cex1", units=NXAttribute("degree", "string")),
            "cex2": DeviceDataset("cex2", units=NXAttribute("degree", "string")),
        },
        "monochromator:NXmonochromator": {
            ...
            "two_theta": ConstDataset(
                40.2, "float", unit=NXAttribute("degree", "string")
            ),
        },
        "slits_s1:NXslit": {
        ...
        "area_detector2:NXdetector": {
            ...
            "x_pixel_size": ConstDataset(
                0.734, "float", units=NXAttribute("mm", "string")
            ),
            "y_pixel_size": ConstDataset(
                1.4809, "float", units=NXAttribute("mm", "string")
            ),
    ...
    def getTemplate(self):
        ...
        zebra_template["entry1:NXentry"]["ZEBRA:NXinstrument"] = copy.deepcopy(
            self._zebra_instrument
        )
        if isinstance(session.instrument, EulerSXTal) and session.instrument.use_psi:
            self._zebra_sample["psi"] = DeviceDataset("psi")
        zebra_template["entry1:NXentry"]["sample:NXsample"] = copy.deepcopy(
            self._zebra_sample
        )
