    ...
    * ``param_display`` (default {}) -- a dictionary containing the device name
    ...
    def _create_device_item(self, devname, add_cat=False, failure=None):
        ...
        if lowlevel_device and not self._show_lowlevel:
            return
        if (
            "nicos.core.data.sink.DataSink" in params.get("classes", [])
            and not self._show_lowlevel
        ):
            return
    ...
    def on_client_cache(self, data):
        ...
        if subkey in self.param_display.get(ldevname, ()):
            ...
            if subkey not in self._devparamitems.setdefault(ldevname, {}):
                devitem = self._devitems[ldevname]
                self._devparamitems[ldevname][subkey] = QTreeWidgetItem(
                    devitem, [subkey, value, ""], PARAM_TYPE
                )
                devitem.setExpanded(True)
            else:
    ...
    def on_tree_customContextMenuRequested(self, point):
        ...
        if item.type() == DEVICE_TYPE:
            ...
            if devinfo.failure:
                ...
                "nicos.core.device.Moveable" in devinfo.classes
                and not self.client.viewonly
            ):
...
class ControlDialog(QDialog):
    ...
    def _reinit(self):
        ...
        if (
            "nicos.core.device.Moveable" in classes
            or "nicos.core.device.Measurable" in classes
        ):
            self.moveBtns.addButton(
                "Stop", QDialogButtonBox.ButtonRole.ResetRole
