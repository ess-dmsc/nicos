...
class SamplePanel(PanelBase):
    ...
    def construct_add_ctrl_buttons(self):
        ...
        # add_ctrl_buttons.btn_add.clicked.connect(self._add_sample_minimal)
    ...
    def on_keyChange(self, key, value, time, expired):
        ...
        if key in self.to_monitor:
            ...
            print("from key change", self._get_samples())

    ###########################################################

    def confirm_add_clicked(self):
        current_samples = self.client.eval("session.experiment.get_samples()", {})
        new_sample = {"name": self.sample_annotations.id_row.edit_value_widget.text()}
        samples = {}
    ...
    def update_sample_selector_items(self):
        ...
        # print("from update", self._get_samples())
        sample_identifiers = self._get_sample_identifiers()
        # print("identifiers", sample_identifiers)
    ...
    def sample_selection_updated(self, item):
        ...
        self.display_sample(samplename)
    ...
    def show_remove_sample_dialog(self):
        selected_sample = self.sample_selector.currentItem().text()
        self.remove_sample_dialog.message.setText(f"Remove sample: '{selected_sample}'")
    ...
    def display_duplicate_id_error(self):
        self.sample_annotations.id_row.info_message.setText("Sample ID already exist")

    def display_missing_id_error(self):
        self.sample_annotations.id_row.info_message.setText("Please enter a sample ID")
    ...
    def _add_sample_minimal(self):
        samples = self._get_samples()
        new_index = len(samples) + 1
        new_sample = self.sample_annotations.id_row.edit_value_widget.text()
        samples[new_index] = new_sample
        self.client.run("Exp.sample.set_samples(samples)")
