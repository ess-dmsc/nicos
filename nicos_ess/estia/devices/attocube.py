...
from nicos.devices.epics.pva import EpicsAnalogMoveable, EpicsReadable
...
class IDS3010Axis(EpicsReadable):
    ...
    parameters = {
        "axis": Param(
            "Index of the axis to be read", default=1, type=int, userparam=False
        ),
        ...
        "mode": Param("Current mode", type=str, category="general", settable=False),
    }

    parameter_overrides = {
        "unit": Override(
            default="um", mandatory=False, settable=False, userparam=False
        ),
    }
...
class IDS3010Control(EpicsAnalogMoveable):
    ...
    def _get_pv_parameters(self):
        return EpicsAnalogMoveable._get_pv_parameters(self) | set(
...
class MirrorDistance(Measurable):
    ...
    parameters = {
        "angle": Param(
            "Index of the axis to be read", default=5.0, type=float, settable=True
        ),
        "offset": Param(
    ...
    attached_devices = {"axis": Attach("IDS Axis for the measurement", Readable)}
    ...
    def doRead(self, maxage=0):
        ...
        return value * 0.5 * cos(self.angle / 360.0 * pi) + self.offset
    ...
    def doWriteOffset(self, value):
        ...
        if self._cache:
            self._cache.put(
                self, "value", self.doRead(0) - diff, currenttime(), self.maxage
            )
