...
class PilotLaser(HasDisablePv, EpicsReadable):
    parameters = {
        "uncertainty_fix": Param(
            "Fixed contribution to uncertainty",
            type=float,
            settable=False,
            volatile=True,
        ),
        "uncertainty_variable": Param(
            "Uncertainty that depends on L", type=float, settable=False
        ),
        "pvprefix": Param(
            "Name of the record PV.",
            type=pvname,
            mandatory=True,
            settable=False,
            userparam=False,
...
class MultilineChannel(EpicsReadable):
    parameters = {
        "channel": Param(
            "Channel number.", type=int, settable=False, userparam=False, internal=True
        ),
        "i_limits": Param(
            "Minimum intensity as raw value.",
            type=limits,
            settable=False,
            internal=True,
        ),
        "gain": Param(
            "Gain for the channel.", type=float, settable=False, internal=True
        ),
        "gain_pv": Param(
            "PV for the gain.",
            type=pvname,
        ...
        "latest_valid": Param(
            "Latest data of a valid measurement.",
            type=float,
            settable=False,
            internal=True,
        ),
        "latest_valid_pv": Param(
            "PV of latest data of a valid measurement.",
            type=pvname,
            settable=False,
            mandatory=True,
            userparam=False,
    ...
    def doPoll(self, n, maxage=0):
        self.pollParams(
            volatile_only=False, param_list=["i_limits", "gain", "latest_valid"]
        )


EnvironmentalParameters = namedtuple(
    "EnvironmentalParameters", ["temperature", "pressure", "humidity"]
)
...
class MultilineController(EpicsReadable):
    parameters = {
        ...
        "front_end_splitter": Param(
            "Turn front end splitter on/off.", type=str, settable=True, internal=True
        ),
        "fes_option": Param(
            "Turn the shutter on or off when not measuring.",
            type=str,
            settable=True,
            internal=True,
        ),
        "single_measurement": Param(
            "Start of a single measurement.", type=str, settable=True, internal=True
        ),
        "continuous_measurement": Param(
