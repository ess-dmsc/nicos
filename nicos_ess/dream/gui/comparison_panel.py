...
class ComparisonPanel(LiveDataPanel):
    ...
    def __init__(self, parent, client, options):
        ...
        self._normaliser = NormaliserFactory.create(NormaliserType.NONORMALISER)
    ...
    def _on_norm_change(self, value):
        self._normaliser = NormaliserFactory.create(self._available_normalisers[value])
        self.on_live_data_update()
    ...
    def _update_2d_plot(self, data_blob=None):
        ...
        if data_blob:
            ...
            if self.reference_data_2d and data.shape == self.reference_data_2d[1].shape:
                data = (
    ...
    def _set_tab_visibility(self, dim):
        visibile_tab_index = (
            self.tabWidget.indexOf(self.tab_1d)
            if dim == 1
            else self.tabWidget.indexOf(self.tab_2d)
        )
