...
class EpicsShutter(EpicsMappedMoveable):
    ...
    def doReset(self):
        """Reset (acknowledge) shutter state by writting on the configured 'resetpv' parameter"""

        if self.resetpv:
            # NOTE: writting True resets regardless previous state
            self._put_pv("resetpv", True)
        else:
            raise NotImplementedError("Reset isn't available on device or missing resetpv param") 
