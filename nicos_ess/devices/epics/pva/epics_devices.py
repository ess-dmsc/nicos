...
class EpicsReadable(EpicsDeviceBase, Readable):
    ...
    _epics_channels = make_read_channels()
...
class EpicsNumericReadable(EpicsReadable):
    ...
    def _build_epics_channels(self):
        ...
        if self.use_prec:
            channels["prec"] = readback_channel(
                ".PREC",
                cache_key="fmtstr",
                pv_prefix_attr="readpv",
...
class EpicsStringReadable(EpicsReadable):
    ...
    _epics_channels = make_read_channels(as_string=True)
...
class EpicsAnalogMoveable(EpicsReadWriteBase, HasPrecision, HasLimits, Moveable):
    ...
    # A lost connection reports status.UNKNOWN. Treat it as a failed move so
    # maw() raises instead of reporting the move done while disconnected.
    ...
    def _compute_status(self, maxage=0):
        ...
        try:
            ...
            # A fresh target/readback read raises on disconnect; report the
            # same UNKNOWN as the cached path instead of letting the raw
            # wrapper error surface as ERROR.
