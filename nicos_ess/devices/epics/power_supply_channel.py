...
class PowerSupplyChannel(EpicsDeviceBase, CanDisable, MappedReadable):
    ...
    _default_pv_root_attr = "ps_pv"
    _primary_channel = "power_rb"
    _epics_channels = {
        "voltage_monitor": EpicsChannelInfo(
            "", "-VMon", EpicsChannelRole.VALUE_AND_STATUS
        ),
        "current_monitor": EpicsChannelInfo(
            "", "-IMon", EpicsChannelRole.VALUE_AND_STATUS
        ),
        "power_rb": EpicsChannelInfo("", "-Pw-RB", EpicsChannelRole.VALUE_AND_STATUS),
        "power": EpicsChannelInfo("", "-Pw", EpicsChannelRole.VALUE, subscribe=False),
        "status_on": EpicsChannelInfo(
            "", "-Status-ON", EpicsChannelRole.VALUE_AND_STATUS
        ),
    }

    def _pvs_to_connect(self):
        # Checking one PV is enough to see that the IOC is there.
        return [self._epics.pv_name_for("voltage_monitor")]
