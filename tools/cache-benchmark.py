...
class Benchmarker:
    ...
    def multi_writer(self):
        ...
        for i, main in enumerate(mains):
            threading.Thread(
                target=lambda i=i, m=main: m.sendall(
                    b"".join(self.all_msg[i * perclient : (i + 1) * perclient])
                )
            ).start()
        for s in subs:
            msg_set = self.recvall(s)
    ...
    def main(self):
        ...
        parser.add_argument(
            "-c", action="store", default="localhost", metavar="HOST", help="cache host"
        )
        parser.add_argument(
            "-n",
            action="store",
        ...
        parser.add_argument("benchmark", action="store", type=str, choices=self.benches)
        ...
        t2 = time.time()
        print(
            f"{opts.benchmark}: {self.nkeys} keys, "
