...
class ExpPanel(Panel):
    ...
    * ``finish_exp_panel`` -- class name of the panel which should be opened
    ...
    def _update_proposal_info(self):
        ...
        self.localContacts.setText(combineUsers(propinfo.get("localcontacts", [])))
    ...
    def _getProposalInput(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self,
                "Error",
                "Invalid users list, please specify as\n\n"
        ...
        try:
            ...
            QMessageBox.warning(
                self,
                "Error",
                "Invalid local contacts, please specify as\n\n"
                "First Name <first@email.com>; Second Name <second@email.com>",
            )
            raise ConfigurationError from None
    ...
    def on_finishButton_clicked(self):
        if self._finish_exp_panel:
            dlg = PanelDialog(
                self, self.client, self._finish_exp_panel, "Finish experiment"
            )
            dlg.exec()
    ...
    def on_queryDBButton_clicked(self):
        try:
            prop, title, _, _, sample, notifEmails, dataEmails, _ = (
        ...
        try:
            ...
            if result:
                ...
                self.sampleName.setText(result.get("default_sample", sample))
                self.notifEmails.setPlainText(
                ...
                self.showError(
                    "Querying proposal management system failed: "
            ...
            self.showError(
                "Querying proposal management system failed for an "
    ...
    def applyChanges(self):
        ...
        if prop and prop != self._orig_propinfo.get("proposal"):
            ...
            if self._new_exp_panel:
                dlg = PanelDialog(
                    self, self.client, self._new_exp_panel, "New experiment"
                )
                dlg.exec()
        else:
        ...
        if notifEmails != self._orig_propinfo.get("notif_emails"):
            self.client.run("SetMailReceivers(%s)" % ", ".join(map(repr, notifEmails)))
            done.append("New mail receivers set.")
        if dataEmails != self._orig_propinfo.get("data_emails"):
            self.client.run("SetDataReceivers(%s)" % ", ".join(map(repr, dataEmails)))
...
class DetEnvPanel(Panel):
    ...
    def on_client_connected(self):
        ...
        for detname in detectors:
            ...
            item.setCheckState(
                Qt.CheckState.Checked
                if detname in self._orig_detlist
                else Qt.CheckState.Unchecked
            )

        # fill environment
        envdevs = self.client.getDeviceList(
            "nicos.core.device.Readable", exclude_class="nicos.core.device.Measurable"
        )
        ...
        for devname in envdevs:
            ...
            item.setCheckState(
                Qt.CheckState.Checked
                if devname in self._orig_envlist
                else Qt.CheckState.Unchecked
            )
        if self.client.viewonly:
            self.buttonBox.setStandardButtons(QDialogButtonBox.StandardButton.Close)
...
class SXTalSamplePanel(GenericSamplePanel):
    ...
    def __init__(self, parent, client, options):
        ...
        params = client.eval("session.experiment.sample.cell.cellparams()", None)
        if params:
            lattice = params[:3]
            angles = params[3:6]
        else:
