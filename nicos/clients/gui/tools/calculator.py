...
PI = math.pi
MEV = constants.electron_volt / 1000.0
...
class CalculatorTool(QDialog):
    ...
    def __init__(self, parent, client, **settings):
        ...
        self.braggfmlLabel.setPixmap(
            QPixmap(
                path.join(path.dirname(__file__), "calculator_images", "braggfml.png")
            )
        )
        ...
        if not self._miezesettings:
            ...
            self.mzformulaLabel.setPixmap(
                QPixmap(
                    path.join(
                        path.dirname(__file__), "calculator_images", "miezefml.png"
                    )
                )
            )
        ...
        self.presets = DlgPresets(
            ...
            [
                (self.tabWidget, 0),
                (self.mzwavelengthInput, "10"),
                (self.mzdistanceInput, "100"),
                (self.inputLambda, "4.5"),
                (self.input2Theta, "0"),
                (self.inputD, "10"),
                (self.inputN, "1"),
                (self.inputQ, "0"),
    ...
    def braggcalc(self, *ignored):
        ...
        try:
            ...
            if "2Theta" in given and "Lambda" in given:
                ...
                if "N" in given:
                    given["D"] = (
                        given["N"] * given["Lambda"] / 2 / math.sin(given["2Theta"] / 2)
                    )
                else:
                    given["N"] = (
                        2 * given["D"] * math.sin(given["2Theta"] / 2) / given["Lambda"]
                    )
                ...
                if "Lambda" in given:
                    ...
                    given["Lambda"] = (
                        2 * given["D"] * math.sin(given["2Theta"] / 2) / given["N"]
                    )
