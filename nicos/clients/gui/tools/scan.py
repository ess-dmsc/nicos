...
from nicos.guisupport.qt import QButtonGroup, QDialog, QIntValidator, pyqtSignal
...
class ScanTool(QDialog):
    ...
    def calc_qscan(self):
        ...
        if (
            self.qscanCentered.isChecked()
            or self.qscanLong.isChecked()
            or self.qscanTrans.isChecked()
        ):
            numstep = 2 * numstep + 1
