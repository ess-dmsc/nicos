...
class CacheDatabase(Device):
    ...
    def ask_hist(self, key, fromtime, totime, interval):
        ...
        if interval in [None, "", "None", "0", "0.0"]:
            ...
            try:
                ...
                interval = None
        for entry in self.queryHistory((category, subkey), fromtime, totime, interval):
    ...
    def tell(self, key, value, time, ttl, from_client):
        ...
        real_update = self.updateEntries(
            newcats, subkey, no_store, CacheEntry(time, ttl, value)
        )
        ...
        if real_update or no_store:
            for client in self._server._connected.values():
                if client is not from_client:
                    for cat in newcats:
                        client.update(
                            f"{cat}/{subkey}", OP_TELL, value or "", time, ttl
                        )
    ...
    def lock(self, key, value, time, ttl):
        ...
        with self._lock_lock:
            ...
            if req == OP_LOCK_LOCK:
                if (
                    entry
                    and entry.value != client_id
                    and (not entry.ttl or entry.time + entry.ttl >= currenttime())
                ):
                    ...
                    self.log.debug(
                        "lock request %s=%s ttl %s, accepted", key, client_id, ttl
                    )
            ...
            # want to unlock?
                ...
                if entry and entry.value != client_id:
                    ...
                    self.log.debug(
                        "unlock request %s=%s, but locked by %s",
                        key,
                        client_id,
                        entry.value,
                    )
                    ...
                    self.log.debug("unlock request %s=%s, accepted", key, client_id)
