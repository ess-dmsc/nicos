...
POLL_MIN_VALID_TIME = (
    0.15  # latest time slot to poll before value times out due to maxage
)
...
class Poller(Device):
    parameters = {
        "autosetup": Param(
            "True if all master setups should always be polled", type=bool, default=True
        ),
        "poll": Param(
            "Setups that should be polled if in master setup "
    ...
    def _worker_thread(self, devname, work_queue):
        ...
        def poll_loop(dev):
            ...
            maxage = interval - POLL_MIN_VALID_TIME if interval else (dev.maxage or 0)
            ...
            while not self._stoprequest:
                ...
                timesout = lastpoll + (
                    dev.maxage - POLL_MIN_VALID_TIME
                    if dev.maxage
                    else POLL_MIN_VALID_TIME
                )
                maxwait = min(nextpoll, timesout) - now
                self.log.debug(
                    "%-10s: maxwait is %g (nextpoll=%g, timesout=%g)",
                    dev,
                    maxwait,
                    nextpoll,
                    timesout,
                ...
                if maxwait > 0:
                    ...
                    try:
                        ...
                        if (
                            event == "adev_busy"
                        ):  # one of our attached_devices went busy
                            interval = POLL_BUSY_INTERVAL
                            maxage = interval / 2.0
                            # also poll
                        elif (
                            event == "adev_normal"
                        ):  # one of our attached_devices is no more busy
                            pass  # just poll
                        elif (
                            event == "adev_target"
                        ):  # one of our attached_devices got new target
                            interval = POLL_BUSY_INTERVAL
                            maxage = interval / 2.0
                            continue
                        elif (
                            event == "adev_value"
                        ):  # one of our attached_devices changed value
                            ...
                            interval = dev.pollinterval
                            maxage = (
                                interval - POLL_MIN_VALID_TIME
                                if interval
                                else (dev.maxage or 0)
                            )
                            continue
                            ...
                            try:
                                ...
                                dev.log.warning(
                                    "error polling parameter %s", event[10:], exc=True
                                )
                ...
                if dev.pollinterval is not None:
                    ...
                    self.log.debug(
                        "%-10s: status = %-25s, value = %s", dev, stval, rdval
                    )
        ...
        while not self._stoprequest:
            try:
                ...
                if not registered:
                    ...
                    session.cache.addCallback(
                        dev, "status", reconfigure_dev_status
                    )  # may spam events
                    ...
                    for adev in dev._adevs.values():
                        ...
                        session.cache.addCallback(
                            adev, "target", reconfigure_adev_target
                        )
                        session.cache.addCallback(
                            adev, "status", reconfigure_adev_status
                        )
                registered = True
        ...
        if self._setup is not None:
            ...
            for tid, frame in list(sys._current_frames().items()):
                if tid in active:
                    ...
                    name = str(tid)
                self.log.info("%s: %s", name, "".join(traceback.format_stack(frame)))
    ...
    def _start_child(self, setup):
        ...
        process = createSubprocess(
            [sys.executable, poller_script, setup] + additional_args
        )
        ...
        while True:
            try:
                ...
                if setup in self._setups and not self._stoprequest:
                    ...
                    session.log.info(
                        "%s poller terminated with %s", setup, whyExited(ret)
                    )
            ...
            for setup, ch in list(self._children.items()):
                ...
                if ret is not None:
                    ...
                    if setup in self._setups and not self._stoprequest:
                        ...
                        session.log.info("%s poller terminated with %s", setup, ret)
        session.log.info("all pollers terminated")
