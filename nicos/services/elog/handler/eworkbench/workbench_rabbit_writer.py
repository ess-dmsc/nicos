...
class Handler(BaseHandler):
    ...
    parameters = {
        "rabbit_url": Param(
            "RabbitMQ host set in pika connection parameters",
            type=str,
            ext_desc=ext_desc_conn,
        ),
        "rabbit_port": Param(
            "RabbitMQ port set in pika connection parameters",
            type=int,
            default=5672,
            ext_desc=ext_desc_conn,
        ),
        ...
        "rabbit_username": Param(
            "RabbitMQ username from pika credentials", type=str, ext_desc=ext_desc_cred
        ),
        "rabbit_password": Param(
            "RabbitMQ password from pika credentials", type=str, ext_desc=ext_desc_cred
        ),
    ...
    def handle_directory(self, time, data):
        ...
        wb_text = (
            wb_format(f"Opened new output files in:   {self._out.logdir}")
            + wb_format(f"Instrument:   {self._out.instr}")
            + wb_format(f"Proposal:   {self._out.proposal}")
        )
    ...
    def handle_newexperiment(self, time, data):
        ...
        self._out.rabbit_producer.produce(
            headers=headers, message=wb_format(f"New Experiment is: {title}")
        )
    ...
    def handle_entry(self, time, data):
        ...
        self._out.rabbit_producer.produce(
            headers=headers, message=wb_format(f"{escape(data)}")
        )
    ...
    def handle_remark(self, time, remark):
        ...
        self._out.rabbit_producer.produce(
            headers=headers, message=wb_format(f"{remark}")
        )
    ...
    def handle_scriptend(self, time, script):
        ...
        self._out.rabbit_producer.produce(
            headers=headers, message=wb_format(f"{escape(script)}")
        )
    ...
    def handle_sample(self, time, sample):
        ...
        self._out.rabbit_producer.produce(
            headers=headers, message=wb_format(f"Sample:   {escape(sample)}")
        )
    ...
    def handle_offset(self, time, data):
        ...
        dev, old, new = data
        offset_info = escape("Offset of %s changed from %s to %s" % (dev, old, new))
        ...
        self._out.rabbit_producer.produce(
            headers=headers, message=wb_format(f"{offset_info}")
        )
    ...
    def handle_message(self, time, message):
        ...
        if message[2] == ERROR:
            ...
            self._out.rabbit_producer.produce(
                headers=headers, message=wb_format(f"{formatted}")
            )
    ...
    def handle_scanend(self, time, dataset):
        ...
        if dataset.xresults:
            for i in range(len(dataset.xnames)):
                ...
                dataset_names.append(dataset.xnames[i] + "(" + dataset.xunits[i] + ")")
                if first == last:
        ...
        self._out.rabbit_producer.produce(headers=headers, message=scan_end_results)
