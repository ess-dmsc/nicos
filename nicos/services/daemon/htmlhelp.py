...
class HelpGenerator:
    ...
    def gen_helpindex(self):
        ...
        for name, obj in session.getExportedObjects():
            ...
            signature = (
                '<tt><a href="cmd:%s">%s</a></tt><small>' % ((real_func.__name__,) * 2)
                + html.escape(argspec)
                + "</small>"
            )
            docstring = html.escape(real_func.__doc__ or " ").splitlines()[0]
            cmds.append("<tr><td>%s</td><td>%s</td></tr>" % (signature, docstring))
        cmds.sort()
        ...
        ret.append(
            '<table width="100%"><tr><th>Name</th><th>Type</th>'
        ...
        for devname in sorted(session.explicit_devices, key=lower):
            ...
            ret.append(
                ...
                % (
                    ...
                    devsetups.get(devname, ""),
                    html.escape(dev.description),
                )
            )
        ret.append("</table>")
        ret.append(self.gen_heading("Setups", "setups"))
        ret.append(
            "<p>These are the available setups.  Use "
            ...
            for setupname, info in sorted(session.getSetupInfo().items()):
                ...
                setups.append(
                    ...
                    % (
                        ...
                        ", ".join(map(devlink, sorted(info["devices"], key=lower))),
                    )
                )
            ret.append(
                '<table width="100%"><tr><th>Name</th><th>Loaded</th>'
    ...
    def gen_devhelp(self, dev):
        ...
        ret.append(
            '<p class="devcls">%s is a device of class %s.</p>'
        ...
        if isinstance(dev, DeviceAlias):
            ...
            if points:
                ret.append(
                    '<p class="devalias">%s is a device alias, it '
                    'points to <a href="dev:%s">%s</a>.' % (dev, points, points)
                )
            else:
                ret.append(
                    '<p class="devalias">%s is a device alias, it '
        ...
        if dev.description:
            ret.append(
                '<p class="devdesc">Device description: '
                + html.escape(dev.description)
                + "</p>"
            )
        if dev.__class__.__doc__:
            ...
            ret.append(
                '<p class="clsdesc">Device class description:</p>'
                + "<blockquote>"
                + self.gen_markup(clsdoc)
                + "</blockquote>"
            )
        ret.append("<h4>Device parameters</h4>")
        ret.append(
            '<table width="100%"><tr><th>Name</th><th>Current value</th>'
        ...
        for name, info in sorted(dev.parameters.items()):
            ...
            ret.append(
                "<tr><td><tt>%s</tt></td><td>%s</td><td>%s</td>"
        ...
        ret.append(
            '<table width="100%"><tr><th>Method</th><th>From class</th>'
            "<th>Description</th></tr>"
        )
        listed = set()
        ...
        def _list(cls):
            if cls in listed:
                return
            listed.add(cls)
            for name, (args, doc, fromtype, is_usermethod) in sorted(
                cls.methods.items()
            ):
    ...
    def gen_genericdoc(self, obj):
        ...
        return (
            self.gen_heading("Python help on %r" % obj)
            + "<pre>"
            + html.escape(ret)
            + "</pre>"
        )
    ...
    def generate(self, obj):
        ...
        if isinstance(obj, str) and obj not in self._specialtopics:
            ...
            return "dev:%s" % obj, self.header + self.gen_devhelp(obj) + self.footer
        elif inspect.isfunction(obj):
            return "cmd:%s" % obj.__name__, self.header + self.gen_funchelp(
                obj
            ) + self.footer
        else:
            return "", self.header + self.gen_genericdoc(obj) + self.footer
