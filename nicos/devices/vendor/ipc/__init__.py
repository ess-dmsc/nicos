...
class Coder(NicosCoder):
    ...
    parameters = {
        "addr": Param(
            "Bus address of the coder", type=intrange(32, 255), mandatory=True
        ),
        "confbyte": Param(
            "Configuration byte of the coder",
            settable=True,
            type=intrange(0, 255),
            prefercache=False,
        ),
        "zerosteps": Param(
            "Coder steps for physical zero", type=float, unit="steps", settable=True
        ),
        "slope": Param(
            "Coder slope", type=float, default=1.0, unit="steps/main", settable=True
        ),
    ...
    def _getcodertype(self, byte):
        ...
        if byte < 4:
            return (
                "incremental encoder, 24bit, "
                + ["no reset", "reset once", "reset always", "reset once to halfrange"][
                    byte
                ]
            )
    ...
    def _endatclearalarm(self):
        ...
        try:
            ...
            raise CommunicationError(self, "cannot clear alarm for encoder") from err
    ...
    def _endatreadalarm(self):
        ...
        try:
            ...
            raise CommunicationError(self, "cannot read alarm for encoder") from err
...
class Motor(HasTimeout, NicosMotor):
    ...
    parameters = {
        ...
        "zerosteps": Param(
            "Motor steps for physical zero", type=float, unit="steps", settable=True
        ),
        "slope": Param(
            "Motor slope", type=float, settable=True, unit="steps/main", default=1.0
        ),
        # those parameters come from the card
        "firmware": Param("Firmware version", type=int),
        "steps": Param(
            "Last position in steps",
            settable=True,
            type=intrange(0, 999999),
            prefercache=False,
        ),
        "speed": Param("Motor speed (0..255)", settable=True, type=intrange(0, 255)),
        "accel": Param(
            "Motor acceleration (0..255)", settable=True, type=intrange(0, 255)
        ),
        "confbyte": Param("Configuration byte", type=intrange(-1, 255), settable=True),
        ...
        "min": Param(
            "Lower motorlimit", settable=True, type=intrange(0, 999999), unit="steps"
        ),
        "max": Param(
            "Upper motorlimit", settable=True, type=intrange(0, 999999), unit="steps"
        ),
        "startdelay": Param(
            "Start delay",
        ...
        "stopdelay": Param(
            "Stop delay", type=floatrange(0, 25), unit="s", settable=True, volatile=True
        ),
        "divider": Param("Speed divider", settable=True, type=intrange(-1, 7)),
    ...
    def doWriteMicrostep(self, value):
        if self._hwtype == "single":
            if value == 1:
                ...
                raise InvalidValueError(self, "microsteps > 2 not supported by card")
        else:
    ...
    def doWritePrecision(self, precision):
        minprec = abs(2.0 / self.slope)
        if precision < minprec:
            self.log.warning("Precision needs to be at least %.3f, adjusting.", minprec)
            return minprec
...
class Input(Readable):
    ...
    parameters = {
        "addr": Param(
            "Bus address of the card", type=intrange(32, 255), mandatory=True
        ),
        "first": Param("First bit to read", type=intrange(0, 15), mandatory=True),
        "last": Param("Last bit to read", type=intrange(0, 15), mandatory=True),
    }
...
class IPCSwitches(Input):
    ...
    def doRead(self, maxage=0):
        ...
        try:
            ...
            raise NicosError(self, "cannot evaluate status byte of stepper") from err
...
class SlitMotor(HasTimeout, NicosMotor):
    ...
    parameters = {
        "addr": Param(
            "Bus address of the slit", type=intrange(32, 255), mandatory=True
        ),
        "side": Param("Side of axis", type=int, mandatory=True),
        "unit": Param("Axis unit", type=str, default="mm"),
        "zerosteps": Param(
            "Motor steps for physical zero", type=int, unit="steps", settable=True
        ),
        "slope": Param(
            "Motor slope", type=float, settable=True, unit="steps/mm", default=1.0
        ),
        "resetpos": Param(
            "Value to move to for reset", type=float, unit="main", mandatory=True
        ),
    }
    ...
    def doReset(self):
        if (
            self._attached_bus.get(self.addr, self.side + 166) != 9999
            and self.doStatus()[0] != status.ERROR
        ):
            return
        self.log.info(
