...
class IPCModBusRS232(HasCommunication, IPCModBus):
    ...
    def send(self, addr, cmd, param=0, length=0):
        try:
            ...
            raise ProgrammingError(self, "Command %s not supported" % cmd) from err
        if limits:
            if length != limits[0] or not limits[1] <= param <= limits[2]:
                raise ProgrammingError(
                    self,
                    "Parameter %s outside allowed values %s for cmd "
    ...
    def _transmit(self, request, retlen, last_try=False):
        ...
        try:
            ...
            for i in range(self.comtries):
                self.log.debug("waiting for response, try %d/%d", i, self.comtries)
...
class IPCModBusSerial(IPCModBusRS232):
    ...
    def doReset(self):
        ...
        self._connection = serial.Serial(
            self.port, baudrate=19200, timeout=self.bustimeout
        )
    ...
    def _transmit(self, request, retlen, last_try=False):
        ...
        try:
            ...
            if last_try:
                raise CommunicationError(self, "serial line failed: %s" % err) from err
            # try reopening connection
