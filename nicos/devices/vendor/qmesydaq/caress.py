...
from nicos.devices.vendor.caress.core import (
    ACTIVE,
    ACTIVE1,
    CARESS,
    CORBA,
...
class QMesydaqCaressDevice(CARESSDevice):
    def _init(self, cid):
        try:
            ...
            if res not in [(0, ON_LINE), (CARESS.OK, ON_LINE)]:
                ...
                if res not in [(0, ON_LINE), (CARESS.OK, ON_LINE)]:
                    self.log.error(
                        "Init module (Re-Init): %r (%d, %s)", res, cid, _config
                    )
    ...
    def doInit(self, mode):
        ...
        if hasattr(self._caressObject, "is_counting_module"):
            is_counting = self._caress_guard(
                self._caressObject.is_counting_module, self.cid
            )
...
class Channel(QMesydaqCaressDevice, ActiveChannel):
    ...
    def _start(self, kind):
        if hasattr(self._caressObject, "start_module"):
            result = self._caress_guard(
                self._caressObject.start_module, kind, self.cid, self.runnumber, 0
            )
        else:
            result = self._caress_guard(
                self._caressObject.start_acquisition_orb, kind, self.runnumber, 0
            )
    ...
    def _stop(self, kind=0):
        if hasattr(self._caressObject, "stop_module"):
            result = self._caress_guard(self._caressObject.stop_module, 11, self._cid)
        else:
            result = self._caress_guard(
                self._caressObject.stop_module_orb, 11, self._cid
            )
        if result in [
            (CARESS.OK, ACTIVE),
            (CARESS.OK, ACTIVE1),
    ...
    def _break(self, kind=0):
        if hasattr(self._caressObject, "stop_module"):
            result = self._caress_guard(self._caressObject.stop_module, kind, self.cid)
        elif self.iscontroller:
            result = self._caress_guard(self._caressObject.stop_acquisition_orb, kind)
        if result[0] not in [0, CARESS.OK]:
    ...
    def _load_preset(self, kind, preset=0):
        if hasattr(self._caressObject, "load_module"):
            result, loaded = self._caress_guard(
                self._caressObject.load_module, kind, self.cid, CARESS.Value(l=preset)
            )
...
class Image(QMesydaqCaressDevice, QMesyDAQImage):
    ...
    def _readblock(self):
        if hasattr(self._caressObject, "readblock_module") and hasattr(
            self._caressObject, "readblock_params"
        ):
            _type = CARESS.TypeLong
            _start = 0
            _end = 0
            try:
                result, _start, _end, _type = self._caress_guard(
                    self._caressObject.readblock_params,
                    READBLOCK_NORMAL,
                    self.cid,
                    _start,
                ...
                result, _status, data = self._caress_guard(
                    self._caressObject.readblock_module,
                    READBLOCK_NORMAL,
                    self.cid,
                    _start,
                    _end,
                ...
                raise CommunicationError(
                    self, "Could not read the CARESS module : %s" % ex
                ) from ex
        else:
            _ = ()
            result = self._caress_guard(
                self._caressObject.read_module_orb, 0, self.cid, _
            )
            ...
            if result[1][0].value() != self.cid:
                raise NicosError(
                    self, "Answer from wrong module!: %d %r" % (self.cid, result[1][0])
                )
    ...
    def doReadArray(self, quality):
        ...
        if res[2] in [0, 1]:  # 2D array
            ...
            self.readresult = [data.sum()]
            return data.reshape(
                (res[0], res[1]), order="C" if not self.transpose else "F"
            )
        # 3D array
    ...
    def _set_option(self, text):
        ...
        if hasattr(self._caressObject, "loadblock_module"):
            self._caressObject.loadblock_module(
                LOAD_NORMAL, self.cid, 1, len(text), CARESS.Value(ab=text)
            )
