...
class THM1176(Measurable):
    ...
    def _query(self, q, binary=True, t=5):
        try:
            ...
            if t == 0:
                raise CommunicationError(self, "error querying: %s" % err) from err
            session.delay(0.5)
            self.doReset()
    ...
    def _execute(self, q, t=5, wait=0):
        try:
            ...
            if t == 0:
                raise CommunicationError(self, "error executing: %s" % err) from err
            session.delay(0.5)
            self.doReset()
            self._execute(q, t - 1)
        session.delay(wait)
        ...
        try:
            ...
            if t == 0:
                raise CommunicationError(
                    self, "error getting status: %s" % err
                ) from err
            session.delay(0.5)
            self.doReset()
