...
class Driveable(HasLimits, HasPrecision, CARESSDevice, Moveable):
    ...
    parameters = {
        "_started": Param(
            "Indicator to signal device is started",
            type=bool,
            default=False,
            settable=False,
        ),
    ...
    def doInit(self, mode):
        ...
        if hasattr(self._caressObject, "is_drivable_module"):
            ...
            (ret, _, _, chtype) = self._caress_guard(
                self._caressObject.readblock_params_orb, -0x80000000, self.cid, 1, 0, 0
            )
            if ret != READBLOCK_NORMAL or chtype != CHTYPE_INT32:
                ...
                (ret, _, dataseq) = self._caress_guard(
                    self._caressObject.int_readblock_module_orb,
                    -0x80000000,
                    self.cid,
                    1,
                    1,
            ...
            is_drivable = self._device_kind() in [
                3,
                7,
                13,
                14,
    ...
    def doStart(self, target):
        ...
        if hasattr(self._caressObject, "drive_module"):
            ...
            result = self._caress_guard(
                self._caressObject.drive_module, 0, self.cid, val, timeout
            )
            ...
            result = self._caress_guard(
                self._caressObject.drive_module_orb, 0, params, 0, timeout
            )
            if result[0] != 0:
                raise NicosError(self, "Could not start the device")
    ...
    def _stop(self, stopmode=KEEP_ACTION):
        ...
        if hasattr(self._caressObject, "stop_module"):
            result = self._caress_guard(
                self._caressObject.stop_module, stopmode, self.cid
            )
            if result in [(CARESS.OK, ACTIVE), (CARESS.OK, ACTIVE1)]:
                raise NicosError(self, "Could not stop the module")
        else:
            result = self._caress_guard(
                self._caressObject.stop_module_orb, stopmode, self.cid
            )
