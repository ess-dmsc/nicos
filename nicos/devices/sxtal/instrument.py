...
class SXTalBase(HasAutoDevices, Instrument, Moveable):
    ...
    parameters = {
        "wavelength": Param("Wavelength", type=float, volatile=True, settable=False),
        "scanmode": Param(
            "Scanmode",
...
class EulerSXTal(SXTalBase):
    ...
    def _readPos(self, maxage=0):
        return PositionFactory(
            "e",
            theta=self._attached_ttheta.read(maxage) / 2.0,
    ...
    def _createPos(self, ttheta, omega, chi, phi):
        return PositionFactory("e", theta=ttheta / 2.0, omega=omega, chi=chi, phi=phi)
