...
class SXTalSample(Sample):
    parameters = {
        ...
        "alpha": Param("alpha", type=floatrange(1.0, 179.0), category="sample"),
        "beta": Param("beta", type=floatrange(1.0, 179.0), category="sample"),
        "gamma": Param("gamma", type=floatrange(1.0, 179.0), category="sample"),
        "rmat": Param(
            "rmat", type=listof(listof(float)), default=None, userparam=False
        ),
        "ubmatrix": Param(
            "UB matrix (rmat^T)", type=listof(listof(float)), category="sample"
        ),
        "bravais": Param(
            "Bravais lattice",
        ...
        "peaklists": Param(
            "Lists of peaks for scanning",
            internal=True,
            type=dictof(str, list),
            settable=True,
        ),
        "poslists": Param(
            "Lists of positions for indexing",
            internal=True,
            type=dictof(str, list),
            settable=True,
        ),
    ...
    def doWriteCell(self, cell):
        ...
        self.log.info("a = %8.3f  b = %8.3f  c = %8.3f", params.a, params.b, params.c)
        self.log.info(
        ...
        self.log.info("Bravais: %s  Laue: %s", cell.bravais.bravais, cell.laue.laue)
