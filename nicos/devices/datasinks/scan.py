...
class ConsoleScanSinkHandler(DataSinkHandler):
    ...
    def begin(self):
        ...
        self._colwidths = [
            max(len(name), len(unit), 8) for (name, unit) in zip(names, units)
        ]
        ...
        if ds.settype != SUBSCAN:
            ...
            session.log.info(
                "Started at:         %s", strftime(TIMEFMT, localtime(ds.started))
            )
    ...
    def addSubset(self, subset):
        ...
        cols = (
            [pointstr]
            + [
                safe_format(info.fmtstr, val)
                for (info, val) in zip(ds.devvalueinfo, point.devvaluelist)
            ]
            + [
                safe_format(info.fmtstr, val)
                for (info, val) in zip(ds.envvalueinfo, point.envvaluelist)
            ]
            + [
                safe_format(info.fmtstr, val)
                for (info, val) in zip(ds.detvalueinfo, point.detvaluelist)
            ]
            + point.filenames
        )
...
class AsciiScanfileSinkHandler(DataSinkHandler):
    ...
    def prepare(self):
        ...
        fp = self.manager.createDataFile(
            self.dataset, self._template, self.sink.subdir, filemode=self.sink.filemode
        )
    ...
    def _write_header(self, ds, nfiles):
        ...
        self._colunits = [u or "-" for u in xunits + [";"] + yunits + [""] * nfiles]
        self._file.flush()
    ...
    def addSubset(self, subset):
        ...
        values = [
            safe_format(info.fmtstr, val)
            for (info, val) in zip(self.dataset.devvalueinfo, point.devvaluelist)
        ] + [
            safe_format(info.fmtstr, val)
            for (info, val) in zip(self.dataset.envvalueinfo, point.envvaluelist)
        ]
        values += [";"]
        values += [
            safe_format(info.fmtstr, val)
                ...
                if lin.startswith("###"):
                    ...
                    if cat:
                        ...
                        self.scandataset.started = mktime(
                            datetime.datetime.strptime(created, TIMEFMT).timetuple()
                        )
                        session.log.debug(
                            "created: %s, started: %s",
                            created,
                            self.scandataset.started,
                        )
                    ...
                    if entry == "header":
                        ...
                        if key == "filename":
                            ...
                            session.log.warning("unknown header: %s=%s", key, value)
                    elif entry == "data":
                        ...
                        if not scanheader:
                            ...
                            devunits, detunits = self.getDevicesDetectors(line[1:])
                            session.log.debug(
                                "units: dev %s, det: %s", devunits, detunits
                            )
                            ...
                            self.scandataset.detvalueinfo = [
                                Value(d.name, unit=u, type=guess_det_type(d.name, u))
                    ...
                    pds._addvalues(
                        {dev.name: (None, val) for dev, val in zip(self.devs, devvals)}
                    )
                    pds.results.update(
                        {det.name: [val] for det, val in zip(self.dets, detvals)}
                    )
