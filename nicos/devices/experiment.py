...
class Experiment(Device):
    ...
    * `scripts` is managed by the session and should contain a stack of code of
    ...
    parameters = {
        ...
        "propprefix": Param(
            "Prefix of the proposal if is a number",
            type=str,
            settable=True,
            default="p",
        ),
        ...
        "title": Param(
            "Experiment title", type=str, volatile=True, category="experiment"
        ),
        "users": Param(
        ...
        "remark": Param(
            "Current remark about experiment configuration",
            type=str,
            settable=True,
            category="experiment",
        ),
        ...
        "managerights": Param(
            "A dict of en/disableDir/FileMode to manage "
        ...
        "mailserver": Param(
            "Mail server name", type=str, settable=True, userparam=False
        ),
        "mailsender": Param(
            "Mail sender address", settable=True, type=none_or(mailaddress)
        ),
        "mailtemplate": Param(
            "Mail template file name (in templates)", type=str, default="mailtext.txt"
        ),
        "mailsecurity": Param(
        ...
        "pausecount": Param(
            "Reason for pausing the count loop",
            type=str,
            settable=True,
            internal=True,
            no_sim_restore=True,
        ...
        "proposalpath": Param(
            "Proposal prefix upon creation of experiment",
            type=str,
            internal=True,
            settable=True,
        ),
        "sampledir": Param(
            "Current sample-specific subdir",
            type=subdir,
            default="",
            internal=True,
            settable=True,
    ...
    def doWriteProposalpath(self, newproposalpath):
        ...
        self._set_symlink(
            ...
            path.relpath(newproposalpath, path.dirname(self.proposalsymlink)),
        )
        ...
        if self.elog:
            ...
            session.elogEvent(
                "directory", (newproposalpath, instname, path.basename(newproposalpath))
            )

    def doWriteSampledir(self, newsampledir):
        # handle current symlink
        self._set_symlink(
            self.samplesymlink, path.join(self.proposalpath, newsampledir)
        )
    ...
    def _set_symlink(self, location, target):
        ...
        if hasattr(os, "symlink"):
            if path.islink(location):
                ...
                os.unlink(location)
            ensureDirectory(
                path.join(path.dirname(location), target), **self.managerights
            )
    ...
    def getDataFilename(self, filepath, *subdirs):
        ...
        if path.isabs(filepath):
            ...
            fullname = path.abspath(path.join(self.datapath, *(subdirs + (filepath,))))
    ...
    def doInit(self, mode):
        ...
        if self.propprefix:
            try:
                ...
                raise ConfigurationError(
                    self,
                    "the serviceexp parameter "
                    "must be set to %r, not just %r"
                    % (self.propprefix + self.serviceexp, self.serviceexp),
                )
        ...
        if self.elog and mode != SIMULATION:
            ...
            ensureDirectory(
                path.join(self.proposalpath, "logbook"), **self.managerights
            )
            session.elogEvent("directory", (self.proposalpath, instname, self.proposal))
        ...
        if not self.templates:
            self._setROParam(
                "templates", path.abspath(path.join(config.nicos_root, "template"))
            )
    ...
    def new(self, proposal, title=None, localcontact=None, user=None, **kwds):
        ...
        if proptype == "user" and self.mustFinish:
            self.log.error(
                "cannot switch directly to new user experiment, "
        ...
        if localcontact:
            if isinstance(localcontact, list):
                ...
                kwds.setdefault("localcontacts", []).append({"name": localcontact})
        ...
        if self.managerights and proptype == "user":
            ...
            self.log.debug("enableDirectory: %s", self.proposalpath_of(proposal))
            enableDirectory(
                self.proposalpath_of(proposal), logger=self.log, **self.managerights
            )
        ...
        if self.proptype == "user":
            if self._mode != SIMULATION:
                ...
                if self.zipdata or self.sendmail:
                    pzip = path.join(self.proposalpath, "..", self.proposal + ".zip")
                try:
                ...
                thd = createThread(
                    "FinishExperiment " + self.proposal,
                    target=self._finish,
                    args=(
                        pzip,
                        self.proposalpath,
    ...
    def getTemplate(self, tmplname):
        ...
        for tmpldir in self.templatepath:
            if path.isfile(path.join(tmpldir, tmplname)):
                with open(path.join(tmpldir, tmplname), "r", encoding="utf-8") as f:
                    return f.read()
    ...
    def checkTemplates(self, proposal, kwargs):
        ...
        for fn, content in self.iterTemplates(only_dot_template=True):
            ...
            finalname = path.join(
                self.proposalpath_of(proposal), self.sampledir, "scripts", newfn
            )
        ...
        items = [
            [item["key"], item["default"] or "", item["description"] or ""]
        ...
        if allmissing:
            raise NicosError(
                "some keywords are missing, please provide them as "
        ...
        for fn, content in self.iterTemplates(only_dot_template=False):
            ...
            if istemplate:
                ...
                try:
                    ...
                    self.log.warning("could not translate template file %s", fn, exc=1)
            # save result
            with open(finalname, "w", encoding="utf-8") as fp:
                fp.write(content)
            os.chmod(
                finalname, self.managerights.get("enableFileMode", DEFAULT_FILE_MODE)
            )
    ...
    def _mail(self, proposal, stats, receivers, zipname, maxAttachmentSize=10000000):
        ...
        try:
            ...
            self.log.warning(
                "reading mail template %s failed", self.mailtemplate, exc=1
            )
        ...
        if os.stat(zipname).st_size < maxAttachmentSize:
            ...
            self.log.info(
                "Zipfile is too big to send via email and will be "
    ...
    def _finish(self, pzip, proposalpath, proposal, proptype, stats, receivers):
            ...
            try:
                ...
                if receivers:
                    try:
                        ...
                        self.log.warning("could not send the data via email", exc=1)
                # "hide" compressed file by moving it into the
                # proposal directory
                self.log.info("moving compressed file to %s", proposalpath)
                try:
                    os.rename(
                        pzipfile, path.join(proposalpath, path.basename(pzipfile))
                    )
                except Exception:
                    self.log.warning(
        ...
        if self.managerights and proptype == "user":
            disableDirectory(proposalpath, logger=self.log, **self.managerights)
    ...
    def _createCustomProposalSymlink(self):
        ...
        # create symlink
        ensureDirectory(path.dirname(self.customproposalsymlink), **self.managerights)
        try:
            self.log.debug(
                "create custom proposal symlink %r -> %r",
    ...
    def _scrubDetEnvLists(self):
        ...
        for entry in self.envlist:
            ...
            if devname not in session.configured_devices:
                self.log.warning(
                    "removing device %r from environment, it "
                    "does not exist in any loaded setup",
                    devname,
                )
            else:
...
class ImagingExperiment(Experiment):
    ...
    parameters = {
        ...
        "lastdarkimage": Param(
            "Last dark image", type=str, settable=False, default="", category="general"
        ),
        "lastopenbeamimage": Param(
            "Last Open Beam image",
            type=str,
            settable=False,
            default="",
            category="general",
...
class SXTalExperiment(Experiment):
    parameters = {
        "centeredrefs": Param(
            "List of centered reflections",
            type=list,
            settable=True,
            category="experiment",
        ),
        "checkrefs": Param(
            "List of reflections to re-check regularly",
            type=list,
            settable=True,
            category="experiment",
        ),
