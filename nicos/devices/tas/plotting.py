...
class SpaceMap:
    ...
    def format_coord(self, x, y):
        ...
        return "( %.4f  %.4f  %.4f )  Q = %.4f A-1     " % (h, k, l, sqrt(x**2 + y**2))
    ...
    def plot_ellipsoid(self, hkl, x, y):
        ...
        plot_resatpoint(
            None, None, "Resolution at selected HKL point", resmat=self.resmat
        )
    ...
    def display_hkl(self, x, y, textobj):
        ...
        text = "hkl = (%.4f %.4f %.4f) E = %.4f %s: %s = %.4f deg;  %s = %.4f deg" % (
            ...
            self.E,
            self.tasinfo["energytransferunit"],
            self.tasinfo["phiname"],
            angles[2],
            self.tasinfo["psiname"],
            angles[3],
        )
        textobj.set_text(text)
        return hkl
    ...
    def plot_map(self):
        ...
        if complen == 0:
            ...
            f = min(gcd(comp[0], comp[1]), gcd(comp[1], comp[2]), gcd(comp[0], comp[2]))
        dir2 /= f

        # set up pylab figure
        pylab.ion()
        pylab.figure(
            "Reciprocal space visualization", figsize=(7, 7), dpi=120, facecolor="1.0"
        )
        pylab.clf()
...
def plot_resatpoint(cfg, par, fignum="Resolution calculation", resmat=None):
    ...
    pylab.rc("text.latex", preamble="\\usepackage{amsmath}\\usepackage{helvet}")
    pylab.subplots_adjust(
        left=0.11, bottom=0.08, right=0.97, top=0.81, wspace=0.25, hspace=0.27
    )
    # register event handler to pylab
    pylab.connect("key_press_event", pylab_key_handler)
...
\end{tabular}\newline """
    ...
    if resmat.par["kfix"] == 1:
        text += (
            r"fixed incident energy $k_i=%2.4f$\,\AA$^{-1}$ ($\equiv %4.2f$\,meV)\newline "
            % (resmat.par["k"], resmat.par["k"] ** 2 * 2.07)
        )
    else:
        text += (
            r"fixed final energy $k_f=%2.4f$\,\AA$^{-1}$ ($\equiv %4.2f$\,meV)\newline "
            % (resmat.par["k"], resmat.par["k"] ** 2 * 2.07)
        )
    text += (
        r"position: qh = %1.3f qk = %1.3f ql = %1.3f (r.l.u.) en = %2.3f (meV)\newline "
    ...
    text += (
        r"$M = \left(\begin{array}{rrrr} %5.2f & %5.2f & %5.2f & %5.2f\\ "
...
def plot_resscan(cfg, par, hkles, fignum="Scan resolution", resmat=None):
    ...
    pylab.clf()
    pylab.subplots_adjust(
        left=0.11, bottom=0.08, right=0.97, top=0.96, wspace=0.25, hspace=0.39
    )
    # register event handler to pylab
    pylab.connect("key_press_event", pylab_key_handler)
    ...
    for i in range(n):
        ...
        if not resmat.ERROR:
            ...
            print(
                "%+1.4f %+1.4f %+1.4f %+1.4f => scattering triangle did not "
