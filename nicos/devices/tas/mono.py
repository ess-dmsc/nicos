...
class Monochromator(HasLimits, HasPrecision, BaseMonochromator):
    ...
    * "THz" -- energy in THz
    ...
    attached_devices = {
        ...
        "focush": Attach("Horizontal focusing axis", Moveable, optional=True),
        "focusv": Attach("Vertical focusing axis", Moveable, optional=True),
    }
    ...
    parameters = {
        "dvalue": Param(
            "d-value of the reflection used",
            unit="A",
            mandatory=True,
            settable=True,
            category="instrument",
        ...
        "reltheta": Param(
            "True if theta position is relative to two-theta",
            type=bool,
            default=False,
            category="instrument",
        ),
        "focmode": Param(
            ...
            default="manual",
            settable=True,
            type=oneof("manual", "flat", "horizontal", "vertical", "double"),
            category="instrument",
        ),
        "hfocuspars": Param(
            "Horizontal focus polynomial coefficients",
            type=listof(float),
            default=[0.0],
            settable=True,
            category="instrument",
        ...
        "hfocusflat": Param(
            "Horizontal focus value for flat mono", type=float, default=0, settable=True
        ),
        "vfocuspars": Param(
            "Vertical focus polynomial coefficients",
            type=listof(float),
            default=[0.0],
            settable=True,
            category="instrument",
        ...
        "vfocusflat": Param(
            "Vertical focus value for flat mono", type=float, default=0, settable=True
        ),
        ...
        "crystalside": Param(
            "Scattering sense for which theta is two-theta/2 "
    ...
    def doInit(self, mode):
        ...
        self._axisprecision = (
            self._attached_twotheta.precision + 2 * self._attached_theta.precision
        )
    ...
    def _calc_angles(self, k):
        try:
            ...
            raise LimitError(
                self,
                "wavelength not reachable with d=%.3f A and n=%s"
    ...
    def doIsAllowed(self, pos):
        ...
        if not ok:
            return ok, "[%s] moving to %s, " % (
                ttdev,
                ttdev.format(ttvalue, unit=True),
            ) + why
        return True, ""
    ...
    def doStatus(self, maxage=0):
        ...
        const, text = multiStatus(
            (
                ...
                for name in ["theta", "twotheta", "focush", "focusv"]
            ),
            maxage,
        )
    ...
    def doFinish(self):
        ...
        if abs(tt - 2.0 * th) > self._axisprecision:
            ...
            self.log.info(
                "precisions: tt:%s, th:%s, combined: %s",
                self._attached_twotheta.precision,
                self._attached_theta.precision,
                self._axisprecision,
            )
    ...
    def doReadPrecision(self):
        ...
        dtheta = self._attached_theta.precision + self._attached_twotheta.precision
        dlambda = abs(
            2.0
            * self.dvalue
    ...
    def _calcurvature(self, l1, l2, k, vertical=True):
        ...
        return 0.5 * (1.0 / l1 + 1.0 / l2) * sin(radians(abs(theta))) ** exp
