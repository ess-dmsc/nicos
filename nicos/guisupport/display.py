...
from nicos.guisupport.utils import setBackgroundColor, setBothColors, setForegroundColor
...
class ValueLabel(SqueezedLabel):
    ...
    dev = PropDef(
        "dev", str, "", "NICOS device name, if set, display value of this device"
    )
    ...
    format = PropDef(
        ...
        str,
        "",
        "Python format string to use for the "
...
class TimeValueLabel(ValueLabel):
    designer_description = (
        "Value Label which displays a time in a human readable format"
    )
...
class ValueDisplay(NicosWidget, QWidget):
    ...
    name = PropDef(
        ...
        str,
        "",
        "Name of the value to display above/"
    ...
    unit = PropDef(
        ...
        str,
        "",
        "Unit of the value to display next to "
    ...
    maxlen = PropDef(
        ...
        int,
        -1,
        "Maximum length of the value string to allow; defaults to no limit",
    )
    width = PropDef(
        "width", int, 8, "Width of the widget in units of the width of one character"
    )
    istext = PropDef(
        "istext",
        bool,
        False,
        'If given, a "text" font will be '
    ...
    showName = PropDef(
        ...
        bool,
        True,
        "If false, do not display the label for the value name",
    )
    showStatus = PropDef(
        "showStatus",
        bool,
        True,
        "If false, do not display the device status as a color of the value text",
    )
    showExpiration = PropDef(
        "showExpiration",
        bool,
        True,
        'If true, display expired cache values as "n/a"',
    )
    horizontal = PropDef(
        "horizontal",
        bool,
        False,
        "If true, display name label left of the value instead of above it",
    )
    ...
    def propertyUpdated(self, pname, value):
        if pname == "dev":
            ...
            if value < 0:
                ...
                self.valuelabel.setMinimumSize(QSize(round(onechar * (value + 0.5)), 0))
        elif pname == "istext":
    ...
    def initUi(self):
        ...
        valuelabel = SensitiveSMLabel(
            "----", self, self._label_entered, self._label_left
        )
        ...
        setBothColors(
            valuelabel,
            (self._colorscheme["fore"][UNKNOWN], self._colorscheme["back"][UNKNOWN]),
        )
    ...
    def registerKeys(self):
        if self.props["dev"]:
            self.registerDevice(
                self.props["dev"], self.props["unit"], self.props["format"]
            )
        else:
            self.registerKey(
                self.props["key"],
                self.props["statuskey"],
                self.props["unit"],
                self.props["format"],
            )
    ...
    def on_devValueChange(self, dev, value, strvalue, unitvalue, expired):
        ...
        if self._expired:
            setBothColors(
                self.valuelabel,
                (self._colorscheme["fore"][UNKNOWN], self._colorscheme["expired"]),
            )
            ...
            setBothColors(
                self.valuelabel,
                (self._colorscheme["fore"][BUSY], self._colorscheme["back"][BUSY]),
            )
    ...
    def _label_entered(self, widget, event, from_mouse=True):
        infotext = "%s = %s" % (
            self.props["name"] or self.props["dev"] or self.props["key"],
        ...
        infotext += ", changed %s ago" % (nicedelta(currenttime() - self._lastchange))
        self.widgetInfo.emit(infotext)
        if from_mouse:
            self._mousetimer = QTimer(
                self, timeout=lambda: self._label_entered(widget, event, False)
            )
...
class PictureDisplay(NicosWidget, QWidget):
    ...
    filepath = PropDef(
        "filepath", str, "", "Path to the picture that should be displayed"
    )
    name = PropDef("name", str, "", "Name (caption) to be displayed above the picture")
    refresh = PropDef("refresh", int, 0, "Interval to check for updates in seconds")
