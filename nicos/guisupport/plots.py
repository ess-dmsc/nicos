...
class NicosTimePlotAxes(NicosPlotAxes):
    ...
    def __init__(
        ...
        NicosPlotAxes.__init__(
            self, viewport, xtick, ytick, majorx, majory, drawX, drawY
        )
    ...
    def doAutoScale(self, curvechanged=None):
        ...
        if (
            self.slidingwindow
            and self.autoscale & PlotAxes.SCALE_X
            and (vc.xmax - xmin) > self.slidingwindow
        ):
...
""",
    ...
    def initUi(self):
        ...
        self.plot = Plot(viewport=(0.1, 0.95, 0.25, 0.95))
    ...
    def propertyUpdated(self, pname, value):
        if pname == "plotwindow":
            ...
            self.setMinimumSize(
                QSize(
                    round(self._scale * (self.props["width"] + 0.5)),
                    round(self._scale * (self.props["height"] + 0.5)),
                )
            )
        elif pname == "legend":
    ...
    def addcurve(self, key, expr, title):
        series = TimeSeries(
            title, self.props["plotinterval"], expr, self.props["plotwindow"], self
        )
        series.initEmpty()
    ...
    def registerKeys(self):
        for keyexpr, title in itertools.zip_longest(
            self.props["devices"], self.props["names"]
        ):
            if title is None:
                title = keyexpr
