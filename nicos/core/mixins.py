...
class HasAutoDevices(DeviceMixinBase):
    ...
    parameters = {
        "autodevice_visibility": Param(
            "Selects in which context the auto "
    ...
    **Important:** If the device is also an instance of `HasOffset`, it should
    ...
    def doReadUserlimits(self):
        if "userlimits" not in self._config:
            self.log.info(
                "setting userlimits from abslimits, which are %s", self.abslimits
            )
    ...
    def _check_in_range(self, curval, userlimits):
        ...
        if curval + precision < userlimits[0]:
            return status.WARN, "below user limit (%s)" % self.format(
                userlimits[0], unit=True
            )
        elif curval - precision > userlimits[1]:
            return status.WARN, "above user limit (%s)" % self.format(
                userlimits[1], unit=True
            )
...
class HasOffset(DeviceMixinBase):
    ...
    parameters = {
        "offset": Param(
            "Offset of device zero to hardware zero",
            unit="main",
            settable=True,
            category="offsets",
            chatty=True,
    ...
    def doWriteOffset(self, value):
        ...
        if self._cache:
            self._cache.put(
                self, "value", self.read(0) - diff, currenttime(), self.maxage
            )
...
class HasMapping(DeviceMixinBase):
    ...
    parameters = {
        "mapping": Param(
            "Mapping of device values to raw (internal) values",
            unit="",
            settable=False,
            mandatory=True,
    ...
    def doIsAllowed(self, target):
        if target not in self.mapping:
            return False, "unknown value: %r, must be one of %s" % (
                target,
                ", ".join(map(repr, sorted(self.mapping))),
            )
        return True, ""
        ...
        *not* call `timeoutAction`.
        ...
        if code == status.BUSY:
            if self.isTimedOut():
                ...
                if not self._timeoutActionCalled and session.mode in (
                    MASTER,
                    MAINTENANCE,
                ):
                    try:
                        ...
                        self.log.exception("error calling timeout action", exc=True)
                    finally:
                        self._timeoutActionCalled = True
            elif self._timesout:
...
class HasWindowTimeout(HasPrecision, HasTimeout):
    ...
    parameters = {
        "window": Param(
            "Time window for checking stabilization",
            unit="s",
            default=60.0,
            fmtstr="%.1f",
            settable=True,
    ...
    def _getTimeoutTimes(self, current_pos, target_pos, current_time):
        ...
        differentiate between 'ramping' and 'stabilization').
        """
        res = HasTimeout._getTimeoutTimes(self, current_pos, target_pos, current_time)
        if not res:
            return None
    ...
    def isAtTarget(self, pos=None, target=None):
        ...
        stable = all(abs(v - target) <= self.precision for v in hist_in_window)
    ...
    def doTime(self, old_value, target):
        ...
        if "speed" in self.parameters and self.speed != 0:
            ...
            return abs(target - old_value) / (self.ramp / 60.0) + self.window
        return self.window
...
class HasCommunication(DeviceMixinBase):
    ...
    parameters = {
        ...
        "comdelay": Param(
            "Delay between retries", unit="s", default=0.1, fmtstr="%.1f", settable=True
        ),
    }
    ...
    def _com_warn(self, retries, name, err, info):
        ...
        if retries == self.comtries - 1:
            self.log.warning("%s failed, retrying up to %d times", name, retries, exc=1)
    ...
    def _enable(self, on):
        ...
        if getattr(self, "requires", None):
            try:
                ...
                raise AccessError(self, "cannot %s device: %s" % (what, err)) from None
