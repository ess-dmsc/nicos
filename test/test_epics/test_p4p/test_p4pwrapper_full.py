...
def test_batch_readings_include_values_enums_and_alarms(pva_rig, pva_wrapper):
...
def test_batch_readings_fail_when_one_pv_is_unavailable(pva_rig):
    wrapper = P4pWrapper(timeout=0.1, context=pva_rig.ctx)
    with pytest.raises(CommunicationError, match="Missing"):
        wrapper.get_pv_readings(
            {
