...
class ScriptSessionTest(ScriptSession):
    ...
    def createRootLogger(self, prefix="nicos", console=True, logfile=True):
        ...
        handler.setFormatter(logging.Formatter("[SCRIPT] %(name)s: %(message)s"))
        self.log.addHandler(handler)
        log_path = path.join(runtime_root, "log")
        try:
            if prefix == "nicos":
                self.log.addHandler(
                    loggers.NicosLogfileHandler(log_path, "nicos", str(os.getpid()))
                )
...
@pytest.mark.parametrize(
    "script", [f for f in os.listdir(testscriptspath) if f.endswith(".nic")]
)
def test_one_script(session, script):
    ...
    if m:
        ...
        assert raises(expected, run_script_session, session, "script_tests", code)
    else:
        # expected success
