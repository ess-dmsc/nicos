...
devices = dict(
    Sample=device(
        "nicos_virt_mlz.reseda.devices.sample.Sample",
    ),
    Exp=device(
        "nicos_mlz.reseda.devices.Experiment",
    ...
    selector_lambda=device(
        "nicos_mlz.reseda.devices.SelectorLambda",
    ...
    selector_delta_lambda=device(
        "nicos_mlz.reseda.devices.SelectorLambdaSpread",
    ...
    echotime=device(
        "nicos_mlz.reseda.devices.EchoTime",
...
for sname in ["cbox_0a", "cbox_0b", "cbox_1a"]:
    devices["%s_fg_freq" % sname] = device(
        "nicos.devices.generic.ManualMove",
        pollinterval=30,
        fmtstr="%.4g",
        unit="Hz",
    ...
    devices["%s_reg_amp" % sname] = device(
        "nicos.devices.generic.ManualMove",
        unit="V",
        abslimits=(0, 10),
    )
    devices["%s_fg_amp" % sname] = device(
        "nicos.devices.generic.ManualMove",
        pollinterval=5,
        unit="V",
        abslimits=(0, 10),
