...
from .conftest import assert_connection_unknown_status
...
class TestEpicsAnalogMoveable:
    ...
    def test_daemon_analog_moveable_fresh_status_unknown_when_fully_disconnected(
        self, device_harness, fake_backend
    ):
        # A fresh status(0) with the IOC fully down must not let the
        # target/readback read raise and surface as (ERROR, raw wrapper text);
        # it must report the same UNKNOWN as the cached path.
        config = analog_moveable_config()
        config["monitor"] = False
        fake_backend.values[config["readpv"]] = 1.0
        fake_backend.values[config["writepv"]] = 1.0
    ...
    def test_poller_analog_moveable_callbacks_cache_target_limits_and_status(
        ...
        fake_backend.emit_update(f"{config['readpv']}.PREC", value=3)
        ...
        assert (
            device_harness.run(
                "poller", poller_device._cache.get, poller_device, "fmtstr"
            )
            == "%.3f"
        )
        assert device_harness.run(
