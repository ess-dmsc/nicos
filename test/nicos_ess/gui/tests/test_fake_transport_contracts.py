...
def test_fake_daemon_returns_minimal_expected_command_shapes(monkeypatch, fake_daemon):
...
def test_fake_daemon_handles_the_real_client_device_list_eval(monkeypatch, fake_daemon):
...
def test_unknown_command_returns_error_reply_and_is_recorded(
    ...
    try:
        assert (
            client.ask("does-not-exist", noerror=True, default="fallback") == "fallback"
        )
        assert client.isconnected is True
    finally:
        _disconnect_client(client)
