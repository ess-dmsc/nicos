...
def test_get_pt100_status_message_error():
    ...
    assert get_pt100_status_message(error | 0b10000) == (
        status.ERROR,
        "limit2 overshot",
    )
    assert get_pt100_status_message(error | 0b100000) == (
        status.ERROR,
        "limit2 undershot",
    )
...
class TestEpicsPT100(TestCase):
    ...
    def test_status_underrange(self, mock):
        ...
        create_method_patch(
            self, EpicsPT100Temperature, "_get_pv", return_value_wrapper(0x40 | 0b1)
        )
    ...
    def test_status_overrange(self, mock):
        sensor = self.session.getDevice("temperature_sensor")
        create_method_patch(
            self, EpicsPT100Temperature, "_get_pv", return_value_wrapper(0x40 | 0b10)
        )
    ...
    def test_status_limit1_undershot(self, mock):
        sensor = self.session.getDevice("temperature_sensor")
        create_method_patch(
            self, EpicsPT100Temperature, "_get_pv", return_value_wrapper(0x40 | 0b100)
        )
    ...
    def test_status_limit1_overshot(self, mock):
        sensor = self.session.getDevice("temperature_sensor")
        create_method_patch(
            self, EpicsPT100Temperature, "_get_pv", return_value_wrapper(0x40 | 0b1000)
        )
    ...
    def test_status_limit2_undershot(self, mock):
        sensor = self.session.getDevice("temperature_sensor")
        create_method_patch(
            self, EpicsPT100Temperature, "_get_pv", return_value_wrapper(0x40 | 0b10000)
        )
    ...
    def test_unknown_error_status(self, mock):
        sensor = self.session.getDevice("temperature_sensor")
        create_method_patch(
            self, EpicsPT100Temperature, "_get_pv", return_value_wrapper(0x40)
        )
    ...
    def test_invalid_status_message_doesn_t_change_device_status(self, mock):
        ...
        create_method_patch(
            self, EpicsPT100Temperature, "_get_pv", return_value_wrapper(-1)
        )
        assert sensor.doStatus() == (status.ERROR, "error")
        sensor._status = (status.UNKNOWN, "")
    ...
    def test_read_return_read_value_if_statuspv_is_missing(self, mock):
        ...
        sensor._value = 132
        create_method_patch(self, EpicsReadable, "doRead", return_value_wrapper(42))
