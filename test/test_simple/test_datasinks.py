...
class TestSinks:
    ...
    def root_setup(self, session):
        ...
        assert path.abspath(exp.datapath) == path.abspath(
            path.join(config.nicos_root, "testdata", year, "p1234", "data")
        )
    ...
    def test_filecounters(self, session):
        ...
        for directory, ctrs in zip(
            ...
            [("scan 43", "point 172"), ("scan 1", "point 5"), ("scan 1", "point 5")],
        ):
    ...
    @pytest.mark.skipif(
        not (quickyaml and yaml), reason="QuickYAML/PyYAML libraries missing"
    )
    def test_yaml_sink_2(self, session):
        ...
        assert contents["measurement"]["sample"]["description"]["name"] == "mysample"
...
def test_sink_ordering(session):
    ...
    def _test(sinks):
        ...
        assert all(
            ...
            for (h1, h2) in zip(dataset.handlers, dataset.handlers[1:])
        )
