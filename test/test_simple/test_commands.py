...
class TestBasic:
    ...
    def test_basic_commands(self, session, log):
        with log.assert_msg_matches(
            [r"Usage: help\(\[object\]\)", r">>> help\(\) {12}# show list of commands"]
        ):
            help(help)
        with log.assert_msg_matches(
            [
                r"name {48}description",
    ...
    def test_setup_commands(self, session, log):
        with log.assert_msg_matches(
            [r"axis +yes", r"stdsystem +yes", r"cache +(?!yes)"]
        ):  # cache not loaded
            ListSetups()
...
def test_experiment_commands(session, log):
    ...
    with log.assert_msg_matches(
        [r"Exp : experiment directory is now .*", r'Exp : User "F. X. User" added']
    ):
        NewExperiment(
            1234, "Test experiment", "L. Contact <l.contact@frm2.tum.de>", "1. User"
        )
...
def test_run_command(session, log):
    ...
    with open(
        os.path.join(session.experiment.scriptpath, "test.py"), "w", encoding="utf-8"
    ) as f:
        f.write("read()")
    run("test")
...
def test_sample_commands(session, log):
    ...
    with log.assert_msg_matches(
        [r"number  sample name  param", r"0 +abc", r"1 +def +45"]
    ):
        ListSamples()

    with log.assert_no_msg_matches([r"0 +abc", r"1 +def +45"]):
        ClearSamples()
...
class TestDevice:
    ...
    def test_rmaw(self, session, log):
        ...
        for dm, da in ((mmin, amax), (mmax, amin)):
            maw(motor, 0, axis, 0)
            rmaw(motor, dm, axis, da / 2.0)
            assert motor.curvalue == dm
            assert axis.motor.curvalue == pytest.approx(da / 2.0)
    ...
    def test_info(self, session, log):
        ...
        with log.assert_msg_matches([r"Device status", r"motor +status: +ok: idle"]):
            info()
    ...
    def test_limits(self, session, log):
        ...
        with log.assert_msg_matches(
            [r"limits for this device", r"absolute limits", r"user limits"]
        ):
            limits(motor, m4)
    ...
    def test_lists(self, session, log):
        ...
        with log.assert_msg_matches(
            [r"\.addUser\(.* Experiment", r"\.version\(.* Device"]
        ):
            ListMethods(exp)
        # axis contains 'usermethods' inherited from CanReference mixin
        with log.assert_msg_matches([r"\.reference\(."]):
            ListMethods(axis)
        with log.assert_msg_matches([r"All created devices", r"motor\s*VirtualMotor"]):
            ListDevices()
...
def test_notifiers(session, log):
    ...
    with log.assert_msg_matches(
        [r"Email addresses", r"copy@example.com", r"receiver@example.com"]
    ):
        ListMailReceivers()
    with log.assert_msg_matches([r"Email addresses", r"receiver@example.com"]):
        ListDataReceivers()
