...
def test_attach_class():
    ...
    assert repr(Attach("devname", MyClass, multiple=2)) == repr(
        Attach("devname", MyClass, multiple=[2])
    )
...
def test_listof():
    assert listof(int)([0.0, 1, "2"]) == [0, 1, 2]
...
def test_floatrange():
    ...
    assert raises(ValueError, floatrange(0, 10), 15.0)
...
def test_mailaddress():
    ...
    assert (
        mailaddress("myaddress@وزارة-الأتصالات.مصر")
        == "myaddress@xn----rmckbbajlc6dj7bxne2c.xn--wgbh1c"
    )
    assert (
        mailaddress("M. Address <my.address@domain.my>")
        == "M. Address <my.address@domain.my>"
    )
    assert (
        mailaddress("M. Address <my.address@domain.my> ")
        == "M. Address <my.address@domain.my> "
...
def test_host():
    ...
    assert (
        host(defaulthost="localhost", defaultport="456")("otherhost") == "otherhost:456"
    )
    assert (
        host(defaulthost="localhost", defaultport="456")("otherhost:789")
        == "otherhost:789"
    )
...
def test_ArrayDesc():
    ...
    assert (
        ad2.name == ad.name
        and ad2.shape == ad.shape
        and ad2.dtype == ad.dtype
        and ad2.dimnames == ad.dimnames
    )
