...
from nicos_mlz.kws1.devices.selector import SelectorSwitcher as KWS1SelectorSwitcher
...
class SelectorLambda(Moveable):
    ...
    parameters = {
        "constants": Param(
            "Conversion constants: " "lam[Ang] = constant/speed[Hz] + offset",
            type=tupleof(float, float),
            mandatory=True,
            unit="Ang Hz",
        ),
        "offsets": Param(
            "Conversion offsets: " "lam[Ang] = constant/speed[Hz] + offset",
            type=tupleof(float, float),
            mandatory=True,
            unit="Ang",
        ),
    ...
    def doRead(self, maxage=0):
        ...
        return (
            (60 * self.constants[tilted] / speed) + self.offsets[tilted]
            if speed
            else -1
        )
    ...
    def doIsAllowed(self, value):
        ...
        speed = int(round(60 * self.constants[tilted] / (value - self.offsets[tilted])))
        return self._attached_seldev.isAllowed(speed)

    def doStart(self, target):
        tilted = bool(self._attached_tiltdev.read(0))
        speed = int(
            round(60 * self.constants[tilted] / (target - self.offsets[tilted]))
        )
