...
devices = dict(
    ep_HV_all=device(
        "nicos_mlz.kws1.devices.gedet.MultiHV",
    ...
    gedet_HV=device(
        "nicos_mlz.kws1.devices.gedet.HVSwitcher",
...
for epname, epicsid in eps:  # + [('ep09x', 'GE-D7561E-EP')]:
    devices[epname + "_T"] = device(
        "nicos.devices.epics.pyepics.EpicsReadable",
    ...
    devices[epname + "_HV"] = device(
        "nicos_mlz.kws1.devices.gedet." "HVEpicsAnalogMoveable",
    ...
    devices[epname + "_cts"] = device(
        "nicos_mlz.kws1.devices.gedet.HVEpicsArrayReadable",
...
for ti in range(1, 3):
    devices["ps%d_V" % ti] = device(
        "nicos_mlz.kws1.devices.gedet.GEPowerSupply",
