...
_experiment = Block(
    ...
    [
        BlockRow(
            ...
            Field(
                name="Sample", key="sample/samplename", width=15, istext=True, maxlen=15
            ),
            Field(
                name="Current status",
                key="exp/action",
...
def make_blocks(name, setup, rows, setups=None):
    ...
    return [
        ...
        Block(
            "VIRTUAL " + name, copy.deepcopy(rows), setups="virtual_" + setup + postfix
        ),
    ]


_selector = make_blocks(
    "Selector",
    "selector",
    [
...
_detector = make_blocks(
    "Detector",
    "detector",
    [
        BlockRow(
            Field(name="Preset", dev="detector", istext=True, width=17),
            Field(name="GE HV", dev="gedet_HV", istext=True),
        ),
        BlockRow(
            Field(
                devices=["det_z", "beamstop_x", "beamstop_y"],
                beamstop=True,
                widget="nicos_mlz.kws1.gui.monitorwidgets.Tube",
                width=70,
                height=13,
            )
        ),
...
_polarizer = make_blocks(
    "Polarizer",
    "polarizer",
    [
        BlockRow(
            Field(name="Pol. setting", dev="polarizer", istext=True),
...
_lenses = make_blocks(
    ...
    [
        BlockRow(
            Field(
                devices=["lens_in", "lens_out"],
                widget="nicos_mlz.kws1.gui.monitorwidgets.Lenses",
                width=30,
                height=8,
            )
        ),
...
_shutter = make_blocks(
    "Shutter / Outside",
    "shutter",
    [
        BlockRow(
            Field(name="Shutter", dev="shutter", istext=True, width=9),
...
_sample = make_blocks(
    ...
    [
        BlockRow(
            ...
            Field(
                device="ap_sam",
                widget="nicos_mlz.kws1.gui.monitorwidgets.SampleSlit",
                width=10,
                height=10,
            ),
        ),
...
_sample_withrot = make_blocks(
    "Sample",
    "sample",
    [
        BlockRow(
            Field(name="Rotation", dev="sam_rot"),
            Field(name="Trans X", dev="sam_trans_x"),
            Field(name="Trans Y", dev="sam_trans_y"),
            Field(
                device="ap_sam",
...
_hexapod = Block(
    ...
    [
        BlockRow(
            Field(name="TX", dev="hexapod_tx"),
            Field(name="TY", dev="hexapod_ty"),
            Field(name="TZ", dev="hexapod_tz"),
        ),
        BlockRow(
            Field(name="RX", dev="hexapod_rx"),
...
_peltier = Block(
    "Peltier/Julabo", [BlockRow("T_peltier", "T_julabo")], setups="peltier"
)
...
_peltierplot = Block(
    ...
    [
        BlockRow(
            Field(plot="TT", dev="T_peltier", width=30, height=25, plotwindow=2 * 3600),
...
_julabo = Block("Julabo", [BlockRow("T_julabo")], setups="waterjulabo and not peltier")

_julaboplot = Block(
    "",
    [
        BlockRow(
            Field(plot="JT", dev="T_julabo", width=30, height=25, plotwindow=2 * 3600),
...
_et = Block("Eurotherm", [BlockRow("T_et")], setups="eurotherm")

_etplot = Block(
    "",
    [
        BlockRow(
            Field(plot="ET", dev="T_et", width=30, height=25, plotwindow=2 * 3600),
...
_ccr = Block(
    ...
    [
        BlockRow(
            Field(name="Setpoint", key="t/setpoint", unitkey="t/unit", format="%.2f"),
            Field(name="Tube", dev="T"),
...
_ccrplot = Block(
    ...
    [
        BlockRow(
            Field(plot="CCR", dev="T", width=30, height=25, plotwindow=2 * 3600),
...
_ccm3a = Block(
    ...
    [
        ...
        BlockRow(
            Field(name="T1", dev="ccm3a_t1"),
            Field(name="T2", dev="ccm3a_t2"),
            Field(name="TA", dev="ccm3a_ta"),
            Field(name="TB", dev="ccm3a_tb"),
        ),
    ],
    setups="ccm3a",
)
...
_ccm3aplot = Block(
    ...
    [
        BlockRow(
            Field(
                plot="CCM3a", dev="ccm3a_t1", width=30, height=25, plotwindow=2 * 3600
            ),
            Field(plot="CCM3a", dev="ccm3a_t2"),
            Field(plot="CCM3a", dev="ccm3a_ta"),
            Field(plot="CCM3a", dev="ccm3a_tb"),
        ),
    ],
    setups="ccm3a",
)
...
_layout = [
    ...
    Row(
        Column(*(_selector + _chopper + _polarizer + _lenses + _daq)),
