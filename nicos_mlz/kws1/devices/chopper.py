...
class ChopperFrequency(WindowTimeoutAO):
    ...
    def doTime(self, old_value, target):
        if old_value is None or target is None:
            return 0.0
        if old_value == target:
            return 20.0  # phase adjustment
        total = 30.0  # phase adjustment
        total += abs(old_value - target) / 0.25  # frequency speedup
        total += 60.0 if target == 0 else 0  # complete stop adds ~1min
        return total
...
class ChopperParams(Moveable):
    ...
    def doIsAllowed(self, pos):
        ...
        if 0 < freq <= 5 or 25 <= freq <= 32:
            return (
                False,
                'moving to "forbidden" frequency ranges '
    ...
    def doStart(self, target):
        if target[0] == 0:
            if self._attached_freq1.read(0) == 0:
                return
            for dev in (
                self._attached_phase1,
                self._attached_phase2,
                self._attached_freq1,
                self._attached_freq2,
            ):
                dev.start(0)
            return
        (freq, opening) = target
    ...
    def doIsAtTarget(self, pos, target):
        ...
        rfreq, ropen = pos
        return (
            abs(tfreq - rfreq) < self._attached_freq1.precision
            and abs(topen - ropen) < self._attached_phase1.precision
        )
...
class Chopper(Moveable):
    ...
    attached_devices = {
        ...
        "det_pos": Attach("Detector preset switcher", DetectorPosSwitcherMixin),
    ...
    parameters = {
        "resolutions": Param(
            "Possible values for the resolution",
            unit="%",
            type=listof(float),
            mandatory=True,
        ),
        "channels": Param(
            "Desired number of TOF channels",
            # TODO: max channels?
        ...
        "shade": Param(
            "Desired overlap of spectrum edges",
        ...
        "tauoffset": Param(
            "Additional offset for time of flight",
            type=floatrange(0.0),
            default=0.0,
            settable=True,
            category="general",
        ...
        "nmax": Param(
            "Maximum number of acquisition frames",
            type=intrange(1, 128),
            default=25,
            settable=True,
        ),
        "calcresult": Param(
            "Last calculated setting",
            type=tupleof(float, float, int),
            settable=True,
            internal=True,
        ),
    }
    ...
    def doInit(self, mode):
        self.valuetype = oneof(
            "off", "manual", *("%.1f%%" % v for v in self.resolutions)
        )
    ...
    def doStart(self, target):
        ...
        try:
            ...
            raise PositionError(
                self,
                "cannot calculate chopper settings: selector or "
                "detector device not at preset",
            ) from None

        self.log.debug(
            "chopper calc inputs: reso=%f, lam=%f, spread=%f, " "det_z=%f",
            reso,
            lam,
            spread,
            det_z,
        )
        freq, opening = calculate(
            lam,
            spread,
            reso,
            self.shade,
        ...
        self.log.debug(
            "calculated chopper settings: freq=%f, opening=%f", freq, opening
        )
    ...
    def doRead(self, maxage=0):
        ...
        if (
            abs(params[0] - self.calcresult[0])
            < self._attached_params._attached_freq1.precision
            and abs(params[1] - self.calcresult[1])
            < self._attached_params._attached_phase1.precision
        ):
            if (
                self._attached_daq.mode == "tof"
                and self._attached_daq.tofchannels == self.channels
                and self._attached_daq.tofinterval == self.calcresult[2]
                and self._attached_daq.tofprogression == 1.0
                and self._attached_daq.tofcustom == []
            ):
                return self.target
        return "unknown"
    ...
    def doStatus(self, maxage=0):
        ...
        if r == "unknown":
            return (
                status.NOTREACHED,
                "unconfigured chopper frequency/phase " "or still moving",
            )
        return status.OK, ""
