...
from nicos.core import ConfigurationError, Param, anytype, dictof, multiWait, tupleof
...
class KWSSample(Sample):
    ...
    parameters = {
        "aperture": Param(
            "Aperture (x, y, w, h) for position",
            type=tupleof(float, float, float, float),
            unit="mm",
            settable=True,
        ),
        "position": Param(
            "Mapping of devices to positions for driving " "to this sample's position",
            type=dictof(str, anytype),
            settable=True,
        ),
        "timefactor": Param(
            "Measurement time factor for this sample",
            type=float,
            settable=True,
            category="sample",
        ),
        "thickness": Param(
            "Sample thickness (info only)",
            type=float,
            settable=True,
            unit="mm",
            category="sample",
        ...
        "detoffset": Param(
            "Detector offset (info only)",
            type=float,
            settable=True,
            unit="mm",
            category="sample",
        ),
        "comment": Param("Sample comment", type=str, settable=True, category="sample"),
        "apname": Param(
            "Name of sample aperture", type=str, mandatory=False, default="ap_sam"
        ),
    }
    ...
    def _applyParams(self, number, parameters):
        ...
        self.log.info(
            "moving to position of sample %s (%s)...",
            number,
    ...
    def set(self, number, parameters):
        for key in (
            ...
            if key not in parameters:
                raise ConfigurationError(self, "missing key %r in sample entry" % key)
