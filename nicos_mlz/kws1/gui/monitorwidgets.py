...
class Tube(NicosWidget, QWidget):
    ...
    beamstop = PropDef("beamstop", bool, False, "Beamstop instead of detector Y/Z")
    smalldet = PropDef(
        "smalldet",
        float,
        0,
        "Positioning of small detector, " "or zero for no small detector",
    )
    ...
    def paintEvent(self, event):
        ...
        if pos_val is not None:
            ...
            painter.translate(xp + fontscale / 2.0, 15 + yoff + (h - 20) / 2.0)
            painter.drawRect(
            ...
            # X translation
            painter.drawText(
                QRectF(xp + xoff, yoff + 2 * fontscale, 7 * fontscale, 2 * fontscale),
                x_str,
                QTextOption(Qt.AlignmentFlag.AlignRight),
            )
            # Y translation
            painter.drawText(
                QRectF(xp + xoff, yoff + 3.5 * fontscale, 7 * fontscale, 2 * fontscale),
                y_str,
                QTextOption(Qt.AlignmentFlag.AlignRight),
            )
            # Z position
            ...
            if self.props["beamstop"]:
                ...
                painter.drawRect(
                    QRectF(xp - 8, yoff + 15 + posscale / 350 * (1100 - y_val), 2, 10)
                )
        ...
        if self.props["smalldet"] and self._curval[4] is not None:
            ...
            painter.drawRect(QRectF(sx - fontscale / 2.0, sy, fontscale, 30))

            painter.drawText(
                QRectF(sx - 10.5 * fontscale, sy, 8 * fontscale, 2 * fontscale),
                x_str,
                QTextOption(Qt.AlignmentFlag.AlignRight),
            )
            painter.drawText(
    ...
    def paintEvent(self, event):
        ...
        for i in range(18):
            ...
            if i >= 18 - pol_bits:
                ...
                painter.drawText(
                    QRectF(x + 3, y + ely - 2, elwidth - 8, elheight / 3 + 2),
                    "POL",
                    QTextOption(Qt.AlignmentFlag.AlignHCenter),
                )
                painter.setPen(QPen(_black.color()))
            painter.drawText(
                QRectF(x, 3, elwidth, 2 * fontscale),
                str(19 - i),
                QTextOption(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignTop),
            )
            x += elwidth
        ...
        for i, slitpos in enumerate([20, 14, 8, 4, 2]):
            ...
            h = (50 - slith) * slhw / 100
            painter.drawRect(
                QRectF(xmiddle - 0.5 * slhw + w, y + 15 + h, slhw - 2 * w, slhw - 2 * h)
            )
            painter.drawText(
                QRectF(xmiddle - 0.8 * elwidth, y + 15, slhw, slhw),
                "%.1f\n%.1f" % (slitw, slith),
                QTextOption(Qt.AlignmentFlag.AlignCenter),
            )
...
class Lenses(NicosWidget, QWidget):
    ...
    def paintEvent(self, event):
        ...
        for i, n, x in [(0, 4, 0), (1, 6, 6), (2, 16, 14)]:
            ...
            for j in range(n):
                painter.drawRect(
                    QRectF((1 + x + j) * lensw, lensy, lensw + 1, lensheight)
                )
...
class SampleSlit(NicosWidget, QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self._opmode == "offcentered":
            ...
            x0, x1, y0, y1 = dx - w / 2.0, dx + w / 2.0, dy - h / 2.0, dy + h / 2.0
        ...
        painter.drawText(
            QRectF(2, 2, ww, wh),
            l1,
            QTextOption(Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignHCenter),
        )
        painter.drawText(
            QRectF(2, 2, ww, wh),
            l2,
            QTextOption(Qt.AlignmentFlag.AlignBottom | Qt.AlignmentFlag.AlignHCenter),
        )
        painter.drawText(
            QRectF(2, 2, ww, wh),
            l3,
            QTextOption(Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignLeft),
        )
        painter.drawText(
            QRectF(2, 2, ww, wh),
            l4,
            QTextOption(Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignRight),
        )
