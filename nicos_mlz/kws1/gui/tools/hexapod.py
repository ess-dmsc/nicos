...
class HexapodTool(DlgUtils, QMainWindow):
    ...
    def __init__(self, parent, _client, **_kwds):
        ...
        try:
            self._controller = DeviceProxy(TANGO_DEV_BASE + "controller")
        ...
        self.inpRampUp.setValue(tx_speed / self.query_attr(self.axes["tx"], "accel"))
        self.lblRampUp.setText(self.inpRampUp.text())
        self.inpRampDown.setValue(tx_speed / self.query_attr(self.axes["tx"], "decel"))
    ...
    def on_butStop_clicked(self):
        try:
            ...
            self.showInfo(
                "exception raised after executing stop on hexapod:\n" "%s" % err
            )

    def updateTimer(self):
        msg = "<font color=darkblue>" + self.exec_cmd(self._controller, "Status")
    ...
    def on_butSetWorkspace_clicked(self):
        ...
        try:
            ...
            workspaces = self.query_attr(
                self._controller, group + "workspaceDefinitions"
            )
            workspaces = [
                [i]
                + (
    ...
    def on_cbsWorkspace_activated(self, index):
        ...
        for _ in range(5):
            ...
            enabled = self.query_attr(self._controller, group + "WorkspaceStatus")[
                index
            ]
    ...
    def on_butSaveVel_clicked(self):
        ...
        self.set_attr(
            self.axes["tx"], "accel", self.inpVelTrans.value() / self.inpRampUp.value()
        )
        self.set_attr(
            self.axes["tx"],
            "decel",
