...
try:
    ...
    try:
        ...
        yaml.add_representer(
            AutoDefaultODict, yaml.representer.SafeRepresenter.represent_dict
        )
    except ImportError:
        yaml = None
...
class YamlDatafileSinkHandler(AsciiScanfileSinkHandler):
    ...
    units = ["second", "degree", _millimeter, _millimeter, _millimeter, "angstrom"]
    ...
    def addSubset(self, subset):
        ...
        if subset.settype != POINT:
            ...
            return
        self.log.debug("%r - %r", self.dataset.detvalueinfo, subset.detvaluelist)
    ...
    def _dump(self):
        if quickyaml:
            quickyaml.Dumper(
                width=self.max_yaml_width, array_handling=quickyaml.ARRAY_AS_SEQ
            ).dump(self._data, self._file)
        elif yaml:
            yaml.dump(
                self._data,
                self._file,
...
class YamlDatafileSink(AsciiScanfileSink):
    ...
    parameters = {
        "filenametemplate": Param(
            ...
            userparam=False,
            settable=False,
            default=["%(proposal)s_" "%(scancounter)08d.yaml"],
        ),
    }

    parameter_overrides = {
        "settypes": Override(default=[SCAN, SUBSCAN]),
        "commentchar": Override(
            default="",
        ),
    }
