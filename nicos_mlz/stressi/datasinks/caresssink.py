...
class CaressScanfileSinkHandler(DataSinkHandler):
    ...
    def _write_integer(self, value, key=None):
        if value is None:
            self.log.error(
                "write_integer: Found 'None' value for %s", key if key else ""
            )
            return
        if key:
            self._defcmd(key)
    ...
    def _write_float(self, value, key=None):
        if value is None:
            self.log.error("write_float: Found 'None' value for %s", key if key else "")
            return
        if key:
            self._defcmd(key)
            self._string(key)
        ...
        for device, key, value in valuelist:
            if key == "value":
                if device == "slits":
                    ...
                    d1["SLITM_U"] = value[0] / 2.0
                    d1["SLITM_D"] = -value[0] / 2.0
                    d1["SLITM_R"] = value[1] / 2.0
                    d1["SLITM_L"] = -value[1] / 2.0
                elif device == "slitp":
                    d1["SLITP_U"] = value[0] / 2.0
                    d1["SLITP_D"] = -value[0] / 2.0
                    d1["SLITP_R"] = value[1] / 2.0
                    d1["SLITP_L"] = -value[1] / 2.0
                elif device == "slite":
                    d1["SLITM_E"] = value
                elif device == "transm":
                    ...
                    d1[device] = session.getDevice(device)._attached_moveable.read()
                    self.log.debug("TRANSM value %f", d1[device])
                elif device == "wav":
                    d1[device] = value or 0.0
    ...
    def _write_defdata(self, dev, master):
        ...
        if self._scan_type == "SGEN2":
            ...
            for d in self.dataset.detvalueinfo:
                if d.name not in ["adet", "tim1", "mon"] and not d.name.endswith(
                    ".sum"
                ):
            ...
            for d in self.dataset.environment:
                if (
                    hasattr(d, "name")
                    and d.name != "etime"
                    and not d.name.endswith(":elapsedtime")
                ):
                    mastervalues += " %s" % d.name.upper()
            mastervalues += ")"
    ...
    def prepare(self):
        ...
        if self.dataset.settype == SCAN:
            _file = self.manager.createDataFile(self.dataset, self._template[0])
        ...
        if _file:
            self._file = _file
        self.log.debug(
            "%s %r %r", self.dataset.settype, self._scan_file, self._file.filepath
        )
    ...
    def putMetainfo(self, metainfo):
        ...
        for dev, key in metainfo:
            self.log.debug("put meta info: %s.%s = %r", dev, key, metainfo[dev, key][0])
    ...
    def _write_header(self, point):
        ...
        if self._scan_type != "SGEN2":
            ...
            for dev in point.environment:
                if (
                    hasattr(dev, "name")
                    and dev.name != "etime"
                    and not dev.name.endswith(":elapsedtime")
                ):
                    d[dev.name.upper()] = 0
        self._write_sl1(d)
        ...
        d.clear()
        # Get pixel size
        xpix, ypix = point.metainfo.get(("image", "pixel_size"), ((0.85, 0.85),))[0]
        d["TTHS"] = (256.0, 128.0, xpix)
        d["NYS"] = (256.0, 128.0, ypix)
        d["YSD"] = (point.metainfo.get(("ysd", "value"), [0])[0],)
        if self._detvalues is not None:
            if len(self._detvalues.shape) == 2:
                d["NYS"] = (
                    float(self._detvalues.shape[0]),
                    self._detvalues.shape[0] / 2,
                    xpix,
                )
                d["TTHS"] = (
                    float(self._detvalues.shape[1]),
                    self._detvalues.shape[1] / 2,
                    ypix,
                )
        ...
        if self._detvalues is not None:
            if len(self._detvalues.shape) == 2:
                d["W1"] = (1, self._detvalues.shape[0], 1, self._detvalues.shape[1])
            elif len(self._detvalues.shape) == 1:
        ...
        if self._scan_type != "SGEN2":
            for info, val in zip(point.envvalueinfo, point.envvaluelist):
                if (
                    hasattr(info, "name")
                    and info.name != "etime"
                    and not info.name.endswith(":elapsedtime")
                ):
                    if isinstance(val, numbers.Real):
                        self._write_float(val)
                    else:
                        self.log.info(
                            "Ignoring environment device '%s' "
                            "returning non-number value",
                            info.name,
                        )
        self._detvalues = None
...
class CaressScanfileSink(FileSink):
    ...
    parameters = {
        ...
        "mapping": Param(
            "Mapping of (string) device values to float values",
            unit="",
            settable=False,
            type=dictof(str, float),
            default={"on": 1, "off": 0, "open": 1, "closed": 0},
        ),
        "flipimage": Param(
            "Flipping image bottom-top before writing",
            type=bool,
            default=False,
            settable=False,
            userparam=False,
