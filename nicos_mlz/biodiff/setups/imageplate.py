...
devices = dict(
    TIFFFileSaver=device(
        "nicos.devices.datasinks.TIFFImageSink",
        description="Saves image data in TIFF format",
        filenametemplate=["%(proposal)s_%(pointcounter)08d.tiff"],
        mode="I;16",
    ),
    imgdrum=device(
        "nicos_mlz.biodiff.devices.detector.ImagePlateDrum",
        description="Image plate detector drum",
        tangodevice=_TANGO_DEV,
        comdelay=5.0,
    ),
    imgplate=device(
        "nicos_mlz.biodiff.devices.detector.ImagePlateImage",
