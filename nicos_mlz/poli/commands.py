...
@usercommand
@helparglist(
    "device1, device2, ..., [rounds=N], [steps=N], [steps_device1=N],"
    ...
    for i in range(nrounds):
        ...
        for dev in devices:
            ...
            if session.mode == SIMULATION:
                ...
                if (
                    math.isnan(fit_center)
                    or not (minvalue <= fit_center <= maxvalue)
                    or fit_ampl < 0.5 * fit_bkgd
                ):
                    maw(dev, center)
                    session.log.error(
                        "Gaussian peak too small, or center " "outside scanning area"
                    )
                    return
        ...
        for dev in devices:
            ...
            session.log.info(
                "%-10s center: %8.6g -> %8.6g (delta %8.6g)",
                dev,
                lastround[dev],
                thisround[dev],
                diff,
...
class QScan(Scan):
    ...
    def __init__(
        ...
        if not isinstance(inst, SXTalBase):
            raise NicosError(
                "cannot do a Q scan, your instrument device " "is not a sxtal device"
            )
        Scan.__init__(
            self,
            [inst],
        ...
        self._envlist[0:0] = [
            inst._attached_gamma,
            inst._attached_omega,
            inst._attached_nu,
        ]
...
def omscan(hkl, width=None, speed=None, timedelta=None, **kwds):
    ...
    om1 = calc["omega"] - width / 2.0
    om2 = calc["omega"] + width / 2.0
    cur_om = instr._attached_omega.read()
    if abs(cur_om - om1) > abs(cur_om - om2):
        om1, om2 = om2, om1
    maw(
        instr._attached_gamma,
        calc["gamma"],
        instr._attached_nu,
        calc["nu"],
        instr._attached_omega,
        om1,
    )
...
def PosListRemove(idx, listname="default"):
    ...
    if idx < 1 or idx > len(lists[listname]):
        ...
        return
    lists[listname] = [e for (i, e) in enumerate(lists[listname]) if i != idx - 1]
    sample.poslists = lists
    session.log.info("Position list %r is now:", listname)
    PosListShow(listname)
...
"""
    ...
    session.log.info(
        "Using (%.4g %.4g %.4g) and (%.4g %.4g %.4g) to calculate" " UB matrix:",
...
def AcceptIndexing():
    ...
    lists = dict(sample.poslists)
    if listname not in lists:
        session.log.warning("Position list %r does not exist anymore", listname)
        return
    posl = list(lists[listname])
    if len(posl) != len(peaks):
        session.log.warning("Position list %r changed after indexing", listname)
        return
    for i in range(len(posl)):
        posl[i] = (posl[i][0], [int(round(ix)) for ix in peaks[i]], posl[i][2])
    lists[listname] = posl
    sample.poslists = lists
    ...
    *hmax*, *kmax* and *lmax*.
    ...
    if kvector:
        try:
            ...
            raise UsageError(
                "kvector needs to be a HKL vector or a list " "of HKL vectors"
            ) from None
    ...
    for i, hkl in enumerate(hkls, 1):
        ...
        for devname, devvalue in poslist:
            if (
                not isfinite(devvalue)
                or not instr._adevs[devname].isAllowed(devvalue)[0]
            ):
                break
    ...
    session.log.info(
        "%d of %d reflections within instrument limits.", len(all_pos), len(hkls)
    )
...
def ScanDataset(name, speed=None, timedelta=None, start=1, cont=True):
    ...
    with open(fullpath, "r", encoding="utf-8") as fp:
        ...
        for row in reader:
            ...
            all_pos.append(
                ([float(h), float(k), float(l)], float(width.replace(",", ".")))
            )
    ...
    for i, (hkl, width) in enumerate(all_pos, start=start):
        ...
        try:
            ...
            if cont:
                ...
                cscan(
                    instr._attached_omega, calc["omega"], stepsize, nsteps, t=timedelta
                )
        finally:
...
def RefineMatrix(listname="default", **kwds):
    ...
    * ``delta_gamma``, ``delta_nu``: offsets for detector axes
    ...
    session.log.info(
        "Refining matrix with %d reflections from position " "list...", len(posl)
    )

    o = orient(sample.cell)
    new_cell, p = o.RefineOrientation(
        posl, kwds, init_lambda, ["gamma", "nu"], init_offsets
    )

    session.log.info("")
    session.log.info("Cell parameters:")
    session.log.info(
        "Initial:    a = %8.4f   b = %8.4f   c = %8.4f   "
        "α = %7.3f   β = %7.3f   γ = %7.3f",
        *sample.cell.cellparams(),
    )
    session.log.info(
        "Final:      a = %8.4f   b = %8.4f   c = %8.4f   "
        "α = %7.3f   β = %7.3f   γ = %7.3f",
        p.a,
        p.b,
        p.c,
    ...
    session.log.info("")
    session.log.info(
        "Initial:    λ = %8.4f   Δγ = %7.3f   Δν = %7.3f", init_lambda, *init_offsets
    )
    session.log.info(
        "Final:      λ = %8.4f   Δγ = %7.3f   Δν = %7.3f",
        p.wavelength,
        p.delta_gamma,
        p.delta_nu,
    )
    session.log.info(
        "Errors: +/-     %8.4f        %7.3f        %7.3f",
        p.errors["wavelength"],
        p.errors["delta_gamma"],
        p.errors["delta_nu"],
    )
    ...
    RefineMatrix._last_result = (new_cell, p.wavelength, p.delta_gamma, p.delta_nu)
