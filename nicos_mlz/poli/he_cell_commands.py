...
def AddHeaderToCellsFile(
    ...
    with open(cells_file(), "a", encoding="utf-8") as f:
        ...
        f.write(
            "# Analyser/decpol cell ({}) pressure = {} bar,  Polariser "
...
def AddPolariserHeaderToCellsFile(
...
def AddTransToCellsFile(analyser_cell_transmission, polariser_cell_transmission):
    ...
    with open(cells_file(), "a", encoding="utf-8") as f:
        ...
        f.write(
            "# Analyser/decpol cell: Transmission = {:.2f} %,  Polariser "
...
def AddPolariserTransToCellsFile(polariser_cell_transmission):
    ...
    with open(cells_file(), "a", encoding="utf-8") as f:
        f.write(SMALL_COMMENT)
        f.write(
            "# Polariser cell: Transmission = {:.2f} %\n".format(
...
def AddHeCells(
    ...
    pause(
        ...
        "Transmission = {:8.2f} %.\n\nNow, insert cell {} into polariser.".format(
            analyser_cell_name,
            analyser_cell_pressure,
    ...
    printinfo(SMALL_SEP)
    printinfo(
        "Measurement with 2 cells: {} [{} bar] in analyser/decpol and "
        "{} [{} bar] in polariser. {}".format(
            analyser_cell_name,
            analyser_cell_pressure,
            polariser_cell_name,
            polariser_cell_pressure,
    ...
    printinfo(
        "Analyser/decpol cell {}: pressure = {} bar, transmission = {:9.4f} %".format(
            analyser_cell_name, analyser_cell_pressure, analyser_cell_transmission
        )
    )
    printinfo(
        "Polariser cell {}: pressure = {} bar, transmission = {:9.4f} %".format(
            polariser_cell_name, polariser_cell_pressure, polariser_cell_transmission
        )
    )
    printinfo(LARGE_SEP)
...
def RemoveHeCells(h_index, k_index, l_index, count_time):
    ...
    printinfo(SMALL_SEP)
    printinfo(
        "Measurement with 2 cells: in analyser/decpol and polariser. {}".format(
            format_datetime()
        )
    )
    printinfo(SMALL_SEP)
    ...
    printinfo(SMALL_SEP)
    printinfo(
        "Measurement with 1 cell: in analyser/decpol. {}".format(format_datetime())
    )
    printinfo(SMALL_SEP)
    cells1 = count(det, count_time)
    AddCountsToCellsFile(True, False, cells1)
    ...
    pause(
        "Polariser cell:\n   Transmission = {:8.2f} %.\n\n"
        "Now, remove cell from analyser/decpol.".format(polariser_cell_transmission)
    )
    maw(gamma, gamma_hkl)
    printinfo(SMALL_SEP)
    printinfo("Measurement without cells. {}".format(format_datetime()))
    ...
    pause(
        "Cells are removed.\n\nPolariser cell:\n"
        "   Transmission = {:8.2f} %,\n\nAnalyser/decpol cell:\n"
        "   Transmission = {:8.2f} %".format(
            polariser_cell_transmission, analyser_cell_transmission
        )
    )
    printinfo(SMALL_SEP)
    printinfo("Cells are removed.")
    printinfo(
        "Polariser cell: transmission = {:9.4f} %".format(polariser_cell_transmission)
    )
    printinfo(
        "Analyser/decpol cell: transmission = {:9.4f} %".format(
            analyser_cell_transmission
        )
    )
    printinfo(LARGE_SEP)
...
def AddHeCellToPolariserHkl(
    h_index, k_index, l_index, count_time, polariser_cell_name, polariser_cell_pressure
):
    ...
    cells0 = count(det, count_time)
    wavelen = wavelength()
    AddPolariserHeaderToCellsFile(polariser_cell_name, polariser_cell_pressure, wavelen)
    AddCountsToCellsFile(False, False, cells0)
    maw(gamma, gamma_cell)
    pause(
        'Insert cell {} into polariser and press "Continue script" after that.'.format(
            polariser_cell_name
        )
    )
    maw(gamma, gamma_hkl)
    printinfo(SMALL_SEP)
    printinfo(
        "Measurement with 1 cell: {} [{} bar] in polariser. {}".format(
...
def RemoveHeCellFromPolariserHkl(h_index, k_index, l_index, count_time):
    ...
    gamma_cell = 60.0
    printinfo(SMALL_SEP)
    printinfo("Measurement with 1 cell: in polariser. {}".format(format_datetime()))
    printinfo(SMALL_SEP)
    cells1 = count(det, count_time)
    AddCountsToCellsFile(False, True, cells1)
    maw(gamma, gamma_cell)
