...
from numpy import arccos, arcsin, arctan2, cos, degrees, isnan, radians, sin, sqrt
...
def angles_to_currents(coeff, chi1, chi2, lam_in, lam_out, gamma):
    ...
    if coeff[4] != 0 or coeff[5] != 0:
        ...
        coeff2 = 0
    det = lam_in * coeff[0] * lam_out * coeff[3] - lam_out * coeff2 * lam_in * coeff[1]
    if det == 0:
...
def pol_in_from_angles(sense, theta_in, chi_in, lam_in, lam_out, gamma):
    ...
    x = -sense * cos(theta_in) * sin(chi_in) * sin(psi) + sense * cos(psi) * sin(
        theta_in
    )
    y = -sense * cos(theta_in) * sin(chi_in) * cos(psi) - sense * sin(psi) * sin(
        theta_in
    )
...
def pol_out_from_angles(sense, theta_out, chi_out, lam_in, lam_out, gamma):
    ...
    psi = _get_psi(sense, lam_in, lam_out, gamma)
    x = -sense * cos(theta_out) * sin(chi_out) * sin(gamma - psi) + sense * cos(
        gamma - psi
    ) * sin(theta_out)
    y = sense * cos(theta_out) * sin(chi_out) * cos(gamma - psi) + sense * sin(
        gamma - psi
    ) * sin(theta_out)
