...
class ZerosFit(PredefinedFit):
    ...
    def fit_model(self, x, pmax, theta0):
        ...
        return (
            -pmax
            * np.cos(np.radians(x - theta0))
            * np.sin(np.radians(x - theta0))
            * (1 + np.cos(gamma))
        )
...
class PCurrentFit(PredefinedFit):
    ...
    def fit_model(self, x, p0, pmax, theta0, period):
        ...
        return p0 + pmax * np.cos(np.radians(x - theta0) * period * lam) * np.exp(
            -((x / ExpDecrease) ** 2)
        )
...
def NutatorsZeros_In(start=45, stop=135, step=5, t=2):
    ...
    nsteps = int((stop - start) / step)
    scan(["nutator1", "nutator2"], [start, start - 90], [step, step], nsteps, adet, t=t)
    fit(ZerosFit, "Asym")
