...
class Controller(BaseChopperController):
    ...
    def _change(self, name, value):
        ...
        if self.ratio > 1:
            if self.ratio < 9:
                ...
                self._attached_discs[4].move(-self.speed * 7.0 / self.ratio)
        else:
    ...
    def _readspeeds_actual(self):
        return [abs(v) * 279.618375 for v in self._readspeeds()]
        ...
        for ch in range(2, 8):
            ...
            if phase_diff > self.phase_accuracy:
                if self.isTimedOut():
                    ...
                    if ch == 5:
                        msg = "cd 5 phase %.2f != %.2f" % (phase, self.phases[ch])
                        if phase_diff >= 360:
                    ...
                    stval = status.BUSY
                msg = "ch %d: phase %s != nominal %s" % (ch, phase, self.phases[ch])
                ret.append(msg)
                if self.isTimedOut():
                    stval = status.OK  # NOTREACHED
                    self.log.warning(msg)
