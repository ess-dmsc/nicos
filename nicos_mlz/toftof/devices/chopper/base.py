...
class BaseChopperController(HasTimeout, Readable):
    parameters = {
        "ch5_90deg_offset": Param(
            "Whether chopper 5 is mounted the right way "
            "(= 0) or with 90deg offset (= 1)",
            type=intrange(0, 1),
            default=0,
            category="general",
        ),
        "phase_accuracy": Param(
            "Required accuracy of the chopper phases",
            settable=True,
            default=10,
            type=float,
        ),
        "speed_accuracy": Param(
            "Required accuracy of the chopper speeds",
            settable=True,
            default=2,
            type=float,
        ),
        "resolution": Param(
            "Current energy resolution",
            volatile=True,
            type=tupleof(float, float),
        ),
        "wavelength": Param(
            "Selected wavelength", unit="AA", settable=True, type=float, default=4.5
        ),
        "speed": Param(
            "Disk speed", unit="rpm", userparam=False, type=int, default=6000
        ),
        "ratio": Param("Frame-overlap ratio", type=int, settable=True, default=1),
        "crc": Param("Counter-rotating mode", type=int, settable=True, default=1),
        "slittype": Param("Slit type", type=int, settable=True, default=1),
        "phases": Param(
            "Current phases", type=listof(float), internal=True, default=[0] * 8
        ),
        "changetime": Param(
            "Time of last change",
            userparam=False,
    ...
    def _getparams(self):
        return (self.wavelength, self.speed, self.ratio, self.crc, self.slittype)

    def _change(self, name, value):
        raise NotImplementedError(
            "please use a proper derived class and " 'implement "_change" method!'
        )
...
class SpeedReadout(Readable):
    ...
    def doRead(self, maxage=0):
        return [v / 279.618375 for v in self._attached_chopper._readspeeds_actual()]
