...
class Controller(BaseChopperController):
    ...
    def doInit(self, mode):
        ...
        try:
            ...
            for ch in range(2, 8):
                phases.append(int(round(self._read(PHASE_SET + ch * 100) / 466.0378)))
            ...
            self.log.warning(
                "could not read initial data from PMAC chopper " "controller", exc=1
            )
    ...
    def _change(self, name, value):
        ...
        rr = self.ratio + 1
        self._writevalues(
            1, 0, self.speed, phases[1], int(round(self.wavelength * 1000.0))
        )
        self._writevalues(2, r1, r2 * self.speed, phases[2], int(self.slittype + 1))
    ...
    def _readspeeds_actual(self):
        return [abs(self._read(CURRENT_SPEED + ch * 100)) for ch in range(1, 8)]
    ...
    def doStatus(self, maxage=0):
        ...
        for ch in range(2, 8):
            ...
            phase_diff = abs(phase - self.phases[ch] / 100.0)
                ...
                if self.isTimedOut():
                    ...
                    if ch == 5:
                        ...
                        if phase_diff >= 360:
                            ...
                            if (
                                phase_diff >= (360.0 - self.phase_accuracy)
                                or abs(phase_diff - 270) <= self.phase_accuracy
                            ):
                                self.log.warning(msg)
                                continue
                        stval = status.ERROR
