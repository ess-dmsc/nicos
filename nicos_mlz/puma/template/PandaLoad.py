...
def plot(*args, **kwargs):
    ...
    for i in myargs:
        if isinstance(i, NicosData):  # got a scan
            ...
            raise UsageError(
                "Unknown type of argument %r. I understand only scans and labelstrings!"
                % i
            )
    pars = kwargs.pop(
        "pars", None
    )  # Parameternames (temperatures/fields/etc) to include in labels
    ...
    if len(kwargs) != 0:
        raise UsageError(
            "too many keyword arguments given I don' understand:\n" ", ".join(
                kwargs.keys()
            )
        )
    # evaluate pars
    ...
    try:
        ...
        plt.title(
            ...
            + m.Exp_remark
            + " scans "
            + shortlist([s[0].number for s in scans])
        )
    except Exception:
...
def MakeMap(*args, **kwargs):
    ...
    contourlines = kwargs.pop(
        "clines", None
    )  # which countourlines to draw (or none for Auto)
    colorlevels = kwargs.pop(
        "clevels", None
    )  # which colorlevels to use (or None for Auto)
    lw = kwargs.pop(
        "cwidths", None
    )  # linewidths for colorlevels (or None for Auto or a single value for fixed value
    if len(kwargs) != 0:
        raise UsageError(
            "too many keyword arguments given I don' understand:\n" ", ".join(
                kwargs.keys()
    ...
    Cl = plt.contour(
        xi, yi, zi, contourlines, linewidths=lw, colors="k", zmin=clim[0], zmax=clim[1]
    )
    Cl.set_clim(clim)
    Cl.zmin = clim[0]
    Cl.zmax = clim[1]
...
class Mapping:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.xcol = kwargs.pop(
            "xcol", scans[0].colnames[0]
        )  # defaults to first scanarg
        self.ycol = kwargs.pop(
            "ycol", scans[0].colnames[1]
        )  # defaults to second scanarg
        ...
        try:
            self.title = (
                scans[0].Exp_proposal
                + " "
                + scans[0].Sample_samplename
                + " "
                + scans[0].Exp_title[:22]
                + " "
                + scans[0].Exp_remark
                + " scans "
                + shortlist([s.number for s in scans])
...
def NicosLoad(prefix, filenum=-1, **kwargs):
    ...
    valid_units = (
        "T K meV A A-1 THz mm deg % m mK s min cts rlu r.l.u. bar mbar".split()
    )

    def parse_vu(
        vu,
    ):  # take a string and try to separate value and unit and return both (or empty strings)
        if vu.find(" ") == -1:
    ...
    def process_header_nicos2(
        data, line, _re_header=re.compile(r"^#?\s*([^:]*?)\s*:\s*(.*?)\s*$")
    ):
    ...
    def process_tableheader_nicos2(data, line):
        if data.get("colnames"):  # have names already, read units:
            ...
            # read colnames
            data.colnames = (
                line[1:].replace(":", "_").split()
            )  # translate : to _ first and then split into columnames
            ...
            for i in range(detcolstart, detcolend - 1):  # only check detector columns
                ...
                while n in data.colnames[i + 1 :]:
                    j = data.colnames.index(
                        n, i + 1
                    )  # index of duplicate, which must be > = i !!!
    ...
    def process_table_nicos2(data, line):
        ...
        if len(c) != len(data.colnames):
            print(
                "Data block: Line too short! has %d columns, but should have %d!"
                % (len(c), len(data.colnames))
            )
            print("Offending line was: %r" % line)
        ...
        for i in range(len(c)):  # iterate over all columns
            ...
            if (
                data.colnames[i] in data.colnames[i + 1 :]
            ):  # there will be another column with same name, forget this value!
                continue
            data[data.colnames[i]].append(
                pythify(c[i])
            )  # append value to correct column
        return True
    ...
    ) as f:  # open file and ignore different line-endings
    ...
    if len(kwargs) > 0:  # try to do normalisation
        for k, v in kwargs.items():  # ignore other kwargs....
            if k in data.detcols:
                if normcol != None:
                    print(
                        "Can not normalize to more than one detector column! Found %s+%s at least!"
                        % (normcol, k)
                    )
                ...
                while line != "":
                    ...
                    for k, v in mapping.items():
                        if line.startswith(k):
                            line = v + line[len(k) :]
                    if line.startswith(
                        "counting for switching devices"
                    ):  # polarized measurements!
                        self.pol_devices = [
                            d.strip()
                            for d in line.split("]")[0].split("[")[1].split(", ")
                        ]
                        self.pol_states = []
                        self.polarized = True
                    ...
                    if line.startswith(
                        "scan data:"
                    ):  # header finished, now parse data section
                        ...
                        for i in f.readline().strip().split():
                            ...
                            if (
                                self.polarized
                            ):  # mangle column names for detx, monx+time
                                if (
                                    i.startswith("det")
                                    or i.startswith("mon")
                                    or i.startswith("time")
                                ):
                                    if i.startswith("time"):
                                        polstate += 1  # time is always first det column
                        ...
                        for i in range(self.columns):
                            if self.colnames[i] != ";":
                                self.__dict__["%s_unit" % self.colnames[i]] = (
                                    self.colunits[i]
                                )
                        while line != "":
                            line = f.readline()
                            ...
                            if line.startswith("***"):
                                ...
                                if len(line) != self.columns:
                                    ...
                                    for i in range(self.columns):
                                        l = self.colnames[i]
                                        if l != ";" and not isinstance(
                                            self.__dict__[l], list
                                        ):
                                            self.__dict__[
                                                l
                                            ] = []  # on first line we prepare the element to hold the data
                                        try:
                                            v = line[i]
                                            v = float(
                                                v
                                            )  # try to convert to float, ignore failure to do so
                                        except Exception:
                                            pass
                    ...
                    try:  # try to parse header lines
                        ...
                        if v.strip() == v and len(v.split()) == 1:
                            ...
                            for i in range(4):
                                self.__dict__[
                                    "%s_%s" % (k, ("left", "right", "bottom", "top")[i])
                                ] = float(v.split()[i])
                                self.__dict__[
                                    "%s_%s_unit"
                                    % (k, ("left", "right", "bottom", "top")[i])
                                ] = v.split()[4]
                            ...
                            for i in range(3):
                                self.__dict__[(k.split()[0].split(", "))[i]] = float(
                                    v.split(", ")[i]
                                )
                                self.__dict__[
                                    "%s_unit" % (k.split()[0].split(", "))[i]
                                ] = k.split()[1][1:-1]
                            # ~ print("LATTICE", k, v)
                        elif k == "opmode":
...
if __name__ == "__main__":
    ...
    plt.plot(
        s1.measE, s1.det2, "o-r", s2.measE, s2.det2, "x:", s3.measE, s3.det2, "bo-"
    )
    plt.show()
