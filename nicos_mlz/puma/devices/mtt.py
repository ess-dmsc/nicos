...
class MttAxis(Axis):
    ...
    parameters = {
        ...
        "polysleep": Param(
            "Sleep time to change the polyethylen shielding " "block",
            default=5,
            unit="s",
            settable=False,
        ),
        "blocksleep": Param(
            "Sleep time to change a mobile block", default=20, unit="s", settable=False
        ),
    }
    ...
    def doStart(self, target):
        ...
        if not self._posthread:
            self._posthread = createThread(
                "positioning thread %s" % self, self._positioningThread
            )
    ...
    def _switchpoly(self):
        ...
        if self._stoprequest == 2 or self._errorstate:
            self._poly = 0
        if self._poly != 0 and abs(self.read(0) - self.offset - self.polypos) <= 0.1:
        ...
        if self._attached_io_flag.read(0) == 1:
            ...
            while self._attached_io_flag.read(0) == 1:
                ...
                if t < 0:
                    ...
                    session.notify(
                        "PUMA: %s" % self.name, "mobil block probably got stuck"
                    )
                    break
