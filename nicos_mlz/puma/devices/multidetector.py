...
class MultiDetectorLayout(CanReference, HasTimeout, BaseSequencer):
    ...
    attached_devices = {
        "rotdetector": Attach(
            "Detector tube position devices", Moveable, multiple=_num_axes
        ),
        "rotguide": Attach("Detector guide devices", Moveable, multiple=_num_axes),
    ...
    parameters = {
        "general_reset": Param("", type=bool, settable=False, default=False),
        "raildistance": Param("", type=float, settable=False, default=20.0, unit="mm"),
        "detectorradius": Param(
            "", type=float, settable=False, default=761.9, unit="mm"
        ),
        "refgap": Param(
            "Gap between detectors during the reference of the " "guides",
        ...
        "gapoffset": Param(
            "Minimum gap for the det 1 from reference position",
            type=float,
            settable=False,
            userparam=False,
            default=4.0,
        ...
        "tubediameter": Param(
            "diameter of detector tubes",
            type=floatrange(0, None),
            unit="mm",
            category="general",
            default=25.4,
        ),
        "psdchannelwidth": Param(
            "PSD channel width",
            type=floatrange(0, None),
            category="general",
            unit="mm",
    ...
    parameter_overrides = {
        "timeout": Override(default=600.0),
    ...
    def park(self, blocking=True):
        ...
        if self._seq_is_running():
            if self._mode == SIMULATION:
                ...
                raise MoveError(
                    self,
                    "Cannot park device, sequence is still "
        ...
        self._startSequence(
            ...
            + [SeqDev(d, p) for d, p in zip(self._rotguide1, self.parkpos[11:][::-1])]
        )
        if blocking:
    ...
    def doIsAllowed(self, target):
        ...
        for dev, pos in zip(self._rotdetector0 + self._rotguide0, target):
            ...
            if ok:
                self.log.debug("%s: requested position %.3f deg allowed", dev, pos)
            else:
                why.append(
    ...
    def _clear_guide_reference(self, guide):
        ...
        while True:
            ...
            if free is None:
                self.log.warning(
                    "Can't free the guide: %s. Please check " "manually why!", guide
                )
    ...
    def _printPos(self):
        ...
        for i, dev in enumerate(self._attached_rotdetector):
            out.append("detector rotation %2d: %7.2f %s" % (i, dev.read(), dev.unit))
        for i, dev in enumerate(self._attached_rotguide):
            out.append("guide rotation    %2d: %7.2f %s" % (i, dev.read(), dev.unit))
        self.log.debug("%s", "\n".join(out))
    ...
    def _checkPositionReached(self, target, mode):
        ...
        self.log.debug("length of list: %d", 0 if target is None else len(target))
        if not target:
            return False
        ...
        if mode == "raw":
            ...
            self.log.warning(
                "not a valid mode given; corrected values or raw " "values?"
            )
            return False
    ...
    def _readAnaTranslation(self):
        ...
        anatranslist1 = list(range(-125, -5, 20)) + [0] + list(range(25, 126, 20))
    ...
    def _sequentialAngleLimit(self, pos):
        ...
        for i in range(self._num_axes):
            ...
            if i == 0:
                ...
                if (
                    ...
                    or abs(dtarget[i] - dtarget[i - 1]) < self.anglis[i]
                ):
                    why.append(
                        "%s %s %s %s"
                        % (dtarget[i - 1], dtarget[i], dtarget[i + 1], self.anglis[i])
                    )
                    allowed.discard(i)
        self.log.debug(
            "movement allowed for the following detectors: %s",
            ", ".join([str(i) for i in allowed]),
        )
        notallowed = check - allowed
        if notallowed:
            self.log.warning(
                "movement not allowed for the following " "detectors: %s",
        ...
        if self.opmode == "multi" and not is_sorted(gtarget):
            return (
                False,
                "detector guide targets not a list of consecutive "
                "values %s" % (gtarget,),
            )
        first = 0
        ...
        if not (rg1_min <= gtarget[first] <= gtarget[first + 1]):
            why.append("rg1: %s %s %s" % (rg1_min, gtarget[first], gtarget[first + 1]))
            allowed.discard(first + 1)
        if not (gtarget[last - 1] <= gtarget[last] <= rg11_max):
            why.append("rg11: %s %s %s" % (gtarget[last - 1], gtarget[last], rg11_max))
        ...
        if notallowed:
            self.log.warning(
                "movement not allowed for the following guides: " "%s",
                ", ".join([str(i) for i in notallowed]),
            )
            return False, why
        return True, ""
