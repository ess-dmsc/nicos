...
class PlotWidget(QWidget):
    def __init__(self, title, xlabel, ylabel, name="unknown", parent=None, **kwds):
        QWidget.__init__(self, parent)
        self.name = name
        parent.setLayout(QVBoxLayout())
        self.plot = MiniPlot(
            xlabel, ylabel, self, color1=COLOR_BLACK, color2=COLOR_RED, **kwds
        )
...
class TransmissionPlot(PlotWidget):
    def __init__(self, direction, parent=None, **kwds):
        PlotWidget.__init__(
            self,
            "Calculated refl/trans coeff of deflectors "
            "for spin-%s neutrons" % direction,
            "Deflector angle (deg)",
            "Refl/trans coefficient",
            parent=parent,
            **kwds,
        )
...
class PolarisationPanel(NicosWidget, Panel):
    ...
    def _optimize(self):
        ...
        L = LAD * sin(-delta5 + kappa5) / sin(-beta5 + kappa5) - 0.5 * BA * cos(theta0)
        deps = 0.5 * (-BD - BA * sin(theta0)) / L
        deps = atan(deps)
        eps5 += abs(deps)

        L = LAD * sin(-delta7 + kappa7) / sin(-beta7 + kappa7) - 0.5 * BA * cos(theta0)
        deps = 0.5 * (-BD - BA * sin(theta0)) / L
        deps = atan(deps)
        eps7 += abs(deps)
    ...
    def _simulate(self):
        ...
        px = arange(-7, 7.01, pa.psdwidth / 10.0)
        self.plot1.setData(px, pa.psdup[1:], pa.psdupa[1:])
        self.plot2.setData(px, pa.psddown[1:], pa.psddowna[1:])

        ang = arange(-2, 2.01, 0.1)
        self.plot3.setData(ang, pa.reflup, (1.0 - array(pa.reflup)).tolist())
        self.plot4.setData(ang, pa.refldown, (1.0 - array(pa.refldown)).tolist())
