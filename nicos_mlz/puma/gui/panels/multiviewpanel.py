...
from nicos_mlz.puma.guisupport.elements import (
    CrystalTable,
    DetectorBox,
    DetectorGuide,
    DetectorTable,
...
class MultiAnalyzerView(QGraphicsView):
    ...
    def __init__(self):
        ...
        for i in range(self.num_crystals):
            self._detectors[i] = DetectorTable(
                self.detsize, self.detradius, scene=scene
            )
            self._guides[i] = DetectorGuide(self._detectors[i])

            self._detectors_t[i] = t = TableTarget(0, 0, self.detsize, scene=scene)
            ...
            self._crystals[i] = CrystalTable(0, y, self.crystalsize, scene=scene)
            self._crystals_t[i] = TableTarget(0, y, self.crystalsize, scene=scene)
            self._reflections[i] = Reflex(
                self.detradius,
                self._crystals[i],
                self._sample,
                self._detectors[i],
            ...
            self._inbeams[i] = Beam(self._sample, self._crystals[i], scene=scene)
            self._outbeams[i] = Beam(
                self._crystals[i], self._reflections[i], scene=scene
            )
        self.update()
    ...
    def update(self):
        ...
        for i in range(self.num_crystals):
            self._detectors[i].setState(
                max(self.status["rd%d" % (i + 1)], self.status["rg%d" % (i + 1)])
            )
            self._detectors[i].setRotation(-(self.values["rd%d" % (i + 1)] - cadrot))
            ...
            self._crystals[i].setState(
                max(self.status["ta%d" % (i + 1)], self.status["ra%d" % (i + 1)])
            )
    ...
    def _calculate_reflection(self, i=0):
        ...
        d = self._detectors[i]
        inangle = degrees(
            atan((c.y() - self._sample.y()) / abs(self._sample.x() - c.x()))
        )
