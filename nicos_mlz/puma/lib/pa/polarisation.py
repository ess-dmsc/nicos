...
class PA:
    ...
    lsa = 100.0  # distance sample-analyzer axes / cm
    lsd1 = 20.0  # sample-deflector 1 / cm
    lsd2 = 25.0  # distance sample-deflector 2 / cm
    lpsd = 150.0  # distance sample - PSD / cm
    ...
    y05 = -7.0  # longitudinal displacement of analyzer 5 / cm
    y06 = 0.0  # longitudinal displacement of analyzer 6 / cm
    y07 = 8.4  # longitudinal displacement of analyzer 7 / cm
    x05 = 2.0  # transverse displacement of analyzer 5 / cm
    x06 = 0.0  # transverse displacement of analyzer 6 / cm
    x07 = -2.0  # transverse displacement of analyzer 7 / cm
    bS = 0.2  # width of sample slit / cm
    bA = 2.5  # width of analyzer blade / cm
    L = 4.0  # length of deflector / cmalpha
    ...
    def run(self):
        ...
        i7down = 0.0
        w_c = [
            gaussian(amp, xc, self.alpha0, -_range / 2.0 + j * _range / (nalpha - 1))
        ...
        for i in range(nxp):
            ...
            for j in range(nalpha):
                alpha = -_range / 2.0 + j * _range / (nalpha - 1)
                ...
                r1up = rdefl(self.gamma1 - alpha, 1) - ddefl(
                    self.gamma1, alpha, self.L, self.d, 1
                )
                r1down = rdefl(self.gamma1 - alpha, 2) - ddefl(
                    self.gamma1, alpha, self.L, self.d, 2
                )
                ...
                xpsd1 = xA(alpha, self.gamma1, self.lpsd, self.lsd1, 0.0, xp, 1)
                ...
                # reflectivity for neutrons with divergence angle alpha
                r2up = rdefl(self.gamma2 - alpha, 1) - ddefl(
                    self.gamma2, alpha, self.L, self.d, 1
                )
                r2down = rdefl(self.gamma2 - alpha, 2) - ddefl(
                    self.gamma2, alpha, self.L, self.d, 2
                )
                ...
                # position of reflected beam on PSD
                xpsd2 = xA(alpha, self.gamma2, self.lpsd, self.lsd2, 0.0, xp, 2)
                ...
                self.psdup[npsd1 - 1] += w1up * (1.0 - f1)
                self.psddown[npsd1] += w1down * f1
                self.psddown[npsd1 - 1] += w1down * (1.0 - f1)
                self.psdup[npsd2] += w2up * f2
                self.psdup[npsd2 - 1] += w2up * (1.0 - f2)
                self.psddown[npsd2] += w2down * f2
                self.psddown[npsd2 - 1] += w2down * (1.0 - f2)
                self.psdup[npsd0] += w0up * f0
                self.psdup[npsd0 - 1] += w0up * (1.0 - f0)
                self.psddown[npsd0] += w0down * f0
                self.psddown[npsd0 - 1] += w0down * (1.0 - f0)
                ...
                self.psdupa[npsd1] += w1up * (1 - w51) * (1.0 - w61) * (1.0 - w71)
                self.psddowna[npsd1] += w1down * (1 - w51) * (1.0 - w61) * (1.0 - w71)
                self.psdupa[npsd2] += w2up * (1 - w52) * (1.0 - w62) * (1.0 - w72)
                self.psddowna[npsd2] += w2down * (1 - w52) * (1.0 - w62) * (1.0 - w72)
                self.psdupa[npsd0] += w0up * (1 - w50) * (1.0 - w60) * (1.0 - w70)
                self.psddowna[npsd0] += w0down * (1 - w50) * (1.0 - w60) * (1.0 - w70)
        ...
        self.fractionup = 1.0 - self.psdintupa / self.psdintup
        self.fractiondown = 1.0 - self.psdintdowna / self.psdintdown
...
if __name__ == "__main__":
    ...
    def write(pa):
        with open("PolData_Out.txt", "w", encoding="utf-8") as f:
            ...
            f.write(
                "tilt-angles analyzers 5,6,7          : %8.2f deg%8.2f deg"
                "%8.2f deg\n" % (pa.theta05, pa.theta06, pa.theta07)
            )
            f.write(
                "lateral analyzer- displacements      : %5.1f cm%5.1f cm"
                "%5.1f cm\n" % (pa.x05, pa.x06, pa.x07)
            )
            f.write(
                "longitudinal analyzer- displacements : %5.1f cm%5.1f cm"
            ...
            f.write(
                "intensities channels 5,6,7, total : %10.2f%10.2f%10.2f"
                "%10.2f\n" % (pa.i5up, pa.i6up, pa.i7up, pa.igesup)
            )
            ...
            f.write(
                "intensities channels 5,6,7, total : %10.2f%10.2f%10.2f"
                "%10.2f\n" % (pa.i5down, pa.i6down, pa.i7down, pa.igesdown)
            )
            f.write("intensity ratio: %5.2f\n" % pa.ratiodown)
