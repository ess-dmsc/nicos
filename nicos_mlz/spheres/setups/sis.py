...
devices = dict(
    sis=device(
        "nicos_mlz.spheres.devices.sisdetector.SISDetector",
    ...
    sisimg=device(
        "nicos_mlz.spheres.devices.sisdetector.SISChannel",
    ...
    sisasink=device(
        "nicos_mlz.spheres.devices.sissinks.AFileSink",
        description="DataSink which writes raw data",
        detectors=["sis"],
        subdir="raw",
        filenametemplate=[
            basename + "%(scancounter)da%(pointnumber)d",
            basename + "a%(pointpropcounter)",
        ],
    ),
    sisusink=device(
        "nicos_mlz.spheres.devices.sissinks.UFileSink",
        description="DataSink which writes user data",
        detectors=["sis"],
        subdir="user",
        filenametemplate=[
            basename + "%(scancounter)du%(pointnumber)d",
    ...
    sislive=device(
        "nicos_mlz.spheres.devices.sissinks.PreviewSink",
        description="Sends image data to LiveViewWidget",
        detectors=["sis"],
    ),
    flux=device(
        "nicos_mlz.spheres.devices.flux.Flux",
