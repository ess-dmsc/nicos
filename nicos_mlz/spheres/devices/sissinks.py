...
class SisYamlFileSinkHandlerBase(YAMLBaseFileSinkHandler):
    ...
    def writeData(self, fp, image):
        ...
        quickyaml.Dumper(
            width=self.max_yaml_width, indent=2, array_handling=self.yaml_array_handling
        ).dump(self.contents, fp)
        ...
        if (
            self.params["reason"] == INTERMEDIATE
...
class UYamlFileSinkHandler(SisYamlFileSinkHandlerBase):
    ...
    def getLongpar(self, start, end):
        ...
        o["env.sam.tmp"] = self.getDevicePars(start, end, self.sink.envcontroller)
        o["env.setp"] = self.getDevicePars(start, end, self.sink.setpointdev)
    ...
    def getDevicePars(self, start, end, dev):
        ...
        o["nobs"] = max(len(self._cache.history(dev, "value", start, end)) - 1, 1)
        o["avge"] = values[0]
    ...
    def getHistogram(self, image):
        ...
        if self.inelasticmode:
            ...
            for histo in rawHistos:
                ...
                row.append(
                    int(
                        sum(histo.t_open_l + histo.t_open_r)
                        / ((len(histo.t_open_l) + len(histo.t_open_r)) / 2)
                    )
                )
                ...
                row.append(
                    int(
                        sum(histo.t_refl_r + histo.t_refl_l)
                        / ((len(histo.t_refl_r) + len(histo.t_refl_l)) / 2)
                    )
                )
                # counts (16)
                row += [
                    int(histo.c_refl_r[x] + histo.c_refl_l[x])
            ...
            for histo in rawHistos:
                ...
                row.append(
                    self._cache.get(
                        self.sink.envcontroller, "value", mintime=histo.tval
                    )
                )
                # env.setp (1)
                row.append(
                    self._cache.get(
                        self.sink.envcontroller, "setpoint", mintime=histo.tval
                    )
                )
                # direct timesteps (1)
...
class SisSink(ImageSink):
    parameters = {"filename": Param("Latest filename", type=str)}
...
class UFileSink(SisSink):
    ...
    parameters = {
        "envcontroller": Param("Environment temperature controller", type=str),
        "setpointdev": Param("Device for the setpoint", type=str),
    }
