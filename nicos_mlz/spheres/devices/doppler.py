...
class Doppler(SequencerMixin, MultiSwitcher):
    ...
    parameters = {
        "margins": Param(
            "margin for readout errors in refdevices",
            dictwith(speed=float, amplitude=float),
            default=dict(speed=0.0, amplitude=0.0),
            settable=True,
        ),
        "maxacqdelay": Param(
        ...
        "customrange": Param(
            "min and max for custom values", tuple, settable=False, volatile=True
        ),
    }

    attached_devices = {
        "switch": Attach("The on/off switch of the doppler", NamedDigitalOutput),
    ...
    def doInit(self, mode):
        ...
        self._sm_values = sorted(
            [x for x in self.mapping if isinstance(x, number_types)]
        )[1:]

        named_vals = {k: v[0] for k, v in self.mapping.items()}

        self.valuetype = oneofdict_or(named_vals, floatrange(0, self._sm_values[-1]))
    ...
    def doStart(self, target):
        ...
        if target not in self.mapping and not self.inRange(target):
            raise UsageError(
                "Values have to be within %.2f and %.2f."
        ...
        if target not in self.mapping:
            self.log.warning(
                "Moving doppler to a speed which is not in the "
    ...
    def _startRaw(self, target):
        if (
            ...
            raise UsageError(
                "Doppler speed can not be changed when script is " "paused."
            )
    ...
    def waitForAcq(self, retries):
        ...
        while retries:
            ...
            if not self.acqIsCounting():
                return True
        self.log.warning("Doppler speed can not be changed while " "SIS is counting.")
        return False
    ...
    def waitForSync(self, target):
        if session.mode == SIMULATION:
            ...
            while True:
                ...
                if self.withinMargins(acq_speed, speed, SPEED) and self.withinMargins(
                    acq_ampl, ampl, AMPLITUDE
                ):
                    return
                session.delay(1)
    ...
    def doStatus(self, maxage=0):
        ...
        if self._attached_switch.read() == "off":
            if not self.withinMargins(acq_speed, 0, SPEED):
                return (
                    status.WARN,
                    "detector registers movement of the "
                    "doppler, although it has been stopped.",
                )
        elif not self.withinMargins(acq_speed, speed, SPEED):
            return (
                status.WARN,
                "detector observes a speed differing " "from the dopplerspeed",
            )
        elif not self.withinMargins(acq_ampl, ampl, AMPLITUDE):
            return (
                status.WARN,
                "detector observes an amplitude differing " "from the doppleramplitude",
            )
        elif round(speed, 2) not in self._sm_values and self.inRange(speed):
