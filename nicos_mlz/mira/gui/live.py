...
class LiveDataPanel(Panel):
        ...
        if not self._no_direct_display and data:
            ...
            if self._format == "pad":
                ...
                self.statusBar.showMessage(
                    "cps: %.2f | total: %s" % (cts / runtime, cts)
                )
            else:
    ...
    def on_actionLoadTOF_triggered(self):
        filename = QFileDialog.getOpenFileName(
            self, "Open TOF File", "", "TOF File (*.tof *.TOF);;All files (*)"
        )[0]
        if filename:
    ...
    def on_actionLoadPAD_triggered(self):
        filename = QFileDialog.getOpenFileName(
            self, "Open PAD File", "", "PAD File (*.pad *.PAD);;All files (*)"
        )[0]
        if filename:
    ...
    def add_to_flist(self, filename, fformat, scroll=True):
        ...
        if (
            self.fileList.count() > 2
            and self.fileList.item(self.fileList.count() - 2).text() == shortname
        ):
            return
    ...
    def on_actionSetAsROI_triggered(self):
        ...
        self.client.run(
            "psd_channel.roi = (%s, %s, %s, %s)"
            % (int(zoom.left()), int(zoom.top()), int(zoom.right()), int(zoom.bottom()))
        )
    ...
    def on_actionPrint_triggered(self):
        ...
        self.statusBar.showMessage(
            "Plot successfully printed to %s." % str(printer.printerName())
        )
    ...
    def on_rangeChanged(self, val):
        if self._range_active:
            self.widget.SetCountRange(self.rangeFrom.value(), self.rangeTo.value())
