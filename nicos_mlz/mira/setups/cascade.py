...
sysconfig = dict(
    datasinks=["psd_padformat", "psd_tofformat", "psd_xmlformat", "psd_liveview"],
)
...
devices = dict(
    fg_burst=device(
        "nicos_mlz.devices.io_trigger.Trigger",
    ...
    psd_padformat=device(
        "nicos.devices.vendor.cascade.CascadePadSink",
        subdir="cascade",
        detectors=["psd", "counter", "timer", "monitor1", "monitor2"],
    ),
    psd_tofformat=device(
        "nicos.devices.vendor.cascade.CascadeTofSink",
        subdir="cascade",
        detectors=["psd", "counter", "timer", "monitor1", "monitor2"],
    ),
    psd_liveview=device(
    ...
    psd_xmlformat=device(
        "nicos_mlz.mira.devices.cascade.XmlSink",
        subdir="cascade",
        timer="timer",
        monitor="mon2",
        sampledet="sampledet",
    ...
    psd_channel=device(
        "nicos.devices.vendor.cascade.CascadeDetector",
    ...
    PSDHV=device(
        "nicos_mlz.mira.devices.iseg.CascadeIsegHV",
