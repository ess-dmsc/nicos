...
class BeamStopAxis(Axis):
    ...
    parameter_overrides = {
        ...
        "maxtries": Override(default=1000, settable=False, type=intrange(1, 1000)),
    }
    ...
    *sigh*.
    ...
    def isCompleted(self):
        if NicosSeqDev.isCompleted(self):
            ...
            return (
                NicosSeqDev.isCompleted(self) and self.dev.status(0)[0] != status.BUSY
            )
...
class BeamStop(SequencerMixin, Moveable):
    ...
    parameters = {
        "slots": Param(
            "Mapping of shape to HW-X-value and sizes",
            userparam=False,
            type=dictof(str, tupleof(float, tupleof(floatrange(0), floatrange(0)))),
            mandatory=True,
        ),
        "ypassage": Param(
            "HW-Y-value of the passage below the shapes",
            type=float,
            mandatory=True,
            userparam=False,
        ),
        ...
        "offsetxy": Param(
            "Offset of our logical coordinates to the HW coords",
            type=tupleof(float, float),
            default=(0.0, 0.0),
            settable=True,
        ),
        "xlimits": Param(
            "HW-limits for free movement of X",
            type=limits,
            mandatory=True,
            userparam=False,
        ),
        "ylimits": Param(
            "HW-limits for free movement of Y",
            type=limits,
            mandatory=True,
            userparam=False,
        ),
    ...
    def doIsAllowed(self, target):
        ...
        if not (xlimits[0] <= target[0] <= xlimits[1]):
            return (
                False,
                "requested X-position %s outside allowed value, "
        ...
        if not (ylimits[0] <= target[1] <= ylimits[1]):
            return (
                False,
                "requested Y-position %s outside allowed value, "
                "please move it manually into %s..%s"
    ...
    def doRead(self, maxage=0):
        return (
            self._attached_xaxis.read(maxage) - self.offsetxy[0],
    ...
    def doWriteShape(self, target):
        ...
        if self.shape not in self.slots:
            raise UsageError(
                "currently used shape unknown, " "(Call instrument scientist!)"
            )
    ...
    def _generateSequence(self, target):
        ...
        if startpos[1] < lowest_y_pos:
            raise UsageError(
                "illegal start position for changing shape, "
    ...
    def _retryFailed(self, step, action, nretries, exc_info):
        ...
        self._setROParam(
            "fixed", "Error during sequence (step %r), needs " "manual fixing!" % step
        )
