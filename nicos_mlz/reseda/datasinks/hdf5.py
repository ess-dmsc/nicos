...
class HDF5DataFile(DataFileBase, h5py.File):
    ...
    def createScalarsDataset(self, number, data):
        ...
        return self._current_group.create_dataset(self.scalars_tmpl % subst, data=data)

    def createImageDataset(self, number, data):
        subst = {"pointcounter": number}

        return self._current_group.create_dataset(self.image_tmpl % subst, data=data)
...
class HDF5SinkHandler(DataSinkHandler):
        ...
        if data[0]:
            ...
            for i, info in enumerate(detector.valueInfo()):
                for attr in ["name", "type", "unit"]:
                    hdf5dataset.attrs["value_info_%d/%s" % (i, attr)] = getattr(
                        info, attr
                    )
    ...
    def _addMetadata(self, metadata, hdf5dataset):
        ...
        for (dev, param), (_, strvalue, unit, _) in metadata.items():
            hdf5dataset.attrs["%s/%s" % (dev, param)] = toAscii(
                "%s %s" % (strvalue, unit)
            ).strip()
...
class HDF5Sink(FileSink):
    parameters = {
        "subscangrouptmpl": Param(
            "Name of the sub scan specific group",
            type=str,
            default="echo_%(subscancounter)d",
        ),
        "scalarstmpl": Param(
            "Name of the scalars dataset", type=str, default="scalars_%(pointcounter)d"
        ),
        "imagetmpl": Param(
            "Name of the image dataset", type=str, default="image_%(pointcounter)d"
        ),
    }
