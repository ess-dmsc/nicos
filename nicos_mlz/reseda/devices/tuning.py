...
class EchoTime(Moveable):
    ...
    attached_devices = {
        ...
        "dependencies": Attach("Echo time dependent devices", Readable, multiple=True),
    }
    ...
    parameters = {
        ...
        "stopfirst": Param(
            "list of Devices to stop before setting a new " "echotime",
            type=listof(str),
            settable=True,
            userparam=False,
            default=[],
        ...
        "currenttable": Param(
            "Currently used tune wave tables",
        ...
        "availtables": Param(
            "Available tunewave tables",
            type=dictof(str, listof(float)),
            settable=False,
            internal=True,
            volatile=True,
        ),
        "wavelengthtolerance": Param(
            "Wavelength tolerance for table" "determination",
            type=float,
            settable=True,
            default=0.1,
        ),
    ...
    def doPreinit(self, mode):
        ...
        self._tunedevs = {entry.name: entry for entry in self._attached_dependencies}

    def doRead(self, maxage=0):
        # read all tuning devices
        devs = {key: value.read(maxage) for key, value in self._tunedevs.items()}
        ...
        for echotime, tunedevs in self.currenttable.items():
            ...
            for tunedev, value in tunedevs.items():
                ...
                if not self._fuzzy_match(value, devs.get(tunedev, None), prec):
                    self.log.debug(
                        "-> no, because %s is at %s not %s " "(prec = %s)",
                        tunedev,
                        devs.get(tunedev, None),
                        value,
                        prec,
                    )
                    success = False
    ...
    def doStart(self, target):
        ...
        for tunedev, val in sorted(self.currenttable[value].items()):
            ...
            if tunedev in self._tunedevs:
                ...
                self.log.warning(
                    "tune device %r from table not in tunedevs! "
