...
class ScanningDetector(NicosScanDet):
    ...
    parameters = {
        ...
        "echostep": Param(
            "Current difference between points",
            type=floatrange(0),
            default=0.1,
            settable=True,
            userparam=True,
        ...
        "echostart": Param(
            "starting current value for the phase coil",
            type=float,
            default=0,
            userparam=True,
            settable=True,
    ...
    def doReadPositions(self):
        return self._calc_currents(self.echopoints, self.echostep, self.echostart)

    def _calc_currents(self, n, echostep=0.1, startval=0):
        return (startval + np.arange(-n // 2 + 1, n // 2 + 1, 1) * echostep).tolist()
    ...
    def valueInfo(self):
        ...
        for imgdet in self._attached_detector._attached_images:
            if getattr(imgdet, "mode", None) in ["image"]:
                return (
                    Value(
                        "fit.avg", unit="", type="other", errors="next", fmtstr="%.1f"
                    ),
                    Value(
                        "fit.avgErr",
                        unit="",
                    ...
                    Value(
                        "fit.phase", unit="", type="other", errors="next", fmtstr="%.3f"
                    ),
                    Value(
                        "fit.phaseErr",
                        unit="",
                    ...
                    Value(
                        "fit.freq", unit="", type="other", errors="next", fmtstr="%.1f"
                    ),
                    Value(
                        "fit.freqErr",
                        unit="",
