...
from nicos.guisupport.qt import QBrush, QColor, QPainter, QPen, Qt, QWidget, pyqtSlot
...
class MonoBlocks(NicosWidget, QWidget):
    ...
    def paintEvent(self, event):
        ...
        if dia <= 0:
            return
        dia_in = int(dia * 4 / 9.0)

        painter.setPen(self.p_black)
        painter.setWindow(-dia / 2, -dia / 2, dia, dia)
        v = painter.viewport()
        painter.setViewport(
            v.left() + (v.width() - dia) / 2, v.top() + (v.height() - dia) / 2, dia, dia
        )
        ...
        painter.drawPie(
            -dia // 2, -dia // 2, dia, dia, 16 * (90 - IN_WIDTH // 2), 16 * IN_WIDTH
        )
