...
def bin_mapping(
    xs, ys, zs, log=False, xscale=1, yscale=1, interpolate=100, minmax=None
):
    xss = array(xs) * xscale
    yss = array(ys) * yscale
    if log:
    ...
    xi, yi = mgrid[min(xss) : max(xss) : interpolate, min(yss) : max(yss) : interpolate]
...
def update_map(x, y, z):
    ...
    if options["mode"] == 0:
        ...
        kwds = {}
        im = fcn(
            xi, yi, zi, 20, extent=(xi[0][0], xi[-1][-1], yi[0][0], yi[-1][-1]), **kwds
        )
    figure.colorbar(im, ax=axes, fraction=0.05)
    colors = np.linspace(0, 1, len(data_x))
    axes.scatter(data_x, data_y, 15, colors, cmap="gray", linewidths=1, edgecolors="w")
