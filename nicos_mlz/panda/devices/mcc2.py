...
class MCC2core(Device):
    ...
    parameters = {
        "channel": Param(
            "Channel of MCC2 to use (X or Y)",
            type=oneof("X", "Y"),
            default="Y",
            prefercache=False,
        ),
        "addr": Param(
            "Address of MCC2 to use (0 to 15)",
            type=intrange(0, 15),
            default=0,
            prefercache=False,
        ),
        "temperature": Param(
            "Temperature of MCC-2 unit in °C", type=int, settable=False, volatile=True
        ),
    }
        ...
        if mode != SIMULATION:
            if not self.comm("IVR").startswith("MCC"):
                raise CommunicationError(
                    self, "No Response from Phytron, " "please check!"
                )
            self._pushParams()
            self.doReset()
...
class MCC2Monoframe(MCC2core, Readable):
    ...
    parameters = {
        ...
        "override": Param(
            "Manual override of detected monocode",
...
class MCC2Coder(MCC2core, NicosCoder):
    ...
    parameters = {
        "slope": Param(
            "Coder units per degree of rotation",
            type=float,
            default=1,
            settable=True,
            unit="1/main",
...
class MCC2Motor(MCC2core, NicosMotor):
    ...
    parameters = {
        "mccmovement": Param(
            "Type of movement, change behaviour of limit " "switches",
        ...
        "slope": Param(
            "Full motor steps per physical unit",
            type=float,
            default=1,
            unit="1/main",
            prefercache=False,
        ...
        "steps": Param(
            "Last position in steps", settable=True, type=int, prefercache=False
        ),
        "accel": Param(
            "Motor acceleration in physical units",
            prefercache=False,
            type=float,
            settable=True,
            unit="1/main**2",
        ...
        "idlecurrent": Param(
            "Current whenever motor is Idle",
            unit="A",
        ...
        "rampcurrent": Param(
            "Current whenever motor is Ramping",
            unit="A",
            type=floatrange(0, 2.5),
            settable=True,
            prefercache=False,
        ...
        "movecurrent": Param(
            "Current whenever motor is moving at speed",
            type=floatrange(0, 2.5),
            unit="A",
            prefercache=False,
            settable=True,
        ...
        "linear": Param(
            "Linear stage (as opposed to choppered stage)",
            type=bool,
            settable=False,
            volatile=True,
        ),
    ...
    def doWriteIdlecurrent(self, value):
        if self.linear:
            self.comm(
                "XP40S%d" % max(0, min(25, round(value / 0.05)))
            )  # SHOULD BE 0.04
        else:
    ...
    def doWriteMovecurrent(self, value):
        if self.linear:
            self.comm(
                "XP41S%d" % max(0, min(25, round(value / 0.05)))
            )  # SHOULD BE 0.04
        else:
    ...
    def doWriteRampcurrent(self, value):
        if self.linear:
            self.comm(
                "XP42S%d" % max(0, min(25, round(value / 0.05)))
            )  # SHOULD BE 0.04
        else:
    ...
    def doReadAccel(self):
        return float(self.comm("XP15R")) / float(self.microstep * abs(self.slope))
    ...
    def doWriteAccel(self, value):
        f = max(
            ...
            min(
                500000,
                4000 * round((value * (abs(self.slope) * self.microstep)) / 4000),
            ),
        )
