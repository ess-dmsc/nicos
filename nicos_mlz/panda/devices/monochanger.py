...
class SeqCheckStatus(SequenceItem):
    ...
    def run(self):
        ...
        if status not in stati:
            raise HWError(
                "Expected %r to have a Status of %r, but has "
...
class SeqCheckPosition(SequenceItem):
    def __init__(self, dev, position, *args, **kwargs):
        SequenceItem.__init__(
            self, dev=dev, position=position, args=args, kwargs=kwargs
        )
    ...
    def run(self):
        ...
        if pos not in positions:
            raise HWError(
                "Expected %r to have a Position of %r, but "
    ...
    def run(self):
        if self.value is not None:
            if getattr(self.obj, self.key) != self.value:
                raise HWError("%s.%s should be %r !" % (self.obj, self.key, self.value))
...
class Changer(BaseSequencer):
    attached_devices = {
        "lift": Attach(
            "Lift moving a mono up or down between 4 " "positions", Moveable
        ),
        "magazine": Attach(
            "Magazine holding the monos and having 4 " "positions", Moveable
        ),
        "liftclamp": Attach("Clamp holding the mono in the lift", Moveable),
        "magazineclamp": Attach("Clamp holding the mono in the magazine", Moveable),
        "tableclamp": Attach("Clamp holding the mono on the table", Moveable),
        "inhibitrelay": Attach("Inhibit to the remaining motor controllers", Moveable),
        "enable": Attach("To enable operation of the changer", Moveable),
        "magazineocc": Attach("Readout for occupancy of the magazine", Readable),
        "magazinestatus": Attach(
            "Readout for status of magazine load position", Readable
        ),
    }

    parameters = {
        "mono_on_table": Param(
            "Name of Mono on the Monotable",
        ...
        "exchangepos": Param(
            "dict of device names to positional values " "for changing monos",
            type=dict,
            settable=False,
            userparam=False,
        ),
        "precisionchange": Param(
    ...
    def PrepareChange(self):
        ...
        if self.mono_in_lift != "None":
            raise HWError(
                self,
                "There is mono %s in the lift, please change "
        ...
        self.log.info(
            "will change position of several devices to the "
        ...
        for devname, prec in self.precisionchange.items():
            ...
            self.log.debug(
                "changing precision of the %s device to the %f", devname, prec[1]
            )
        ...
        for devname, prec in self.precisionchange.items():
            ...
            self.log.debug(
                "changing precision of the %s device back to " "the %f",
                devname,
                prec[0],
            )
    ...
    def FinishChange(self):
        ...
        self.log.warning(
            "Please restart the daemon or reload the setups to " "init the new Mono:"
        )
    ...
    def _gen_mag2lift(self, magpos=None):
        ...
        seq.append(
            SeqSetAttr(self, "mono_in_lift", self.monos[self.positions.index(magpos)])
        )
    ...
    def _gen_lift2mag(self, magpos=None):
        ...
        seq.append(
            SeqCheckAttr(
                self, "mono_in_lift", values=[m for m in self.monos if m != "None"]
            )
        )
        # prepare magazin
    ...
    def _gen_lift2table(self):
        ...
        seq.append(
            SeqCall(self.log.info, "closing table grab and releasing lift clamp")
        )
    ...
    def _gen_table2lift(self):
        ...
        seq.append(
            SeqCheckAttr(
                self, "mono_on_table", values=[m for m in self.monos if m != "None"]
            )
        )
        # transfer mono to lift
        seq.append(SeqCall(self.log.info, "moving down the lift"))
        ...
        seq.append(
            SeqCall(self.log.info, "moving the lift with mono to parking position")
        )
        seq.append(SeqDev(self._attached_lift, "2"))  # park position
        seq.append(SeqDev(self._attached_tableclamp, "close"))
        return seq
    ...
    def change(self, old, whereto):
        ...
        if old not in self.monos + ["None"]:
            raise UsageError(
                self,
                "'%s' is illegal value for Mono, use one "
        ...
        if self.mono_on_table != old:
            raise UsageError(
                self,
                "Mono %s is not supposed to be on the "
        ...
        if whereto != "None":
            seq.extend(self._gen_mag2lift(self.positions[self.monos.index(whereto)]))
            seq.extend(self._gen_lift2table())
            seq.append(
                SeqDev(self._attached_magazine, self.shields[self.monos.index(whereto)])
            )
    ...
    def printstatusinfo(self):
        ...
        self.log.info(
            "PLC is %s",
            "enabled"
            if self._attached_enable.read() == 0xEF16
            else "disabled, you need to set enable_word to the " "correct value",
        )
        ...
        try:
            ...
            self.log.info(
                "magazine is at %r which is assigned to %s",
                magpos,
        ...
        if self._attached_magazinestatus.read() == "free":
            ...
            self.log.info(
                "magazine is currently occupied with monochromator "
