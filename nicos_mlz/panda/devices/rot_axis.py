...
class RefAxis(Axis):
    parameters = {
        "refpos": Param("Reference position", type=none_or(float), default=None),
        "autoref": Param(
        ...
        "refspeed": Param(
            "Motorspeed when referencing or None/0 to use normal" "speed setting",
            type=none_or(float),
            default=None,
        ),
    }
    ...
    def doStart(self, target):
        ...
        if (
            ...
            and not self._referencing
            and self.autoref
            and self._moves > abs(self.autoref)
        ):
    ...
    def doReference(self, gotopos=None):
        ...
        if self._mode not in [MASTER, MAINTENANCE]:
            if self._mode == SIMULATION:
                ...
                self.log.error(
                    "Can't reference if not in master or " "maintenance mode!"
                )
            return
        ...
        try:
            ...
            for stepsize in steps:
                ...
                self.log.error(
                    "Referencing: RefSwitch still active after " "%.1f %s, exiting!",
                    sum(steps),
                    self.unit,
                )
                self.start(oldpos)
            ...
            while not (refsw(m)) and tries > 0:
                ...
                try:
                    m.doStart(m.doRead() - stepsize / 3.0)
                except NicosError:
            ...
            self.poll()
            self.log.info(
                "Found Refswitch at %.2f, should have been at %.2f, " "lost %.3f %s",
                m.doRead(),
                self.refpos,
...
class RotAxis(RefAxis):
    ...
    parameters = {
        "wraparound": Param(
            "Axis wraps around above this value", type=float, default=360.0
        ),
        "wrapwaittime": Param(
            "Time to wait after wraparound",
            unit="s",
            settable=True,
            type=float,
            default=0.0,
        ...
        "switchspan": Param(
            "Size of value range where switch is active",
            unit="main",
            settable=True,
            default=100,
        ),
        "leaveswitchstep": Param(
            "Stepsize when leaving switch", unit="main", settable=True, default=3
        ),
    }
    ...
    def doReference(self, gotopos=None):
        ...
        if not (-self.wraparound <= self.refpos <= self.wraparound):
            raise ConfigurationError(
                self,
                "Refpos needs to be a float within "
        ...
        try:
            ...
            if not refsw(m):
                ...
                if tries == 0:
                    self.log.error(
                        "Referencing: switch not found after " "%.1f %s, exiting!",
                        self.wraparound,
                        self.unit,
                    )
                    self.doStart(oldpos)
            ...
            while refsw(m) and tries > 0:
                self.log.debug(
                    "at %f: another %d %f %s slots left to try",
                    m.doRead(),
                    tries,
                    self.leaveswitchstep,
                    self.unit,
                )
            ...
            self.poll()
            self.log.info(
                "Found refswitch at %.1f, should have been at %.1f, " "lost %.2f %s",
...
class VirtualRotAxisMotor(VirtualMotor):
    ...
    def _refswitch(self):
        return 200.0 <= self.doRead() <= 220.0
