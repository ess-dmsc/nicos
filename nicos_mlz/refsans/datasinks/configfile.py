...
class ConfigObjDatafileSinkHandler(DataSinkHandler):
    ...
    def _write_meas_comment(self, metainfo):
        ...
        self._data["measurement"]["Measurement Comment"] = metainfo["Exp", "remark"][0]
        ds = self.dataset
        scaninfo = ds.info.split("-")[-1].strip()
        if self._data:
            self._data["measurement"]["scaninfo"] = scaninfo
        try:
            self._data["measurement"]["usercomment"] = metainfo["det", "usercomment"][0]
        except (KeyError, IndexError):
            self._data["measurement"]["usercomment"] = "None"
        for dev in REFSANS:
            self._data["measurement"][dev] = metainfo["REFSANS", dev][0]
        # self._data['measurement']['Instrumentenverantwortlicher'] = metainfo[
        #     'Instrumentenverantwortlicher', 'value'][2]
        self._data["measurement"]["Sample Name"] = (
            "%s" % metainfo["Sample", "samplename"][0]
        )
        for dev in EXP:
    ...
    def _write_PlanB(self, metainfo):
        ...
        self._write_label_ext(
            metainfo, "Absolute_Positions", Slits, "value", Slits_PlanB_label
        )
        self._write_label_ext(
            metainfo, "Absolute_Positions", PlanB, "value", PlanB_label
        )
        self._write_label_ext(
            metainfo, "Absolute_Positions", ana4gpio, "value", ana4gpio_label
        )
    ...
    def _write_slits(self, metainfo):
        ...
        self._write_label_ext(metainfo, "Slits_mode", Slits, "mode", Slits_label)
    ...
    def _write_lateral(self, metainfo):
        self._write_label_ext(metainfo, "Lateral", lateral, "value", lateral_label)
        self._write_label_ext(
            metainfo, "Lateral", simple_lateral, "value", simple_lateral_label
        )
    ...
    def _write_optic(self, metainfo):
        ...
        self._write_label_ext(metainfo, "optic_mode", optic, "mode", optic_label)
    ...
    def _write_attenuators(self, metainfo):
        ...
        for tup in keys:
            if tup[1] == "value" and attenuator in tup[0]:
                ...
                self._data["Attenuators"][tup[0]] = metainfo[tup[0], "value"][0]
    ...
    def _write_analog_encoder(self, metainfo):
        ...
        for tup in keys:
            if tup[1] == "value" and analog_encoder in tup[0]:
                element_part.append(tup[0])
                self._data["analog_encoder"][tup[0]] = metainfo[tup[0], "value"][0]
            ...
            if (devname, "value") in metainfo:
                ...
                if devname == "chopper":
                    ...
                    for key in metainfo[devname, "value"][0].keys():
                        self._data["chopper_config"][key] = metainfo[devname, "value"][
                            0
                        ][key]
                else:
    ...
    def putResults(self, quality, results):
        ...
        * INTERRUPTED is for data that has been read after the counting was
        ...
        if quality == FINAL:
            dic = {}
            self._final_dic(self.dataset.detectors[0]._attached_images, ".sum", dic)
            self._final_dic(self.dataset.detectors[0]._attached_monitors, "", dic)
            self._final_dic(self.dataset.detectors[0]._attached_timers, "", dic)
            self._final_dic(self.dataset.detectors[0]._attached_counters, "", dic)
            if self._data:
                try:
    ...
    def _final_dic(self, att, tag, dic):
        for data in att:
            for info, val in zip(self.dataset.detvalueinfo, self.dataset.detvaluelist):
...
class ConfigObjDatafileSink(FileSink):
    ...
    parameters = {
        "filenametemplate": Param(
            ...
            userparam=False,
            settable=False,
            default=["%(proposal)s_" "%(pointcounter)08d.cfg"],
        ),
    }
