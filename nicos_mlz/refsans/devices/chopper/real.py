...
class ChopperMaster(ChopperBase, ChopperMasterBase):
    parameters = {
        "fatal": Param(
            "Emergency off: Frontswitch and Backplane",
            type=str,
            settable=False,
            volatile=True,
            userparam=True,
    ...
    def doWriteDelay(self, delay):
        self._write_controller("m4079=%dm4080=%%d" % 1, int(round(delay * 100)))
    ...
    def _commute(self):
        self.log.info(
            "commute: see in speed history: burst for 3min, "
    ...
    def doReference(self, *args):
        ...
        fatal = self.fatal
        if fatal and fatal != "ok":
            self.log.error("still fatal %s deny reset: clear Error in RACK2", fatal)
            # return
...
class ChopperDisc(ChopperBase, ChopperDiscBase, Moveable):
    ...
    def doStart(self, target):
        if self.speedup:
            ...
            session.delay(0.4)  # .1 is too short; .2 does not work correctly
            self.log.info("speed up >%s<", self._attached_comm.communicate("m4062"))
        self.log.info("set speed %d", target)
        if self.chopper != 1 or self.gear != 0:
            self.log.warning(
                "changed chopper:%d gear:%d edge:%s", self.chopper, self.gear, self.edge
            )
        ...
        if self.speedup:
            ...
            session.delay(0.4)
            self.log.info("speed down >%s<", self._attached_comm.communicate("m4062"))
    ...
    def doReadPhase(self):
        ...
        while set_to >= 360.0:
            set_to -= 360.0
        self.log.debug(
            "read %.2f phase  %.2f ref %.2f gear %d SB",
            res,
            set_to,
            self.reference,
            self.gear,
    ...
    def doWritePhase(self, value):
        ...
        self.log.info(
            "Disk %d angle %0.2f Phase %0.2f gear %d ref %.2f",
            self.chopper,
            value,
            set_to,
            self.gear,
        ...
        while set_to > (+180.0 / self.gear):
            set_to -= 360.0 / self.gear
        while set_to < (-180.0 / self.gear):
            set_to += 360.0 / self.gear
        # The Chopper understands "negative hundreths degrees"
        set_to = int(round(set_to * -100))  # SB
        self.log.debug("Disk %d Phase %d gear %d", self.chopper, set_to, self.gear)
        # Notausgang = 10
        # while Notausgang:
        #    Notausgang -= 1
    ...
    def doWritePos(self, target):
        if target == 0:
            raise ConfigurationError(
                self, "'%d' is an invalid value for " "parameter 'pos'" % target
            )
        self._attached_translation.move(target)


class ChopperDiscTranslation(ChopperDiscTranslationBase, ChopperBase, HasOffset, Motor):
    ...
    def doStart(self, target):
        ...
        session.delay(0.1)  # time is needed to transfer and initiate start
...
class ChopperDiscTranslationEncoder(ChopperBase, Readable):
    parameters = {
        "addr": Param(
            "addr of encodervalue", type=int, settable=False, mandatory=False, default=7
        ),
    }
