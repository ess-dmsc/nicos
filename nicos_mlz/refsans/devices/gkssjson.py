...
from nicos.core.errors import CommunicationError, ConfigurationError, NicosError
...
class JsonBase(Readable):
    ...
    parameters = {
        "url": Param("URL reading the values", type=str),
        "timeout": Param("timeout to get an answers from URL", default=0.1),
        "valuekey": Param(
            "Key inside the json dict, only to proof comm",
            type=str,
        ),
    }
...
class SdsRatemeter(JsonBase):
    ...
    parameters = {
        ...
        "masks": Param(
            "Maps mask to attached devices positions", type=dict, mandatory=True
        ),
        "controlurl": Param("URL to the control side", type=str, userparam=False),
    }
    ...
    def doWriteMode(self, target):
        ...
        try:
            ...
            if res.status_code != 200:
                self.log.error("write mode: res %d %s", res.status_code, res.reason)
            res.close()
    ...
    def clear(self):
        ...
        try:
            res = requests.post(
                self.controlurl, timeout=self.timeout, data={"mon_button": "1"}
            )
            if res.status_code != 200:
