...
class BeckhoffBase(PyTangoDevice):
    ...
    parameters = {
        "address": Param(
            "Starting offset (words) of Motor control Block",
            type=motoraddress,
            mandatory=True,
            settable=False,
            userparam=False,
        ...
        "firmware": Param(
            "Firmware Version of motor control block",
            type=str,
            default="?",
            unit="",
            settable=False,
    ...
    def _steps2phys(self, steps):
        ...
        self.log.debug(
            "_steps2phys ruler: %r steps -> %s", steps, self.format(value, unit=True)
        )
        return value
    ...
    def _phys2steps(self, value):
        ...
        self.log.debug(
            "_phys2steps ruler: %s -> %r steps", self.format(value, unit=True), steps
        )
        return steps

    def _speed2phys(self, speed):
        self.log.debug(
            "_speed2phys: %r speed -> %s/s", speed, self.format(speed, unit=True)
        )
    ...
    def _phys2speed(self, value):
        ...
        self.log.debug(
            "_steps2phys: %s/s -> %r speed", self.format(value, unit=True), speed
        )
        return speed
    ...
    def _HW_status(self):
        ...
        if errval:
            ...
            return status.ERROR, "ERROR %d: %s, %s" % (
                errnum,
                self.HW_Errors.get(errnum, "Unknown Error {0:d}".format(errnum)),
                msg,
            )
...
class BeckhoffMotorBase(PolynomFit, CanReference, BeckhoffBase, BaseSequencer):
    ...
    parameters = {
        ...
        "potentiometer": Param(
            "Motor potentiometer calibrated value",
            type=float,
            settable=False,
            userparam=True,
            volatile=True,
        ...
        "firmware": Param(
            "firmware version", type=str, settable=False, userparam=True, volatile=True
        ),
        "maxtemp": Param(
    ...
    def _HW_readParameter(self, index):
        if index not in self.HW_readable_Params:
            raise UsageError("Reading not possible for parameter index %s" % index)
        ...
        for i in range(10):
            ...
            if (stat & 0x4000) != 0:
                raise UsageError(
                    self,
                    "NACK ReadPar command not recognized "
    ...
    def _HW_writeParameter(self, index, value, store2eeprom=False):
        if index not in self.HW_writeable_Params:
            raise UsageError("Writing not possible for parameter index %s" % index)
        ...
        if store2eeprom:
            if self._HW_readStatusWord() & (1 << 6) == 0:
                ...
                raise UsageError(
                    self, "Param acces no possible until target " "reached"
                )
    ...
    def _HW_wait_while_BUSY(self, timeout=100):
        ...
        while timeout > 0:
            ...
            if (
                (statval & 0b10100000 == 0)
                and (statval & 0b100000000)
                and (statval & 0b1)
            ):
                return
    ...
    def _HW_wait_while_HOT(self):
        ...
        for a in range(anz):
            ...
            if temp < self.maxtemp:  # wait if temp>33 until temp<26
                ...
                return True
            self.log.info("%d Celsius Timeout in: %.1f min", temp, (anz - a) * sd / 60)
            session.delay(sd)
        raise NicosTimeoutError("HW still HOT after {0:d} min".format(self.waittime))
    ...
    def _generateSequence(self, target):
        ...
        seq.append(SeqMethod(self, "_writeDestination", self._phys2steps(target)))
...
class SingleMotorOfADoubleMotorNOK(AutoDevice, Moveable):
    ...
    parameters = {
        "index": Param(
            "right side of nok", type=oneof(0, 1), settable=False, mandatory=True
        ),
    }
    ...
    def doStart(self, target):
        self.log.debug("SingleMotorOfADoubleMotorNOK for %s to %s", self.name, target)
...
class DoubleMotorBeckhoff(PseudoNOK, BeckhoffMotorBase):
    ...
    parameter_overrides = {
        "ruler": Override(
            type=tupleof(float, float), mandatory=True, settable=False, userparam=False
        ),
    }
    ...
    def doReadPotentiometer(self):
        return [self._fit(a) for a in self._HW_readParameter("potentiometer")][0]
    ...
    def _indexReadPosition(self, i):
        return self._asteps2phys(self._areadPosition(self.addresses[i]), self.ruler[i])
    ...
    def _indexHW_status(self, i):
        ...
        statval = (
            self._areadStatusWord(self.addresses[i]) ^ self.HW_Status_Inv
        ) & ~self.HW_Status_Ign
    ...
    def _generateSequence(self, target, indexes, code):
        ...
        for i in indexes:
            ...
            seq.append(
                SeqMethod(self, "_awriteControlBit", 0, code, 10, self.addresses[i])
            )
        return seq
...
class SingleSideRead(Readable):
    ...
    parameters = {
        "index": Param("side of nok", type=oneof(0, 1), settable=False, mandatory=True),
    }
...
class DoubleMotorBeckhoffNOK(HasAutoDevices, DoubleMotorBeckhoff):
    ...
    parameters = {
        ...
        "nok_motor": Param(
            "Position of the motor for this NOK",
            type=tupleof(float, float),
            settable=False,
            unit="mm",
            category="general",
        ),
        "inclinationlimits": Param(
            "Allowed range for the positional " "difference",
            type=limits,
            mandatory=True,
        ),
        "backlash": Param(
            "Backlash correction in phys. units", type=float, default=0.0, unit="main"
        ),
        "offsets": Param(
            "Offsets of NOK-Motors (reactor side, sample side)",
            type=tupleof(float, float),
            default=(0.0, 0.0),
            settable=False,
            unit="main",
    ...
    parameter_overrides = {
        ...
        "masks": Override(
            type=dictwith(**{name: float for name in MODES}), unit="", mandatory=True
        ),
    ...
    def doIsAllowed(self, target):
        ...
        target_r, target_s = (target[0] + self.offsets[0], target[1] + self.offsets[1])
    ...
    def doStop(self):
        self.log.debug(
            "DoubleMotorBeckhoffNOK doStop %s %s", self.addresses, self.address
        )
        bit = 6
        value = 1
        numbits = 1
        self.log.debug(
            "_writeControlBit doStop DoubleMotorBeckhoffNOK %r, %r", bit, value
        )
