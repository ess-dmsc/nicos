...
class TubeAngle(HasLimits, Moveable):
    ...
    parameters = {
        "yokepos": Param(
            "Position of yoke from pivot point",
    ...
    def doReadAbslimits(self):
        ...
        return (
            degrees(atan2(yokelimits[0], self.yokepos)),
...
class DetAngle(HasLimits, Moveable):
    ...
    parameters = {
        "pivot1pos": Param(
            "Distance of the pivot point 1 from wall",
            type=floatrange(0),
            mandatory=False,
            default=290,
            unit="mm",
        ...
        "b2pos": Param(
            "Distance of B2 aperture from wall",
            type=floatrange(0),
            mandatory=False,
            default=165,
            unit="mm",
        ...
        "b3pos": Param(
            "Distance of B3 aperture from wall",
            type=floatrange(0),
            mandatory=False,
            default=285,
            unit="mm",
        ...
        "detoffset": Param(
            "Offset from virtual tube base to lower edge of " "detector",
            type=floatrange(0),
            mandatory=False,
            default=619.772,
            unit="mm",
        ...
        "beamheight": Param(
            "Height of beam above ground level",
            type=floatrange(0),
            mandatory=False,
            default=1193.7,
            unit="mm",
        ...
        "range": Param(
            "range of angles between upper and lower detector edge",
    ...
    def _update(self, maxage=0):
        ...
        self._xoffset = (
            self.pivot1pos
            + pivot.grid * (pivot.read(maxage) - 1)
            - (self.samplepos + self.b3pos)
        )
    ...
    def doReadRange(self):
        alpha = self.doRead(0)
        opening = degrees(atan2(self.detheight, self._tpos)) / 2.0
