...
class SingleSlit(PseudoNOK, HasOffset, Moveable):
    ...
    parameters = {
        ...
        "_offsets": Param(
            "List of offsets per mode position",
            settable=False,
            internal=True,
    ...
    def doRead(self, maxage=0):
        return self._attached_motor.read(maxage) - self.masks[self.mode] - self.offset
    ...
    def doStart(self, target):
        self._attached_motor.start(target + self.masks[self.mode] + self.offset)
...
class DoubleSlit(PseudoNOK, Moveable):
    ...
    parameters = {
        ...
        "maxheight": Param(
            "Max opening of the slit", type=floatrange(0), settable=False, default=12.0
        ),
        "opmode": Param(
            "Mode of operation for the slit",
    ...
    def doInit(self, mode):
        ...
        for name, idx, opmode in [("center", 0, CENTERED), ("opening", 1, CENTERED)]:
            self.__dict__[name] = SingleSlitAxis(
                "%s.%s" % (self.name, name),
                slit=self,
                unit=self.unit,
                visibility=(),
    ...
    def _calculate_slits(self, arg, direction):
        self.log.debug(
            "calculate slits: dir:%s mode:%s arg %s", direction, self.mode, str(arg)
        )
        if direction:
            reactor, sample = arg
    ...
    def doRead(self, maxage=0):
        return self._calculate_slits(
            [self._attached_slit_r.read(maxage), self._attached_slit_s.read(maxage)],
            True,
        )
    ...
    def doIsAllowed(self, targets):
        ...
        try:
            ...
            why.append("%s" % e)
        for dev, pos in zip(self._motors, self._calculate_slits(targets, False)):
            ok, _why = dev.isAllowed(pos)
            if not ok:
                why.append(
                    "%s: requested position %.3f %s out of limits; %s"
                    % (dev, pos, dev.unit, _why)
                )
            else:
                self.log.debug(
                    "%s: requested position %.3f %s allowed", dev, pos, dev.unit
                )
        if why:
    ...
    def doStart(self, targets):
        ...
        for dev, target in zip(
            [self._attached_slit_r, self._attached_slit_s],
...
class DoubleSlitSequence(BaseSequencer, DoubleSlit):
    ...
    def _generateSequence(self, target):
        ...
        if True and False:  # pylint: disable=condition-evals-to-constant
            safe_seq = []
            step = (target[0] + 0.5001) * np.sign(dif)
            akt = center + step
            while True:
                if akt == target[1]:
...
class SingleSlitAxis(AutoDevice, Moveable):
    ...
    parameters = {
        "index": Param(
            "Which index of the super slit is used for this device",
            type=int,
            userparam=False,
        ),
        "opmode": Param(
            "Mode of the super slit to be used for this device",
            type=str,
            userparam=False,
            category="experiment",
        ),
