...
class Base(Readable):
    ...
    parameters = {
        "stringserver": Param(
            "string server", type=oneof("pop", "read"), settable=True, default="pop"
        ),
        "data": Param(
            "stored data", internal=True, type=str, settable=True, default=""
        ),
    }
...
class MoveName(HasLimits, ReadName, Moveable):
    parameters = {
        "speed": Param(
            "speed for specific action",
            type=floatrange(0, 600),
            unit="cm x cm/min",
            default=600,
            settable=True,
        ),
...
class Press(HasPrecision, MoveName):
    parameters = {
        "plate_perimeter": Param(
            "perimeter of plate",
            type=floatrange(10, 30),
            mandatory=False,
            settable=True,
            userparam=True,
            default=21,
        ),
        "calibration_weight": Param(
            "perimeter of plate",
            type=floatrange(50, 200),
            mandatory=False,
            settable=True,
            userparam=True,
            default=101.6,
