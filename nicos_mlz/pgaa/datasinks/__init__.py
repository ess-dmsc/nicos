...
from nicos.devices.datasinks.image import ImageFileReader, SingleFileSinkHandler
...
class SinkHandler(SingleFileSinkHandler):
    ...
    def _createFile(self, **kwargs):
        if self._file is None:
            ...
            templates = [
                t + "%s%s" % (path.extsep, self.filetype)
    ...
    def putMetainfo(self, metainfo):
        ...
        self._addinfo["Beam"] = (
            "O"
            if metainfo[self.detector.name, "enablevalues"][0][0] == "closed"
            else ""
        )
        self._addinfo["Vacuum"] = "V" if _value("chamber_pressure") < 10.0 else ""
    ...
    def _write_file(
        self, fp, addinfo, livetime, truetime, spectrum, ecalslope, ecalintercept
    ):
...
class MCASinkHandler(SinkHandler):
    ...
    def _write_file(
        ...
        spectr_name = array("b", "{:<26}".format(addinfo.get("Name", "")[:26]).encode())
...
class CHNSinkHandler(SinkHandler):
    ...
    def _write_file(
        ...
        s = "{:8}".format(
            datetime.fromtimestamp(int(self.dataset.started)).strftime("%d%b%y1")
        )
...
class Sink(FileSink):
    ...
    parameter_overrides = {
        ...
        "filenametemplate": Override(
            default=[
                "%(Prefix)s%(pointcounter)05d_%(Pos)02d-%(Name)s_"
...
class CSVSinkHandler(DataSinkHandler):
    ...
    def end(self):
        ...
        addinfo["Beam"] = self._metainfo[self.detector.name, "enablevalues"][0][0]
        ...
        addinfo["started"] = datetime.fromtimestamp(self.dataset.started).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        addinfo["stopped"] = datetime.fromtimestamp(self.dataset.finished).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        ...
        with self.manager.createDataFile(
            self.dataset, self._template, self.sink.subdir, fileclass=CSVDataFile
        ) as fp:
            ...
            writer = DictWriter(fp, fieldnames=fieldnames, extrasaction="ignore")
            if not fp.tell():
                writer.writeheader()
...
class CSVDataSink(FileSink):
    ...
    parameters = {
        "lastuid": Param(
            "UUID of the last handled dataset",
            settable=False,
            type=str,
            internal=True,
            mandatory=False,
        ...
        "filecount": Param(
            "Last value of the point counter",
            settable=False,
            type=int,
            internal=True,
            mandatory=False,
        ...
        "datafilenametemplate": Param(
            "Template for data file name",
            type=str,
            settable=False,
            prefercache=False,
