...
class BasicScriptHandler:
    def __init__(self):
        try:
            with open(
                path.join(my_uipath, "basic_script.py"), "r", encoding="utf-8"
            ) as f:
                basic_script = f.read()
        except OSError:
            basic_script = ""
...
class PGAAPanel(Panel):
    ...
    def __init__(self, parent, client, options):
        ...
        self.attenuatorLed1 = Led(
            "att1", "in", "out", client, colorActive="blue", colorInactive="yellow"
        )
        self.AttLayout.addWidget(self.attenuatorLed1)
        self.attenuatorLed2 = Led(
            "att2", "in", "out", client, colorActive="blue", colorInactive="yellow"
        )
        self.AttLayout.addWidget(self.attenuatorLed2)
        self.attenuatorLed3 = Led(
            "att3", "in", "out", client, colorActive="blue", colorInactive="yellow"
        )
        ...
        self.log_table = TableWidget(LogSource(client, self), Log.column_order, self)
        ...
        self.pattern_table = TableWidget(Template(client, self), Row.column_order, self)
...
class TableWidget(QTableWidget):
    ...
    def dropEvent(self, e):
        row_at = (
            self.empty_row if self.empty_row is not None else self.rowAt(e.pos().y())
        )
    ...
    def _create_elements(self, **kwargs):
        ...
        for value in self.par2cell:
            ...
            if value not in self.par2cell or self.par2cell[value] is CellItem:
                ...
                w = self.par2cell[value](self, parent=self, state=kwargs[value])
            items.append(w)
...
class Measurement(BasicScriptHandler, Row):
    ...
    suffix_re = re.compile(
        r"(?P<att>\d*)(?P<elcol>[CE])(?P<beam>O?)" "(?P<vacuum>V?)(?P<suffix>.*)"
    )
    ...
    def __init__(self, parent=None, init_script=None):
        ...
        self.data = {
            key.replace("/", "_").replace(" ", "_"): None for key in self.column_order
        }
        self.data.update(
            {
                key.replace("/", "_").replace(" ", "_"): Row.par2cell[
                    key
                ].standard_value
                for key in Row.par2cell
            }
        )
        if isinstance(init_script, dict):
            ...
            self.data.update(
                {"Name": tmp[1].split("-")[1], "Suffix": self._extract_suffix(tmp[-1])}
            )
        else:
        ...
        if status["script"].startswith("# PGAA"):  # PGAA script runs
            ...
            for request in status["requests"]:
                if request["reqid"] == self.reqid:
                    self._client.tell("update", newScript, "PGAAPanel", self.reqid)
    ...
    def on_update(self, newreq):
        ...
        for key, data in new_data.items():
            ...
            if k == "at_after":
                ...
                k = "stop by"
            if "__%s__" % key in self.basic_script and k in self.column_order:
...
class Log(Row):
    ...
    def __init__(self, data, controller, parent=None):
        self.stop_cond_cells = {}
        if "started" in data:
            data["started"] = datetime.fromtimestamp(data["started"]).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        if "stopped" in data:
            data["stopped"] = datetime.fromtimestamp(data["stopped"]).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
...
class QueueSource(QObject):
    ...
    def on_newsequence(self, requests):
        ...
        for new_ind, request in enumerate(requests):
            ...
            if not request == local_ids()[new_ind]:
                if (
                    local_ids()[new_ind + 1] == request
                    and requests[new_ind + 1] == local_ids()[new_ind]
                ):
                    index = new_ind
                    next_ind = new_ind + 1
    ...
    def queue_script(self, script_data=""):
        ...
        # is empty
        if not self.sequence and not self._break_reqs and self._breakreqid is None:
    ...
    def on_processing(self, processing_req):
        ...
        if (
            processing_req is not None
            and processing_req["name"] is not None
            and processing_req["name"].startswith("PGAAPanel")
        ):
    ...
    def insert_new(self, index, source):
        ...
        if isinstance(source.source, Template):
            ...
            script_data = Measurement(
                init_script=source.source.sequence[logind].data
            ).get_script()
        else:
            return
        ...
        if reqid is not None:
            ...
            new_measurement = QueuedMeasurement(self._client, request, self.parent())
...
class LogSource(QObject):
    ...
    def on_new_dataset(self, dataset):
        ...
        if hasattr(dataset, "scaninfo"):
            ...
            info["Detectors"] = "[%s]" % ",".join(d for d in dataset.detectors)
            info["Pressure"] = (
                "%.3f mbar" % dataset.metainfo["chamber_pressure", "value"][0]
            )
            ...
            for cond in ["TrueTime", "LiveTime", "ClockTime", "cond"]:
                if cond in dataset.preset:
                    ...
                    break
            info["Beam"] = dataset.metainfo[dataset.detectors[0], "enablevalues"][0][0]
...
class ProgressTracker(BasicScriptHandler, QObject):
    ...
    def on_status(self, statusline):
        ...
        if status in (STATUS_IDLE, STATUS_IDLEEXC):
            if (self.basic_script.count("\n") + 1) == line or status == STATUS_IDLEEXC:
                self.clear_widgets()
