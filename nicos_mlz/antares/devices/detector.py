...
from nicos.core.params import Value, floatrange, intrange, none_or, oneof, tupleof
    ...
    def openFastshutter(self):
        ...
        if self.openfastshutter:
            ...
            if fastshutter.status(0)[0] == status.ERROR:
                self.log.warning(
                    "resetting fast shutter before opening: it is" " in error state"
                )
                fastshutter.reset()
...
def absolute_win_path(val=ntpath.join("C:", ntpath.sep)):
    ...
    if ntpath.isabs(val) and ntpath.splitdrive(val)[0]:
        return val
    raise ValueError(
        "%r is not a valid absolute path (should start with drive"
        " name like C:)" % val
    )


class AndorHFRCamera(PyTangoDevice, UsesFastshutter, ImageChannelMixin, ActiveChannel):
    ...
    parameters = {
        ...
        "spooldirectory": Param(
            "Path to spool the images on the remote " "machine",
            type=absolute_win_path,
            category="general",
        ),
        "extratime": Param(
            "Extra sleeping time to sync with Andors Basic",
            type=floatrange(0),
            default=3,
            settable=True,
        ),
        "_started": Param(
            "Cached counting start flag",
            type=none_or(float),
            default=None,
            settable=True,
            internal=True,
        ...
        try:
            ...
            if msg.strip() != "ready!":
                raise CommunicationError(
                    self, "Camera script gave wrong " "answer - please check!"
                )
        except NicosError:
            self.log.warning(
                "Camera is not responding - please start "
    ...
    def doSetPreset(self, **presets):
        ...
        self._write_presets(
            "spoolfile", presets.get("spoolfile", session.experiment.sample.samplename)
        )
    ...
    def _write_command(self, command):
        ...
        if not ret.startswith("ACK"):
            if ret.startswith("ERR"):
                raise InvalidValueError(self, "Command: %s is invalid " % command)
            raise InvalidValueError(
                self, "Command: %s has invalid " "parameters" % command
            )
...
class Sharpness(PostprocessPassiveChannel):
    parameters = {
        ...
        "sig_log": Param(
            "Sig log", type=floatrange(0, 1), settable=True, userparam=True, default=0.8
        ),
    }
