    ...
    * above 300K, the CCR/coldhead/transfer point stays at 300K,
    ...
    parameters = {
        "regulationmode": Param(
            "Preferred regulation point: stick or tube",
            unit="",
            type=oneof("stick", "tube", "both"),
            settable=True,
            chatty=True,
            category="general",
        ),
    ...
    parameter_overrides = {
        "unit": Override(default="K", type=oneof("K"), settable=False, mandatory=False),
        ...
        if mode != SIMULATION:
            if self._attached_stick is None or self._attached_tube is None:
                raise ConfigurationError(
                    self, "Both stick and tube needs to " "be set for this device!"
                )
    ...
    def __start_tube_stick(self, tubetarget, sticktarget):
        ...
        if not ok:
            raise InvalidValueError(self, why)
        self.log.debug(
            "Moving %s to %r and %s to %r",
            self._attached_tube.name,
            tubetarget,
            self._attached_stick,
            sticktarget,
        )
        try:
        ...
        if target < self._attached_tube.absmax:
            if self.regulationmode == "stick":
                ...
                raise ProgrammingError(
                    self,
                    "unknown mode %r, don't know how"
                    " to handle it!" % self.regulationmode,
                )
        else:
            self.log.debug(
                "ignoring mode, as target %r is above %s.absmax",
                target,
                self._attached_tube.name,
            )
    ...
    def doRead(self, maxage=0):
        ...
        if self.regulationmode in ("stick", "both"):
            ...
            raise ProgrammingError(
                self,
                "unknown mode %r, don't know how to "
    ...
    def __get_param(self, attrname):
        ...
        if tubeval == stickval:
            ...
            self.log.warning(
                "%s.%s (%r) != %s.%s (%r), please set %s.%s to " "the desired value!",
                self._attached_tube.name,
                attrname,
                tubeval,
                self._attached_stick.name,
                attrname,
                stickval,
                self.name,
                attrname,
            ...
            if (
                self._attached_stick.target is not None
                and self._attached_stick.target > self._attached_tube.absmax
            ):
                res = stickval
            else:
                if self.regulationmode == "stick":
    ...
    def doWriteRamp(self, value):
        ...
        if value == 0.0:
            value = rampmax
            self.log.warning(
                "Ramp rate of 0 is deprecated, using %d " "K/min instead", value
            )
    ...
    def doWriteRegulationmode(self, value):
        self.log.info(
            "To use the new regulationmode %r, please start/move " "%s...",
            value,
            self.name,
        )
