...
class HighVoltagePowerSupply(PyTangoDevice, BaseSequencer):
    ...
    parameters = {
        "waterflow": Param(
            "waterflow of the cooling water",
            type=float,
            settable=False,
            unit="l/min",
            volatile=True,
        ...
        "heatercurrent": Param(
            "heater current of the x-ray tube",
            type=float,
            settable=False,
            unit="A",
            volatile=True,
        ...
        "ramp": Param(
            "ramp of the current and voltage",
            type=float,
            settable=True,
            unit="unit/min",
        ),
        "switchdelay": Param(
            "time to switch between voltage and current "
            "ramping, is needed to avoid cooling water "
            "switching off",
            type=floatrange(1, 300),
            default=60,
            unit="s",
            userparam=False,
            settable=True,
    ...
    def _generateSequence(self, target):
        ...
        if vchannel.voltage < 1 or cchannel.current < 1:
            return self._onseq(True) + [SeqSleep(5)] + self._vcseq(voltage, current)
    ...
    def doTime(self, old_value, target):
        return (
            sum(
                dev.doTime(ov, t)
                for (dev, ov, t) in zip(
                    (self._attached_voltage, self._attached_current), old_value, target
                )
            )
            + self.switchdelay
        )
