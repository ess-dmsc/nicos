...
class KeyenceImage(ImageChannelMixin, ActiveChannel):
    parameters = {
        ...
        "rotation": Param(
            "Rotation of the original read out image",
    ...
    def _trigger(self):
        ...
        try:
            ...
            if msg != cmd:
                self.log.warning("Wrong reply from Keyence was received: %s", msg)
        except socket.error:
    ...
    def doReadArray(self, quality):
        ...
        with urllib.request.urlopen(
            ...
            arr = np.rot90(
                np.asarray(Image.open(io.BytesIO(r.read()))), self.rotation // 90
            )
