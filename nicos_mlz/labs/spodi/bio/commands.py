...
def imgFit(im):
    ...
    psdVgt_bounds = (
        [1500, 0, 0, lower_limit, -100, 0, 0, lower_limit, 1500, 0, 0, lower_limit, 0],
        [
            2500,
            1,
            10,
    ...
    p0_init = (2500, 1, 5, mid, 500, 1, 5, mid, 2500, 1, 5, mid, 0)

    x_fit = []
    y_fit = []
    x_data = np.linspace(lower_limit, upper_limit, (upper_limit - lower_limit))
    for xline in range(roi[1], roi[1] + roi[3], 10):
        ...
        try:
            popt, _pcov = curve_fit(
                psdVgt, x_data, y_data, p0=p0_init, bounds=psdVgt_bounds, method="trf"
            )
            ...
            if r_squared >= 0.98:
                ...
                p0_init = (
                    ...
                    1,
                    5,
                    x_data[int(y_data.index(max(y_data)))],
                    500,
                    1,
                    5,
                    x_data[int(y_data.index(max(y_data)))],
