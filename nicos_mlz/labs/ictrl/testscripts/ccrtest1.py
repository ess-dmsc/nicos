...
def check_pressure(comment, outdev, outvalue, indev, inrange):
    ...
    if inrange[0] <= val <= inrange[1]:
        ...
        printerror("%20s : %s" % (comment, "FAILED : p = %.4f (%r)" % (val, inrange)))
    ...
    if indev.read() == "on":
        ...
        if st == "on":
            if compressor.status(0)[0] == ERROR:
                ...
                printerror(
                    "20%s : %s" % (comment, "FAILED : compressor not in" " ERROR state")
                )
            errdev.move("off")
        else:
            printerror(
                "%20s : %s" % (comment, "FAILED : compressor not " "switched on")
            )
    else:
...
check_compressor_errors(
    "Gas Pressure Error", tb_err_gaspressure, compressor, tb_compressor
)
check_compressor_errors("Motor Error", tb_err_motortemp, compressor, tb_compressor)
check_compressor_errors("Power Error", tb_err_power, compressor, tb_compressor)
check_compressor_errors("Water inlet Error", tb_err_waterin, compressor, tb_compressor)
check_compressor_errors(
    "Water outlet Error", tb_err_waterout, compressor, tb_compressor
)
