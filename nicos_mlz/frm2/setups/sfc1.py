...
for i in range(1, 4):
    devices[f"pressure{i}"] = device(
        "nicos_mlz.devices.cetoni.CetoniSensor",
        description=f"Sensor {i}",
        tangodevice=tango_base + f"pressure/sensor{i}",
        fmtstr="%.2f",
        pollinterval=300,
    )
    devices[f"pump{i}"] = device(
        "nicos_mlz.devices.cetoni.CetoniPump",
...
for mod in modules:
    ...
    help_topics["Cetoni commands"] = help_topics["Cetoni commands"].join(
        [topic(i) for i in dir(mod) if callable(getattr(mod, i)) and i not in excluded]
    )
