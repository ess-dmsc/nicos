...
devices = {
    ...
    f"T_{setupname}_htstick": device(
        "nicos.devices.entangle.TemperatureController",
    ...
    f"{setupname}_vti_regulation": device(
...
alias_config = {
    "T": {
        f"T_{setupname}_vti": 220,
        f"T_{setupname}_stick": 210,
...
watch_conditions = [
    dict(
        condition="ccm8v_lhe_value < 30",
        type="se",
        setup="ccm8v",
        message="8T Magnet: He level below 30%",
    ),
    dict(
        condition="ccm8v_shutdown_value > 0",
        type="se",
        setup="ccm8v",
