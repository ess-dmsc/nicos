...
_selector = make_blocks(
    ...
    [
        ...
        BlockRow(
            Field(name="Lambda", dev="sel_lambda"), Field(name="Speed", dev="sel_speed")
        ),
    ],
)

_resolution = make_blocks(
    "Resolution",
    "resolution",
    [
        BlockRow(
            Field(name="Preset", dev="resolution", istext=True, width=17),
            Field(
                device="sel_ap2",
...
_julabo = Block("Sample T", [BlockRow("T")])

_julaboplot = Block(
    "",
    [
        BlockRow(
            Field(plot="T", dev="T", width=30, height=25, plotwindow=2 * 3600),
...
_layout = [
    ...
    Row(
        Column(*(_selector + _polarizer + _daq)),
