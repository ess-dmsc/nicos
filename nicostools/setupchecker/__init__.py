...
from nicos.core.sessions.setups import (
    SETUP_GROUPS,
...
interpol_re = re.compile(
    r"%[-#0 +]*(?:[0-9]+)?(?:\.(?:[0-9]*))?" r"[hlL]?[diouxXeEfFgGcrs%]"
)
    ...
    def _exec_all(self, setuplist):
        for setupname, filename in setuplist:
            ...
            try:
                ...
                ns["devices"] = fixup_stacked_devices(self.log, ns.get("devices", {}))
...
class SetupChecker:
    ...
    def _find_binding(self, binding):
        assign = [
            x
            for x in self.collection.setup_ast[self.filename].body
        ...
        if name == "fmtstr":
            if "%" not in value:
                ...
                for piece in pieces:
                    if "%" in piece:
                        self.log_error(
                            "%s: parameter fmtstr has an invalid " "placeholder (%r)",
                            devname,
                            piece,
                            extra=self.find_deventry(devname, name),
                        )
    ...
    def check_device(self, devname, devconfig, is_special=False):
        ...
        try:
            ...
            self.log.warning(
                "device class %r for %r not importable: %s",
                devconfig[0],
                devname,
                err,
                extra=self.find_deventry(devname),
            ...
            self.log_error(
                "could not get device class %r for %r:",
                devconfig[0],
                devname,
                extra=self.find_deventry(devname),
            )
            ...
            for aname, ainfo in cls.attached_devices.items():
                try:
                    ...
                    self.log_error(
                        ...
                        "wrongly configured: %s" % (devname, aname, cls.__name__, err),
        ...
        if not hasattr(cls, "parameters"):
            self.log.warning("%s: class %r has no 'parameters'", devname, cls.__name__)
        else:
            if "lowlevel" in config:
                self.log_error(
                    '%s: "lowlevel" parameter is given',
                    devname,
            ...
            for pname, pinfo in cls.parameters.items():
                if pname in config:
                    ...
                    try:
                        ...
                        self.log_error(
                            "%s: parameter %r value %r is " "invalid: %s",
                            devname,
                            pname,
                            config[pname],
                            e,
                    ...
                    self.log_error(
                        "%s: mandatory parameter %r missing",
                        devname,
                        pname,
                        extra=self.find_deventry(devname),
                    )
        if config:
            ...
            self.log_error(
                "%s: configured parameters not accepted by the " "device class: %s",
                devname,
                ", ".join(config),
    ...
    def check(self):
        ...
        if group != "special":
            for devname in devs:
                ...
                other_group = self.collection.namespace[other].get("group", "optional")
                if self_group == "basic" and other_group == "basic":
                    continue
                if setupname(other) in self.ns.get(
                    "excludes", []
                ) or self.setupname in self.collection.namespace[other].get(
                    "excludes", []
                ):
                    continue
                # it's also ok if it is a sample, experiment, or instrument
                # device
                if devname in ["Sample", "Exp"] or "instrument" in devs[devname][1]:
                    continue
                self.log.warning(
                    "device name %s duplicate: also in %s",
                    devname,
        ...
        if isinstance(aliascfg, dict):  # else we complained above already
            for aliasname, entrydict in aliascfg.items():
                if not (isinstance(aliasname, str) and isinstance(entrydict, dict)):
                    self.log_error(
        ...
        if not isinstance(display_order, int) or not 0 <= display_order <= 100:
            self.log_error(
                "display_order should be an integer between " "0 and 100",
        ...
        for devname, devconfig in devs.items():
            self.check_device(devname, devconfig, group in ("special", "configdata"))
    ...
    def check_guiconfig_panel_spec(self, spec, context="main window"):
        ...
        if isinstance(spec, (guicfg.hsplit, guicfg.vsplit, guicfg.hbox, guicfg.vbox)):
            ...
            for child in spec[1]:
                if not (isinstance(child, tuple) and len(child) == 2):
                    self.log_error(
                        "dock item should be a (name, panel) tuple," " found %r", child
                    )
                else:
                    self.check_guiconfig_panel_spec(child[1], context)
            ...
            try:
                ...
                self.log_error(
                    "class %r for %s not importable: %s", spec.clsname, context, err
                )
            else:
                if qt and not issubclass(cls, Panel):
                    self.log.warning(
                        "class %r for %s is not a Panel " "subclass",
                        spec.clsname,
                        context,
                    )
    ...
    def check_guiconfig_tool_spec(self, spec):
        ...
        if isinstance(spec, guicfg.tool):
            try:
                ...
                self.log_error(
                    "class %r for tool %r not importable: %s",
                    spec.clsname,
                    spec.name,
                    err,
                )
                ...
                if qt and not issubclass(cls, (qt.QDialog, qt.QMainWindow)):
                    self.log.warning(
                        "class %r for tool %r is not a QDialog or" " QMainWindow",
                        spec.clsname,
                        spec.name,
                    )
            self.validate_setup_spec(spec)
        ...
        if group != "special":
            ...
            for helpname in helptopics:
                ...
                if setupname(other) in self.ns.get(
                    ...
                    continue
                self.log.error(
                    "Help topic name %s duplicate: also in %s",
                    helpname,
