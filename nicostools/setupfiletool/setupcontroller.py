...
def __walkForSetups(log, previousDir, newDir, listOfSetups):
    ...
    for item in os.listdir(os.path.join(previousDir, newDir)):
        if item.endswith(".py"):
            try:
                listOfSetups.append(Setup(os.path.join(previousDir, newDir, item), log))
            except RuntimeError as e:
                log.warning(e.message)
        elif os.path.isdir(os.path.join(previousDir, newDir, item)):
            __walkForSetups(log, os.path.join(previousDir, newDir), item, listOfSetups)
...
class Setup:
    def __init__(self, abspath, log):
        ...
        for deviceName, device in devs.items():
            self.devices[deviceName] = Device(deviceName, device[0], copy(device[1]))
