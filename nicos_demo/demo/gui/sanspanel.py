...
class SANSPanel(Panel):
    ...
    def updateStatus(self, status, exception=False):
        ...
        setBackgroundColor(
            self.curstatus, self._idle if status == "idle" else self._busy
        )
    ...
    def on_start_clicked(self):
        dpos = []
        for dp, cb in zip(
            [1, 2, 5, 10, 20], [self.dp1m, self.dp2m, self.dp5m, self.dp10m, self.dp20m]
        ):
            if cb.isChecked():
                dpos.append(dp)
        if not dpos:
        ...
        coll = (
            self.coll10.isChecked()
            and "10m"
            or (self.coll15.isChecked() and "15m" or "20m")
        )
        code = "maw(coll, %r)\nscan(det1_z, [%s], det, t=%.1f)\n" % (
            coll,
            ", ".join(str(x) for x in dpos),
            ctime,
        )
    ...
    def execScript(self, script):
        ...
        if self.current_status != "idle":
            ...
            qwindow.questionText.setText(
                "A script is currently running.  What" " do you want to do?"
            )
            icon = qwindow.style().standardIcon
            qwindow.iconLabel.setPixmap(
                icon(QStyle.StandardPixmap.SP_MessageBoxQuestion).pixmap(32, 32)
            )
            b0 = QPushButton(
                icon(QStyle.StandardPixmap.SP_DialogCancelButton), "Cancel"
            )
            b1 = QPushButton(
                icon(QStyle.StandardPixmap.SP_DialogOkButton), "Queue script"
            )
            b2 = QPushButton(
                icon(QStyle.StandardPixmap.SP_MessageBoxWarning), "Execute now!"
            )
            qwindow.buttonBox.addButton(b0, QDialogButtonBox.ButtonRole.ApplyRole)
            qwindow.buttonBox.addButton(b1, QDialogButtonBox.ButtonRole.ApplyRole)
            qwindow.buttonBox.addButton(b2, QDialogButtonBox.ButtonRole.ApplyRole)
