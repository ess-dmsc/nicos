...
devices = dict(
    med=device(
        "nicos_mlz.puma.devices.MultiDetectorLayout",
...
for i in range(11):
    ...
    devices["ctr%d" % (i + 1)] = device(
        "nicos.devices.generic.VirtualCounter",
        visibility=(),
        type="counter",
        countrate=1 + int(2000 * math.exp(-(((i + 1) - 6) ** 2) / 2.0)),
        fmtstr="%d",
    )
